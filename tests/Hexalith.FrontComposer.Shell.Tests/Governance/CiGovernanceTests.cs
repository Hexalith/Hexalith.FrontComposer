...
public sealed class CiGovernanceTests {
    ...
    // Quarantined trait + metadata comment must be removed in the same change.
    // ----------------------------------------------------------------------------
    ...
    public void Story12_4_Def14_AttestBuildProvenanceStep_IsWiredInReleaseWorkflow() {
        ...
        // `Run semantic-release` and binding the attestation bundle into the sealed
        // manifest.
        string root = RepositoryRoot();
        string workflow = File.ReadAllText(Path.Combine(root, ".github/workflows/release.yml"));

        workflow.Contains("actions/attest-build-provenance@v2", StringComparison.Ordinal).ShouldBeTrue(
            "AC9/Def14: release.yml must invoke actions/attest-build-provenance@v2 to produce real attestations before semantic-release publishes.");
        ...
        attestStepIdx.ShouldBeLessThan(
            semanticReleaseIdx,
            "AC9/Def14: attest-build-provenance must run before semantic-release so the attestation bundle is bound into the sealed manifest.");
    }
    ...
    public void Story12_4_Def14_AttestationsWritePermission_IsRestored() {
        ...
        workflow.Contains("attestations: write", StringComparison.Ordinal).ShouldBeTrue(
            "AC9/Def14: release.yml job permissions must include `attestations: write` so attest-build-provenance can sign and upload.");
        workflow.Contains("id-token: write", StringComparison.Ordinal).ShouldBeTrue(
            "AC9/Def14: release.yml job permissions must include `id-token: write` so the attestation step can mint an OIDC token for the signing provider.");
    }
    ...
    public void Story12_4_Def23_ManifestMissingFingerprints_NoRoot_IsBlocked() {
        ...
        File.WriteAllText(manifest, """
            {
              ...
              "packages": [],
              "seal": { "algorithm": "sha256", "hash": "0", "sealed_at": "2026-05-19T00:00:00+00:00" }
            }
            """);
        ...
        result.ExitCode.ShouldNotBe(
            0,
            "AC30/Def23: verify-manifest --no-root must fail closed when release_definition_fingerprints is empty (was exit 0).");
