...
public sealed class CiGovernanceTests {
    ...
    public void EventStoreRuntimeIdentitySeparatesCurrentCompatibilityFromHistoricalApproval() {
        ...
        currentCompatibility.GetProperty("eventStoreSourceSha").GetString().ShouldBe(sealedV2SourceSha);
        currentCompatibility.GetProperty("eventStorePackageVersion").GetString().ShouldBe(sealedV2Version);
        ...
        liveProviderLane.ShouldContain("set -euo pipefail");
        liveProviderLane.ShouldContain("provider_ledger=\"$RUNNER_TEMP/frontcomposer-provider-packages.json\"");
        liveProviderLane.ShouldContain("provider_packages=\"$(mktemp -d \"$RUNNER_TEMP/frontcomposer-provider-packages.XXXXXX\")\"");
        ...
        liveProviderLane.ShouldContain("--package-ledger \"$provider_ledger\"");
        liveProviderLane.ShouldContain("--package-root \"$provider_packages\"");
        int providerPackageRootIndex = liveProviderLane.IndexOf("provider_packages=", StringComparison.Ordinal);
        int providerPackageExportIndex = liveProviderLane.IndexOf("export NUGET_PACKAGES=", StringComparison.Ordinal);
        ...
        appHostLane.ShouldContain("set -euo pipefail");
        appHostLane.ShouldContain("apphost_packages=\"$(mktemp -d \"$RUNNER_TEMP/frontcomposer-apphost-packages.XXXXXX\")\"");
        appHostLane.ShouldContain("test ! -L \"$apphost_packages\"");
        ...
        int appHostPackageRootIndex = appHostLane.IndexOf("apphost_packages=", StringComparison.Ordinal);
        int appHostCaptureIndex = appHostLane.IndexOf("python3 eng/pact_provider_apphost_smoke.py", StringComparison.Ordinal);
        (appHostPackageRootIndex < appHostCaptureIndex).ShouldBeTrue();

        string liveCaptureValidationLane = ExtractNamedStep(quality, "Gate 2c: Validate current live capture");
        liveCaptureValidationLane.ShouldContain("set -euo pipefail");
        liveCaptureValidationLane.ShouldContain("python3 eng/eventstore_runtime_evidence.py \\");
        liveCaptureValidationLane.ShouldContain("--live-evidence-root _bmad-output/implementation-artifacts/evidence/pact-provider-reconciliation \\");
        ...
        publishCandidateLane.ShouldNotContain("|| true");

        string appHostSmokeSource = File.ReadAllText(Path.Combine(root, "eng/pact_provider_apphost_smoke.py"));
        appHostSmokeSource.ShouldContain("environment[\"NUGET_PACKAGES\"] = str(package_root)");
        appHostSmokeSource.ShouldContain("runtime_evidence.APPHOST_PACKAGE_ASSETS_ROOT");
        ...
        appHostCaptureSourceEnd.ShouldBeGreaterThan(appHostCaptureSourceStart);
        ...
        appHostCaptureSource.ShouldContain("_, package_issues = _resolved_apphost_package_ledger(");
        appHostCaptureSource.ShouldContain("package_ledger, package_issues = _resolved_apphost_package_ledger(");
        ...
        int appHostInitialLedgerIndex = appHostCaptureSource.IndexOf("_, package_issues = _resolved_apphost_package_ledger(", StringComparison.Ordinal);
        int appHostInitialLedgerSealedIndex = appHostCaptureSource.IndexOf("if package_issues:", appHostInitialLedgerIndex, StringComparison.Ordinal);
        int appHostExecutionIndex = appHostCaptureSource.IndexOf("evidence[\"executionStartedAt\"]", StringComparison.Ordinal);
        int appHostEvaluationIndex = appHostCaptureSource.IndexOf("source_graph_before = _evaluate_source_graph(", StringComparison.Ordinal);
        int appHostBuildIndex = appHostCaptureSource.IndexOf("prebuild = runtime.command(", StringComparison.Ordinal);
        int appHostFinalLedgerIndex = appHostCaptureSource.IndexOf("package_ledger, package_issues = _resolved_apphost_package_ledger(", appHostBuildIndex, StringComparison.Ordinal);
        int appHostFinalLedgerSealedIndex = appHostCaptureSource.IndexOf("if package_issues:", appHostFinalLedgerIndex, StringComparison.Ordinal);
        int appHostStartIndex = appHostCaptureSource.IndexOf("start = runtime.command([*START_COMMAND", StringComparison.Ordinal);
        (appHostRestoreIndex < appHostInitialLedgerIndex).ShouldBeTrue();
        (appHostInitialLedgerIndex < appHostInitialLedgerSealedIndex).ShouldBeTrue();
        (appHostInitialLedgerSealedIndex < appHostEvaluationIndex).ShouldBeTrue();
        (appHostEvaluationIndex < appHostBuildIndex).ShouldBeTrue();
        (appHostBuildIndex < appHostFinalLedgerIndex).ShouldBeTrue();
        (appHostFinalLedgerIndex < appHostFinalLedgerSealedIndex).ShouldBeTrue();
        (appHostFinalLedgerSealedIndex < appHostExecutionIndex).ShouldBeTrue();
        (appHostExecutionIndex < appHostStartIndex).ShouldBeTrue();

        string cleanupLane = ExtractNamedStep(quality, "Gate 2c: Clean package authority roots");
        cleanupLane.ShouldContain("if: always()");
        cleanupLane.ShouldContain("shell: bash");
        cleanupLane.ShouldContain("set -euo pipefail");
        cleanupLane.ShouldContain("for package_root in \"${FRONTCOMPOSER_PROVIDER_PACKAGES:-}\" \"${FRONTCOMPOSER_APPHOST_PACKAGES:-}\"; do");
        cleanupLane.ShouldContain("if [ -n \"$package_root\" ] && [ -d \"$package_root\" ] && [ ! -L \"$package_root\" ]; then");
        cleanupLane.ShouldContain("find \"$package_root\" -mindepth 1 -delete");
        cleanupLane.ShouldContain("rmdir \"$package_root\"");
