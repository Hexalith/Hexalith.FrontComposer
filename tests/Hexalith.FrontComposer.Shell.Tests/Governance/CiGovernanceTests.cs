...
public sealed class CiGovernanceTests {
    ...
    public void EventStoreRuntimeIdentitySeparatesCurrentCompatibilityFromHistoricalApproval() {
        ...
        appHostSmokeSource.ShouldContain("\"-p:GeneratePackageOnBuild=false\"");
        ...
        int appHostLedgerIndex = appHostCaptureSource.IndexOf("package_ledger, package_issues = runtime_evidence.resolved_package_ledger(", StringComparison.Ordinal);
        int appHostLedgerSealedIndex = appHostCaptureSource.IndexOf("if package_issues:", appHostLedgerIndex, StringComparison.Ordinal);
        ...
        (appHostRestoreIndex < appHostLedgerIndex).ShouldBeTrue();
        (appHostLedgerIndex < appHostLedgerSealedIndex).ShouldBeTrue();
        (appHostLedgerSealedIndex < appHostExecutionIndex).ShouldBeTrue();
        (appHostExecutionIndex < appHostEvaluationIndex).ShouldBeTrue();
        (appHostEvaluationIndex < appHostBuildIndex).ShouldBeTrue();
        (appHostBuildIndex < appHostStartIndex).ShouldBeTrue();
