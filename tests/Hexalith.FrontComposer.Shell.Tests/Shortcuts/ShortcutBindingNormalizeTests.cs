...
public class ShortcutBindingNormalizeTests {
    ...
    public void Normalize_LowercasesAndReordersModifiers(string input, string expected) => ShortcutBinding.Normalize(input).ShouldBe(expected);
    ...
    [InlineData("a")]
    [InlineData("Z")]
    public void Normalize_RejectsBareSingleLetterWithoutModifier(string input) => Should.Throw<ArgumentException>(() => ShortcutBinding.Normalize(input));

    [Theory]
    [InlineData("")]
    [InlineData(" ")]
    [InlineData("\t")]
    public void Normalize_RejectsEmptyOrWhitespace(string input) => Should.Throw<ArgumentException>(() => ShortcutBinding.Normalize(input));
