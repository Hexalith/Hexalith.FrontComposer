...
class PackReleasePackagesTests(unittest.TestCase):
    ...
    def test_plan_for_current_release_forwards_validation_to_every_command(self) -> None:
        result = self.run_plan("2.0.0-review.plan")

        self.assertEqual(0, result.returncode, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual("v2.0", payload["releaseLine"])
    ...
    def test_plan_rejects_release_before_suppressions_take_effect(self) -> None:
        result = self.run_plan("1.12.0")

        self.assertNotEqual(0, result.returncode)
        self.assertIn("targetRelease v2.0 is later than --version 1.12.0", result.stderr)

    def test_plan_rejects_release_when_suppressions_expire(self) -> None:
        result = self.run_plan("2.1.0")

        self.assertNotEqual(0, result.returncode)
        self.assertIn("expiresAfter v2.1 has been reached by --version 2.1.0", result.stderr)
    ...
    def test_plan_rejects_release_line_that_differs_from_ledger(self) -> None:
        ...
        payload["currentRelease"] = "v2.2"
        for row in payload["suppressions"]:
            row["targetRelease"] = "v2.0"
            row["expiresAfter"] = "v3.0"

        with tempfile.TemporaryDirectory() as directory:
            ledger = pathlib.Path(directory) / "compatibility-suppressions.json"
            ledger.write_text(json.dumps(payload), encoding="utf-8")
            result = self.run_plan("2.1.0", ledger)

        self.assertNotEqual(0, result.returncode)
        self.assertIn(
            "--version release line v2.1 does not match currentRelease v2.2",
            result.stderr,
        )
