...
public sealed class FcShellOptions {
    ...
    /// <summary>
    /// Gets or sets development-only overlay and starter-template generation options.
    /// </summary>
    public FcShellDevModeOptions DevMode { get; set; } = new();

    /// <summary>
    /// Gets or sets the runtime customization contract validation mode. Story 6-6 P17 / AC2.
    /// </summary>
    /// <remarks>
    /// <see cref="CustomizationContractValidationMode.LogAndSkip"/> (default) preserves the
    /// adopter-friendly behavior of skipping incompatible Major-mismatched override descriptors
    /// at registry hydration with a warning log. <see cref="CustomizationContractValidationMode.FailClosedOnMajorMismatch"/>
    /// honors AC2 strict wording: at startup, the registries' aggregated rejection log is read
    /// by a hosted service that throws <see cref="System.InvalidOperationException"/> when any
    /// Major-mismatch rejection was recorded so the application never boots with silently
    /// dropped Level 2 / 3 / 4 customization registrations.
    /// </remarks>
