...
Commands support deterministic success, rejection, immediate timeout, and stall-at-`Syncing`. Each test host emits the same canonical ULID sequence from a fresh instance, with distinct message and correlation identifiers for every dispatch. Queries and pages accept per-request callbacks with last-write-wins configuration. `TestAuthorizationEvaluator` is registered as the exact evaluator exposed by the host and fails closed for unknown policies.
...
Evidence formatting redacts configured tenant/user values and token, secret, password, authorization, API-key, cookie, private-key, or connection-string keyed values before truncation and assertion output. Cyclic, unsupported, or throwing payloads produce a bounded `<serialization-unavailable>` marker without changing the configured command outcome; process-fatal exceptions still propagate. The package public surface is pinned by `PublicAPI.Shipped.txt`; update that baseline intentionally when adding adopter-facing APIs.
