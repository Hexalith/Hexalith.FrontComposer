...
public static class CommandFormEmitter {
    ...
    private static void EmitSubmitMethod(StringBuilder sb, CommandFormModel form, CommandFluxorModel fluxor) {
        ...
        _ = sb.AppendLine("            }");
        _ = sb.AppendLine();
        _ = sb.AppendLine("            // Story 5-5 P17 / P18 — skip duplicate AcknowledgedAction dispatch when the resolver");
        _ = sb.AppendLine("            // already moved this MessageId to a terminal state on a parallel path, or when the");
        _ = sb.AppendLine("            // pending registry rejected the MessageId as malformed. The lifecycle service would");
        _ = sb.AppendLine("            // otherwise transition Acknowledged on top of an already-terminal entry, breaking the");
        _ = sb.AppendLine("            // exactly-once UI invariant.");
