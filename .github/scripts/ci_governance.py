...
def reintroduction(args: argparse.Namespace) -> int:
    ...
    if args.output:
        write_json(args.output, payload)
    if args.apply and action == "open-reintroduction-pr":
        require_trusted_context(args)
        raise SystemExit("reintroduction PR creation requires a reviewable source patch generated by the workflow")
    return 0
