...
The selected external maintainer owns the independent run and its dated result. Hexalith.Tenants is the selected maintainer until a dated Product decision selects a substitute. The Tenants maintainer records external acceptance in the Hexalith.Tenants repository at `_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.md`, the PRD's G-6/SM-1 evidence path. If Product makes the dated D-7 decision to select Hexalith.Parties, the Parties maintainer records equivalently candidate-bound Parties evidence for the same gate. Running this fixture inside FrontComposer is useful validation, but does not close EXT-ADOPTER-1.
...
Install Python 3, the .NET 10 SDK, and matching `Microsoft.NETCore.App` and `Microsoft.AspNetCore.App` runtime patch versions. Record the chosen runtime version from `dotnet --list-runtimes`. The runner pins the copied fixture to the newest installed stable .NET 10 SDK, so an installed preview or later SDK does not build it. Fluent UI Blazor comes from the candidate Shell package's pinned dependency, so the fixture never carries its own Fluent UI version. Supply an EventStore base endpoint for the real command/query registration; this render-only proof does not require that endpoint to respond. The proof only renders the generated surfaces; it does not send a command or save a domain payload.

Copy `samples/AdopterProofKit/v1` and `eng/adopter_proof.py` from the candidate SHA's checkout, so the kit matches the candidate's rendered markup and diagnostics, to a clean consumer checkout, preserving their relative layout (`eng/` beside `samples/`). Use candidate packages from a local feed directory. The script restores the fixture in a temporary directory with a fresh NuGet cache, so prior packages cannot satisfy the run.
...
The fixture's host calls `AddHexalithFrontComposerQuickstart(o => o.ScanAssemblies(typeof(ProofDomain).Assembly))` → `AddHexalithDomain<ProofDomain>()` → `AddHexalithEventStore(...)`. The Fluxor scan of the domain assembly is required: `AddHexalithDomain<TMarker>()` registers the generated manifests but not the generated Fluxor features. Its domain annotations generate `ProofProjectionView` and the `/commands/Proof/CreateProofCommand` page. The fixture mounts the generated view at the module's `/proof/proof-projection` navigation route. The runner checks both rendered HTML surfaces, then checks that missing Quickstart and misordered EventStore fail with named startup diagnostics before any page renders. It also starts Quickstart alone and checks the existing no-modules home state. The host retains the EventStore command and query registrations throughout the success case. The Shell renders nothing without a validated tenant and user, so the fixture replaces the fail-closed `IUserContextAccessor` with `ProofUserContextAccessor`, a fixed public tenant (`adopter-proof`) and user (`adopter-proof-operator`) that stand in for sign-in; demo tenants remain refused. Your own host supplies that identity through its authentication bridge, such as `AddHexalithFrontComposerServerSecurity(...)`. Without one, every page shows the Shell's "Workspace unavailable" state. The fixed identity never appears in the result.
