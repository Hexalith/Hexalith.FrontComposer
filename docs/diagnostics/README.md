...
Normal CI must not call live package feeds, GitHub, or the public docs site to validate this contract. Package/API compatibility gates use .NET package validation / ApiCompat for packable FrontComposer library packages, with `FrontComposerPackageValidationBaselineVersion` set to the latest published `4.3.0` baseline and applied conditionally in `Directory.Build.targets` only when `EnableFrontComposerPackageValidation=true` is set (default off). Contracts.UI pins the same published `4.3.0` baseline explicitly; validation is not disabled globally. The baseline is not a free-standing literal: the release policy derives the release line it must sit on. The baseline may be no more than one release line behind the line under validation -- it may sit on that line itself (packing the `4.3.1` hotfix against published `4.3.0`) or on the one immediately before it (packing `4.4.0` requires `4.3.x`); two or more lines back fails closed. The static repository check applies the same tolerance against the ledger's `currentRelease`. The .NET SDK itself disables `EnablePackageValidation` for the CLI's `PackAsTool` layout because library API/package validation does not support tool packages; the CLI still participates in inventory, symbol, clean-install, and command-output compatibility gates. Suppression evidence is checked in at `compatibility-suppressions.json` using schema `2.0`; the explicit production policy validates its `currentRelease`, `targetRelease`, and `expiresAfter` release lines before build and rechecks baseline/XML parity in the live packer before package-output cleanup. Shared synthetic CI keeps the positional pack contract and skips release-line matching only; every live `dotnet pack` still enables package validation. The planned `v4.4` ledger is empty because the published `4.3.0` baseline absorbs all 26 reviewed MCP benchmark-type removals from `v4.0`. Every new package-validation suppression remains forbidden unless separately reviewed and release-bounded. Reports are normalized so unavailable network/cache state cannot silently skip diagnostics governance.

Every package-validation suppression is a release-scoped exception, not a permanent compatibility policy. Its XML row must name one exact diagnostic and target, and `compatibility-suppressions.json` must carry one matching package/TFM/diagnostic/target row with an approved reason, owning story, current/target release, and next-line expiry. Wildcards, unmatched XML or JSON rows, pre-target use, expired rows, and suppressions retained after the package baseline advances fail closed. For an intentional major break, the Release Owner approves the major posture before the ledger advances; after that major publishes, the next lifecycle update raises the package baseline and removes both XML and JSON rows before the expiry release can pack.
...
Release policy base tip 2026-09-05T10:44:10Z
