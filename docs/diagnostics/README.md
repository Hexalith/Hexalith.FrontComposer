...
Normal CI must not call live package feeds, GitHub, or the public docs site to validate this contract. Package/API compatibility gates use .NET package validation / ApiCompat for packable FrontComposer library packages, with `FrontComposerPackageValidationBaselineVersion` set to the latest stable `1.12.0` baseline and applied conditionally in `Directory.Build.targets` only when `EnableFrontComposerPackageValidation=true` is set (default off). First-release Contracts.UI uses a project-specific no-baseline marker with a required removal trigger after `2.0.0`; validation is not disabled globally. The .NET SDK itself disables `EnablePackageValidation` for the CLI's `PackAsTool` layout because library API/package validation does not support tool packages; the CLI still participates in inventory, symbol, clean-install, and command-output compatibility gates. Suppression evidence is checked in at `compatibility-suppressions.json` using schema `2.0`; its `currentRelease`, `targetRelease`, and `expiresAfter` release lines are enforced by the semantic-release pack script before any build or pack command runs. Reports are normalized so unavailable network/cache state cannot silently skip diagnostics governance.
