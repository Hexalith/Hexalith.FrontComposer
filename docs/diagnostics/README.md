...
Normal CI must not call live package feeds, GitHub, or the public docs site to validate this contract. Package/API compatibility gates use .NET package validation / ApiCompat for packable FrontComposer library packages, with `FrontComposerPackageValidationBaselineVersion` set to the latest published `3.0.0` baseline and applied conditionally in `Directory.Build.targets` only when `EnableFrontComposerPackageValidation=true` is set (default off). Contracts.UI pins the same published `3.0.0` baseline explicitly; validation is not disabled globally. The .NET SDK itself disables `EnablePackageValidation` for the CLI's `PackAsTool` layout because library API/package validation does not support tool packages; the CLI still participates in inventory, symbol, clean-install, and command-output compatibility gates. Suppression evidence is checked in at `compatibility-suppressions.json` using schema `2.0`; its `currentRelease`, `targetRelease`, and `expiresAfter` release lines are enforced by the semantic-release pack script before any build or pack command runs. The approved `v4.0` ledger contains exactly the 26 MCP `CP0001` benchmark-type removals required by Story 11.17c and expires them before `v4.1`; every other package-validation suppression remains forbidden unless separately reviewed and release-bounded. Reports are normalized so unavailable network/cache state cannot silently skip diagnostics governance.
