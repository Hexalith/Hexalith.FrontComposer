...
Normal CI must not call live package feeds, GitHub, or the public docs site to validate this contract. Package/API compatibility gates use .NET package validation / ApiCompat for packable FrontComposer library packages, with `FrontComposerPackageValidationBaselineVersion` set to the latest published `4.1.1` baseline and applied conditionally in `Directory.Build.targets` only when `EnableFrontComposerPackageValidation=true` is set (default off). Contracts.UI pins the same published `4.1.1` baseline explicitly; validation is not disabled globally. The .NET SDK itself disables `EnablePackageValidation` for the CLI's `PackAsTool` layout because library API/package validation does not support tool packages; the CLI still participates in inventory, symbol, clean-install, and command-output compatibility gates. Suppression evidence is checked in at `compatibility-suppressions.json` using schema `2.0`; the explicit production policy validates its `currentRelease`, `targetRelease`, and `expiresAfter` release lines before build and rechecks baseline/XML parity in the live packer before package-output cleanup. Shared synthetic CI keeps the positional pack contract and skips release-line matching only; every live `dotnet pack` still enables package validation. The planned `v4.3` ledger is empty because the published `4.1.1` baseline absorbs all 26 reviewed MCP benchmark-type removals from `v4.0`. Every new package-validation suppression remains forbidden unless separately reviewed and release-bounded. Reports are normalized so unavailable network/cache state cannot silently skip diagnostics governance.
