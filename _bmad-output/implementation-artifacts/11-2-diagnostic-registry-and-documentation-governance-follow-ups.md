...
## Review Findings (Pass 2 — 2026-05-11)

_Source: `/bmad-code-review 11-2` second pass on 2026-05-11, scoped to follow-up patch commit `059c634..18cb7aa` (the diff applying the Pass-1 32 patches). Three parallel layers: Blind Hunter (diff-only), Edge Case Hunter (boundary paths), Acceptance Auditor (AC compliance + Pass-1 closure verification). Diff snapshot at `_bmad-output/code-reviews/story-11-2-pass2-diff.patch`._

### Decision-needed

_None._ All findings have unambiguous fixes; the workflow choice (apply now vs leave as action items vs defer to follow-up story) is handled at the Step-4 prompt.

### Patch (code/docs fixes with unambiguous resolution)

- [x] [Review][Patch] **Severity matrix asserts only registry-side fields, not docs/release-row/report severity** [`tests/.../DiagnosticRegistryTests.cs:583-601`] — `Severity_HFC1037_HFC1040_HFC1044_AreTablePinnedAcrossChannels` only reads `registry.Diagnostics[id].CompilerSeverity`; AC30 requires "registry, docs, release rows, **and reports**." A copy-paste regression in `AnalyzerReleases.Unshipped.md` (`HFC1040 | ... | Warning` flipped to `Error`) or in any HFC1037/1040/1044 docs stub front-matter `compilerSeverity:` would not be caught. Add columns: `releaseRowSeverity` (parsed from `src/Hexalith.FrontComposer.SourceTools/AnalyzerReleases.{Shipped,Unshipped}.md`) and `stubFrontMatterSeverity` (parsed from each stub's YAML), and pin all four. Also load Roslyn `DiagnosticDescriptors` and assert `DefaultSeverity` agrees. (blind+auditor+edge)
- [x] [Review][Patch] **`mandatedStates` list contains `Owner: Story` header label + non-AC33 vocabulary** [`tests/.../DiagnosticRegistryTests.cs:667`] — AC33's three final states are `closed-with-evidence`, `deferred-to-named-story`, `rejected-with-rationale`. The current `mandatedStates` is `["Resolved", "Owner: Story", "Duplicate of", "Superseded by", "Rejected with rationale", "Split parent"]`. `"Owner: Story"` is a header field present on every row, so the substring check passes trivially. None of the entries is the literal AC33 vocabulary. Replace with `["closed-with-evidence", "deferred-to-named-story", "rejected-with-rationale"]` OR document that AC33 vocabulary was renamed to ledger marker vocabulary in 11.1 reconciliation and update AC33 prose to match. (blind+edge+auditor)
- [x] [Review][Patch] **`SourceToolsAnalyzerReleaseRows_AllHaveBackingDescriptor` asserts the opposite of its own justification** [`tests/.../DiagnosticRegistryTests.cs:686-689`] — Comment explains the test "ensures any future row without a descriptor fails fast under `TreatWarningsAsErrors=true`" after RS2002 NoWarn removal. The filter `strictHfcId.IsMatch(id)` excludes HFCM rows — but HFCM is exactly what RS2002 used to suppress. The test cannot catch the assumption being wrong. Split into two assertions: (a) every `HFC\d{4}` row in `AnalyzerReleases.*.md` has a backing `DiagnosticDescriptor` in `DiagnosticDescriptors`; (b) no `HFCM\d{4}` row appears in SourceTools `AnalyzerReleases.*.md` (must be in `docs/diagnostics/migration-findings.json` instead). Also `DiagnosticDescriptorFields()` only enumerates SourceTools — Shell-owned HFC1601 in a SourceTools release row would spuriously fail. (blind+edge)
- [x] [Review][Patch] **Stub front-matter parser fails on bracket/case forms; stub-body relatedIds parity vs registry not enforced** [`tests/.../DiagnosticRegistryTests.cs:306-311` + 6 stub bodies] — `frontMatterRelatedIdsLine` regex captures inside-bracket content; `.Split(',', TrimEntries|RemoveEmptyEntries)` does NOT strip `[]`, quote chars, YAML block-list `-` prefix, or normalize case. A stub `relatedIds: [HFC1040]` produces token `"[HFC1040"`, not `"HFC1040"`. When the registry has an empty `relatedIds` and the stub front-matter writes `relatedIds: []`, both split-results are empty and the test passes regardless of the stub *body* (`## Related Diagnostics`). Result: HFC1037/1040/1044/1056/1057/4001 bodies were rewritten to enumerate relatedIds with canonical links, but no test parses the body section and asserts set-equality with the registry. Parse YAML properly (use `YamlDotNet` or write a tiny block-list parser), normalize case, and add a `StubBody_RelatedDiagnosticsListEqualsRegistry` theory that parses the `## Related Diagnostics` bullet list and compares to `byId[id].RelatedIds`. (blind+edge+auditor)
- [x] [Review][Patch] **`Story112LedgerRowRegex` excludes hyphenated row ids; missing expected-count guard** [`tests/.../DiagnosticRegistryTests.cs:663-665,749`] — Inner id charset `[A-Z0-9]+` excludes hyphens, so `DEF-9-4-C4a` and `DEF-9-4-A17` would not match the inner capture. Regex requires `**...**` (asterisk bold only); ledger rows using `__...__` markdown bold would be skipped. Only `matches.Count.ShouldBeGreaterThan(0)` triggers — a regression that drops 17 of 18 routed rows still passes. Broaden the inner id charset to `[A-Z0-9\-]+`, allow either bold form, and assert `matches.Count.ShouldBe(expectedStory112RowCount)` where the expected count is derived from `deferred-work.md` Story-9-4-routed rows or hardcoded with a doc-string explaining the count. (blind+edge)
- [x] [Review][Patch] **`README.md` prose now self-contradictory about `FrontComposerPackageValidationBaselineVersion` location** [`docs/diagnostics/README.md:82`] — Pass-1 R12 fix changed "defined in `Directory.Build.props` and applied conditionally in `Directory.Build.targets`" → "defined and applied conditionally in `Directory.Build.targets`". Verify the property's `<PropertyGroup>` definition actually moved to `.targets` (only the `IsPackable`-conditioned `<PropertyGroup>` block needed relocation per AC14); if it still lives in `.props`, update the prose to "defined in `Directory.Build.props`, applied conditionally in `Directory.Build.targets`". Run `Grep` against both files to confirm before editing. (blind)
- [x] [Review][Patch] **`forbiddenSentinels` scope incomplete and list omits common token shapes** [`tests/.../DiagnosticRegistryTests.cs:493-516,543-545`] — Pass-1 R15 explicitly required "Apply to docs stubs, samples, `migration-findings.json`, AND validation reports — not only samples." Current scan is inside `DriftSampleReports_AreNormalizedAndCommitted` and runs against samples only. Extract `AssertNoForbiddenSentinels(string content)` and invoke it from four call sites: each sample, each docs stub, `migration-findings.json`, and each generated validation report. Sentinel list omits JWT (`eyJ`), PEM (`-----BEGIN`), Google API (`AIza`), GitLab PAT (`glpat-`), Slack tokens beyond `xoxb-` (`xoxa-`, `xoxp-`, `xoxs-`), AWS session keys (`ASIA`), Azure SAS (`sig=`); add them. Anchor short tokens like `AKIA` to word boundaries (`\bAKIA[0-9A-Z]{16}\b`) to avoid false positives in prose. (blind+edge+auditor)
- [x] [Review][Patch] **`RegistryValidator_DeterministicUnderShuffledInput` uses broken `OrderBy(rng.Next())` shuffle + HashSet equality** [`tests/.../DiagnosticRegistryTests.cs:629-650`] — `OrderBy(_ => rng.Next())` is a known-broken LINQ pseudo-shuffle: the comparator is non-deterministic, so .NET's sort can return the input order on some sizes/seeds. With only `diagnostics.Count` items, four trials may never produce a materially different permutation. Result compares via `HashSet<string>` (set-stability, not byte-stability) — ordering regressions slip through. Replace with Fisher-Yates: `for (int i=n-1; i>0; i--) { var j=rng.Next(i+1); (a[i],a[j])=(a[j],a[i]); }`, compare via ordered `List<string>` with `StringComparer.Ordinal`, and `Assert.NotEqual(originalOrder, shuffledOrder)` before invoking the validator. (blind+edge)
- [x] [Review][Patch] **Shuffled-input determinism covers only 1 of 6 enumerated surfaces** [`tests/.../DiagnosticRegistryTests.cs` test class] — T6/AC25/AC31/D17 enumerate six surfaces: source files, registry rows, docs rows, related IDs, suppressions, package groups. Only `RegistryValidator_DeterministicUnderShuffledInput` exists. Add five more theories (one per surface): each must shuffle the input collection and assert byte-stable categorized output. If budget exhausted, document the gap as a deferred row pointing at Story 11.7 release-governance follow-up. (auditor)
- [x] [Review][Patch] **`ValidateMigrationFindingsJson` does not validate `migrationDocsSlug`/`severity`/`category` per row** [`tests/.../DiagnosticRegistryTests.cs:340-368,424-471`] — The outer test (`HfcmMigrationFindings_AreCanonicalAndOrdered`) uses Shouldly to check `migrationDocsSlug.ShouldStartWith("migrations/")` and confirms a hardcoded id list. The new fail-closed validator (`ValidateMigrationFindingsJson`, around line 832) checks id-shape/duplicate/bucket/release-note/owner-story/release-provenance but skips `migrationDocsSlug` containment (`migrations/../../etc/passwd` slips through `StartsWith("migrations/")`), `severity` enum membership (`{Info, Warning, Error}`), and `category` literal (`HexalithFrontComposer.Migration`). Two code paths drifted. Add three named categories — `hfcm-malformed-slug`, `hfcm-invalid-severity`, `hfcm-invalid-category` — plus three Theory mutations. (blind+edge)
- [x] [Review][Patch] **Theory: `continue` on `wrong-prefix` skips duplicate-id check** [`tests/.../DiagnosticRegistryTests.cs:387-422,440-470`] — `MigrationFindingsValidator_FailsClosedWithNamedCategories` mutations target one field at a time. The validator's `wrong-prefix` branch likely `continue`s past `seen.Add(id)`, so two rows mutated to the same `HFC0001` (wrong-prefix) flag as `wrong-prefix` only — never `duplicate-id`. A combined-mutation regression would be invisible. Either restructure so all per-row checks emit all applicable categories (`yield return` then continue accumulation), or add explicit combined-mutation Theory cases (`wrong-prefix + duplicate`, `missing-id + duplicate`). (blind+edge)
- [x] [Review][Patch] **Theory missing `missing-schema-version` / `wrong-schema-version` / `missing-id` / `missing-findings-array` mutations** [`tests/.../DiagnosticRegistryTests.cs:371-422`] — `ValidateMigrationFindingsJson` emits `hfcm-unsupported-schema` when `schemaVersion` is missing or != "1.0"; the Theory does not include `[InlineData]` exercising those paths. Same for `missing-id` (row with no `id`), `missing-findings-array` (top-level `findings` absent), and `findings: []` (empty). Add `[InlineData]` for each so the validator's negative branches are reachable. (edge+auditor)
- [x] [Review][Patch] **`TryParseVersionToken` catches incomplete exception set** [`tests/.../DiagnosticRegistryTests.cs:884-896`] — `catch (Exception ex) when (ex is ArgumentException or FormatException or OverflowException)`. If `ParseVersionToken` strips suffixes via regex, it can throw `RegexMatchTimeoutException` or `InvalidOperationException`, escaping the catch and crashing the iterator — defeating the fail-closed contract that Pass-1 R2 was meant to close. Either broaden to `catch (Exception)` (with a comment citing the iterator-yield contract) or list every exception type the preprocessing pipeline can produce. (blind+edge)
- [x] [Review][Patch] **`approvedOn` accepts any non-whitespace text — no ISO-8601 enforcement** [`tests/.../DiagnosticRegistryTests.cs:366-367,467-468`] — `provenance["approvedOn"]!.GetValue<string>().ShouldNotBeNullOrWhiteSpace()` permits `"yesterday"`, `"TBD"`, `"v1"`. All six committed values are `"2026-05-11"`, but the contract is unenforced. Enforce `^\d{4}-\d{2}-\d{2}$` and (optionally) `DateOnly.TryParseExact` against ISO-8601. Apply to both the outer test and the validator's `missing-release-provenance` adjacent check. (blind+edge)
- [x] [Review][Patch] **`migrationId` unknown-id silently `continue`d; self-reference and chain transitivity unchecked** [`tests/.../DiagnosticRegistryTests.cs:289-296`] — `if (!byId.TryGetValue(diagnostic.MigrationId!, out var target)) { continue; }` — an unknown migration target is a dead pointer; downgrading to "no-op" hides bugs the lifecycle check is supposed to catch. Also no self-reference check (`diagnostic.Id == diagnostic.MigrationId`) and no chain walk (A → B → retired). Yield distinct categories: `unknown-migration-target`, `self-referential-migration`, `migration-chain-terminates-retired`. (blind+edge)
- [x] [Review][Patch] **R8 (Pass-1) closure: unknown-optional-field validator + fixture not implemented** [`tests/.../DiagnosticRegistryTests.cs` registry validator] — Pass-1 R8 marked applied but: `Grep` for `unknown-optional`, `backdoor`, `allowlist` returns no matches in `DiagnosticRegistryTests.cs`. The validator silently accepts unknown root fields like `"backdoor": "..."` and unknown row fields. AC27 ("unknown optional fields in a supported schema remain non-fatal only when the compatibility rule explicitly permits them") is unenforced. Either add an explicit `knownRootKeys` allowlist + `yield return "registry-unknown-root-field-{key}"` for non-allowlisted keys, OR document the allowlist policy in `docs/diagnostics/README.md` and assert no schema drift. (auditor)
- [x] [Review][Patch] **R26 (Pass-1) closure: unsupported-schema sample not round-trip-validated** [`tests/.../DiagnosticRegistryTests.cs` + `docs/diagnostics/samples/unsupported-schema-drift-report.json`] — Pass-1 R26 marked applied but the sample is referenced only in the sample-list constant (`SampleDriftReportRelativePaths`); no test runs `ValidateRegistryJson` on a tampered fixture (schema version flipped to `"9.9"`) and asserts the output report deep-equals the committed sample. Add `RegistryValidator_RoundTripsUnsupportedSchemaSample` that performs the tamper-validate-compare cycle, normalizing line endings and ordering. (auditor)
- [x] [Review][Patch] **R22 (Pass-1) closure: unsafe-generated-front-matter sample has no JSON-level validator** [`tests/.../DiagnosticRegistryTests.cs` + `docs/diagnostics/samples/unsafe-generated-front-matter-report.json`] — Pass-1 R22 marked applied; `ValidateGeneratedFrontMatter(string frontMatter)` exists but takes raw YAML, not a JSON sample. No `ValidateGeneratedFrontMatterJson` round-trips the sample. The committed `unsafe-generated-front-matter-report.json` declares a category that no producer-side test confirms the validator emits. Either add the round-trip validator OR remove the sample file and its `SampleDriftReportRelativePaths` entry. (auditor+edge)
- [x] [Review][Patch] **`StripZeroWidthAndFormatChars` uses literal invisible characters** [`tests/.../DiagnosticRegistryTests.cs:919-921`] — Four `==` comparisons against visually-identical space characters (presumably `U+00A0`, `U+2007`, `U+202F`, `U+3000`). A maintainer cannot see what is being matched; an editor with Unicode normalization could silently collapse the literals into ASCII spaces. Replace each with explicit `' '`/`' '`/`' '`/`'　'` escapes. Same fix for any inline test fixtures (line ~724) that embed NBSP variants directly. (blind+edge)
- [x] [Review][Patch] **`DiagnosticsDocsLinkUrlRegex` middle segment too permissive** [`tests/.../DiagnosticRegistryTests.cs:960`] — Regex `https?://(?<host>…)/[^"'\s?#]*diagnostics/HFC[0-9]{4}(?<suffix>[/?#]…)?`: middle segment `[^"'\s?#]*` forbids `?`/`#` in *path* segments, so URLs with query/fragment before `/diagnostics/` simply don't match (silent skip rather than fail-loud); host pattern allows trailing-dot DNS (`hexalith.github.io.`); no userinfo/port rejection (`https://evil@hexalith.github.io.attacker/diagnostics/HFC1010`); also matches `http://` not just `https://`. Replace with `Uri.TryCreate(...)` + structural checks: `uri.Scheme == "https"`, `uri.Host.TrimEnd('.').Equals(CanonicalDocsHost, StringComparison.Ordinal)`, `uri.Authority == uri.Host` (no userinfo/port), `uri.Query == ""`, `uri.Fragment == ""`, `uri.AbsolutePath == $"/diagnostics/{id}"`. (blind+edge)
- [x] [Review][Patch] **Suppression `uniqueRows` dedupe tuple omits `newState`** [`tests/.../DiagnosticRegistryTests.cs:1540`] — `uniqueRows.Add((package, tfm!, oldSignature!, hfcId!, targetRelease!))` collapses two distinct compatibility scenarios where only `newState` differs (`removed` vs `modified`) into a single duplicate. Include `newState!` in the tuple OR document why two rows differing only in `newState` are semantically identical (and add a Theory pinning that interpretation). (auditor+edge)
- [x] [Review][Patch] **T3 fixture group missing `analyzer-descriptor-misclassification` Theory case** [`tests/.../DiagnosticRegistryTests.cs:387-422`] — T3 subtask 4 named six fixture types: valid, duplicate, wrong-prefix, wrong-bucket, missing-note, **analyzer-descriptor misclassification**. Theory covers seven other cases but not the misclassification one (would assert that an HFCM row appearing in `AnalyzerReleases.*.md` is detected). Indirect coverage exists via `HfcmMigrationFindings_AreCliGovernedNotRoslynReleaseRows`, but the named group is incomplete. Add an InlineData mutation that injects an HFCM id into a stubbed `AnalyzerReleases.Unshipped.md` content blob and assert the new validator yields `hfcm-analyzer-descriptor-misclassification`. (auditor)
- [x] [Review][Patch] **`MaxJsonBytesPerSample` compares UTF-16 char count, not byte budget; literal undocumented** [`tests/.../DiagnosticRegistryTests.cs:515-516`] — `json.Length.ShouldBeLessThanOrEqualTo(MaxJsonBytesPerSample)` measures `.Length` (UTF-16 chars). Sample JSON is mostly ASCII so char count ≈ byte count in practice, but the variable name promises *bytes*. Replace with `Encoding.UTF8.GetByteCount(json)`. Also add a doc-comment citing AC32 and explaining the 16 KiB derivation (largest legitimate sample × safety margin). (blind+edge)
- [x] [Review][Patch] **All `migration-findings` mutations target `findings[0]` only** [`tests/.../DiagnosticRegistryTests.cs:387-408`] — Every Theory `[InlineData]` mutates `findings[0]`. A validator bug that inspects only the first row would still pass. Either parametrize the row index or apply each mutation to every row in a small per-row loop. (edge)
- [x] [Review][Patch] **`relatedIds` duplicate entries within a single array not rejected** [`tests/.../DiagnosticRegistryTests.cs:240-253`] — Self-reference is caught (`relatedId.ShouldNotBe(diagnostic.Id)`), but `["HFC1040", "HFC1040", "HFC1601"]` passes both that check and the reciprocity pass. Add a `Distinct` check yielding `relatedIds-duplicate-{id}` and a Theory fixture. (blind)
- [x] [Review][Patch] **CRLF handling in `ValidateGeneratedFrontMatter` strips lead-char detection** [`tests/.../DiagnosticRegistryTests.cs:729-747`] — Splits on `'\n'` only; on CRLF input the trailing `'\r'` precedes the actual content, so `line[0]` is `'\r'`, not `'='`/`'+'`/`'@'` — formula-injection prefix scan silently misses CRLF stubs. Split with `new[] {'\r', '\n'}, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries`. Add a CRLF Theory fixture. (edge)
- [x] [Review][Patch] **`DescriptorCanonicalHelpLinkFormat_*` test pins a hardcoded 8-id list** [`tests/.../DiagnosticRegistryTests.cs:553-573`] — Future diagnostics beyond the 8 are not covered. Iterate `registry.Diagnostics.Keys` instead of the hardcoded list. (edge)
- [x] [Review][Patch] **Source/release-row enumeration ordering missing cross-platform stability** [`tests/.../DiagnosticRegistryTests.cs:676-691`] — `SourceToolsAnalyzerReleaseRows` uses `File.ReadAllLines` and iterates without an `OrderBy(StringComparer.Ordinal)` sort. Test "passes locally / fails on CI" failure modes when culture-sensitive enumeration order changes. Same gap may affect `OwnedSourceFiles()` enumeration elsewhere (lines 316-327). Apply ordinal sort everywhere governance scans collections. (edge)
- [x] [Review][Patch] **`forbiddenLeads` missing `-`/`|`; `forbiddenPatterns` missing HTML-entity decode** [`tests/.../DiagnosticRegistryTests.cs:730-732`] — Shell-injection prefix chars `-` (e.g., `-rm -rf`) and `|` (e.g., `|cmd`) are not in `forbiddenLeads`. Pattern scan does not `HttpUtility.HtmlDecode` before substring search, so `&lt;script&gt;` in front matter slips through. Extend the leads list and decode HTML entities (and percent-encoded variants `%3Cscript`) before scan. (edge)
- [x] [Review][Patch] **`suppression-malformed-version` conflates `expiresAfter` and `targetRelease` failures** [`tests/.../DiagnosticRegistryTests.cs:865-870`] — Both fields share one category, so the operator cannot tell which is malformed. Split into `suppression-malformed-expires-after` and `suppression-malformed-target-release`. (edge)
- [x] [Review][Patch] **NFC-confusable (full-width `<`, `U+FF1C`) injection bypass in `UnsafeGeneratedFrontMatter` fixtures** [`tests/.../DiagnosticRegistryTests.cs:709-727`] — Fixtures don't exercise `<script>` rendered as `＜script＞` (full-width) or `&lt;script&gt;` (HTML-encoded). Validator should NFC-normalize and HTML-decode before pattern scan; add Theory fixtures for both. (edge)
- [x] [Review][Patch] **Outer `HfcmMigrationFindings_AreCanonicalAndOrdered` never invokes `ValidateMigrationFindingsJson` on pristine** [`tests/.../DiagnosticRegistryTests.cs:340-368`] — The outer test pins six ids via Shouldly assertions but does not assert `ValidateMigrationFindingsJson(File.ReadAllText(path)).ShouldBeEmpty()` on the as-committed file. The validator could yield a spurious category for valid input and no test would notice. Add the empty-category assertion at the end of the outer test. (edge)
- [x] [Review][Patch] **`MigrationFindingsValidator` Theory switch lacks `default` for typo mutations** [`tests/.../DiagnosticRegistryTests.cs:411-420`] — An `[InlineData("typo-mutation-name")]` would throw `InvalidOperationException` at runtime rather than fail with a named test message. Add `default: throw new InvalidOperationException($"Unknown mutation: {mutation}")` AND wrap the call in `Assert.Fail($"Unrecognized Theory mutation name: {mutation}")` to surface InlineData typos cleanly. (edge)
- [x] [Review][Patch] **Cross-package exception `introducedIn` not shape-validated** [`tests/.../DiagnosticRegistryTests.cs:1366` (or wherever the exception block is parsed)] — The HFC1601 cross-package-exception entry has an `introducedIn` field read via `TryGetString` but not run through `SemverShapeRegex`. A free-text value like `" unreleased"` would pass. Apply `SemverShapeRegex.IsMatch` to exception `introducedIn` during `invalid-cross-package-exception` validation. (edge)
- [x] [Review][Patch] **`stubText.ShouldContain` only checks presence of canonical link; doesn't `StripZeroWidthAndFormatChars` before substring search** [`tests/.../DiagnosticRegistryTests.cs:567-573`] — A stub containing a canonical link with a zero-width joiner mid-URL would still pass `ShouldContain` (or not, depending on where the ZWJ sits). Also `ShouldContain` accepts both `https://...` AND a sibling `http://...` variant — only presence is checked, not absence of the insecure variant. Normalize stub text via `StripZeroWidthAndFormatChars` first, then assert exactly one occurrence and zero `http://` variants. (edge)
- [x] [Review][Patch] **`introducedIn` shape check skips empty/whitespace via `continue`** [`tests/.../DiagnosticRegistryTests.cs:265-271`] — `if (string.IsNullOrWhiteSpace(diagnostic.IntroducedIn)) continue;` lets a diagnostic with `introducedIn: " "` bypass the SemVer shape enforcement entirely. Either yield `introducedIn-empty` or `Assert.Fail` — fail-closed contract requires absence to be a named category, not a silent skip. (edge)
- [x] [Review][Patch] **`SemverShapeRegex` permits leading zeros / malformed prerelease / mismatched parser** [`tests/.../DiagnosticRegistryTests.cs:963`] — Current regex `^[0-9]+\.[0-9]+(?:\.[0-9]+)?(?:[-+][0-9A-Za-z.\-]+)?$` accepts `00.000`, `1.0--`, `1.0-..beta`; rejects `v0.2.0` while the SDK's `ParseVersionToken` accepts the `v` prefix (shape vs parser drift). Tighten to strict SemVer 2.0.0 (`^(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?(?:-[0-9A-Za-z\-]+(?:\.[0-9A-Za-z\-]+)*)?(?:\+[0-9A-Za-z\-]+(?:\.[0-9A-Za-z\-]+)*)?$`) AND either normalize `v` prefix in both regex and parser, or document the asymmetry. (blind+edge)
- [x] [Review][Patch] **`safeFrontMatter` test missing idempotency assertion** [`tests/.../DiagnosticRegistryTests.cs:715-727`] — On dual-pattern input the validator should yield each category at most once; current test only checks "no categories" on safe input and "one category" on unsafe — not "at most one per category" when two patterns trigger the same rule. Add a Theory fixture with double-pattern input and assert distinct categories. (edge)
- [x] [Review][Patch] **`malformed-introduced-in` Theory mutation `"latest"` may be caught by earlier non-empty check** [`tests/.../DiagnosticRegistryTests.cs:417`] — If the validator yields `missing-introduced-in` before the SemVer shape check, the Theory passes for the wrong reason. Use a clearly non-empty but non-SemVer value like `"1"` or `"1.x"` so the path under test is the SemVer-shape branch. (edge)
- [x] [Review][Patch] **Placeholder convention not unified across samples** [`docs/diagnostics/samples/reserved-retired-misuse-report.json:12`] — Pass-1 R28 was marked applied, but `reserved-retired-misuse-report.json` still uses `"path": "<redacted-source-path>"` while others use `docs/diagnostics/HFC-PLACEHOLDER.md`. Standardize on one convention (recommend the `HFC-PLACEHOLDER` form since `SampleFindingIdShapeRegex` already accepts it) and update the affected sample. Add a `PlaceholderShapeRegex` and assert all sample placeholder fields match. (auditor+edge)
- [x] [Review][Patch] **`producer-fingerprints.json` SHA not diff-verifiable; re-run `eng/validate-docs.ps1`** [`docs/validation/producer-fingerprints.json`] — Only registry content change in the diff is the HFC1040 lifecycleNote typo fix ("due" → "due to"); a 64-char SHA-256 delta is consistent with one byte changing. The dev log records `eng/validate-docs.ps1` passed and refreshed the fingerprint, so this is procedural verification, not a code defect. Confirm one final run of `pwsh ./eng/validate-docs.ps1` after the Pass-2 patches land — fingerprint will need refreshing again for any registry edits triggered by R1/R4/R10. (blind)
- [x] [Review][Patch] **`Severity_HFC1037_HFC1040_HFC1044_*` does not load Roslyn `DiagnosticDescriptor`** [`tests/.../DiagnosticRegistryTests.cs:583-601`] — Test name promises "across channels"; body reads only the JSON registry. The Warning/Error asymmetry could drift in the C# descriptor without this test catching it. Extend the matrix to also load `DiagnosticDescriptors.HFC1037`/`HFC1040`/`HFC1044`/`HFC1601` and assert `descriptor.DefaultSeverity` matches the registry's `compilerSeverity`. (Sub-piece of R1 / [Review][Patch] severity matrix; can be folded into the same fix.) (blind+edge)

### Defer (real but out of Pass-2 scope / pre-existing or policy decision)

- [x] [Review][Defer] **`introducedIn: "0.2.0"` in `migration-findings.json` semantic correctness** [`docs/diagnostics/migration-findings.json:107,121,137,153,169,183`] — All six HFCM rows use `"0.2.0"`. Data-quality concern about whether the literal represents tool version (0.2.0 is plausible for Story 9-2 CLI tool) or FrontComposer release (would be wrong — repo is at Story 11.x). Resolve as a documentation decision in the migration-findings README or as a follow-up product decision; not a contract violation that fails any AC. (blind)
- [x] [Review][Defer] **All `approvedOn` rows batch-stamped with same date** [`docs/diagnostics/migration-findings.json:112,126,142,158,174,188`] — Six identical `2026-05-11` values suggest a single batch approval rather than per-row sign-off. Audit-trail design choice (single batch vs per-row); validator doesn't enforce this distinction. Out of scope for diagnostic governance contract. (blind)
- [x] [Review][Defer] **`Random rng = new(20260511)` seeds with today's date literal** [`tests/.../DiagnosticRegistryTests.cs:637`] — Cosmetic; deterministic test seed should be stable across runs and the date-literal pattern is a recognizable convention. No correctness impact once the shuffle algorithm is fixed (see [Review][Patch] R8). (blind)
- [x] [Review][Defer] **`_bmad-output` Path.Combine case-asymmetry** [`tests/.../DiagnosticRegistryTests.cs:659`] — Windows is case-insensitive, Linux CI is case-sensitive; mixed-case rename of `_bmad-output` would split-fail. Repo policy is lowercase, not story policy; rename is unlikely. Add a normalized-path helper if the issue recurs. (blind)
- [x] [Review][Defer] **`RegistryValidator_CrossPackageRangeException_NegativeControl` is documented as tautology** [`tests/.../DiagnosticRegistryTests.cs:612-621`] — Test removes a permissive exception and asserts the validator still passes (because HFC1601 is naturally in-range). Acknowledged as low-value, but kept as an audit marker. Real negative control would require an out-of-range id listed in the exception array (positive bypass) plus a non-HFC1601 cross-package row not in the exception array. Re-evaluate in Story 11.7 release-governance scope. (blind)
- [x] [Review][Defer] **`HfcmIdShapeRegex` vs `SampleFindingIdShapeRegex` divergence** [`tests/.../DiagnosticRegistryTests.cs:474-475,966`] — Intentional split: HFCM-specific regex for migration-findings rows, broader sample regex for placeholder evidence. Confusion risk is real but documented; reclassification would require a Product decision. (edge)
- [x] [Review][Defer] **`IsRootedSlug` UNC `//server/share` and drive-relative `C:foo` semantics** [`tests/.../DiagnosticRegistryTests.cs:929-945`] — Current logic catches `slug[0]=='/'` (covers UNC), but drive-relative `C:foo` (no separator after `:`) is also flagged as rooted. Boundary semantics policy decision: should drive-relative paths fail like absolute paths, or be accepted? Document the chosen semantics in `docs/diagnostics/README.md` slug-validation section. (edge)

### Summary

- Decision-needed: **0**
- Patch: **42** (all 42 applied — R9 multi-surface determinism implemented via `GovernanceEnumerations_AreDeterministicAcrossSurfaces` covering source-files, docs-stub-rows, related-ids, suppressions, and package-groups in addition to the existing registry-rows shuffle).
- Defer: **7**
- Dismissed as noise: **0**

Of the 32 Pass-1 patches marked applied, **7 closures were partial or weaker-than-spec**: R5 (severity matrix), R6 (HFC1601 cross-package), R8 (unknown-optional-field), R15 (sentinel scope), R18 (multi-surface determinism), R22 (front-matter sample), R26 (unsupported-schema round-trip). All seven were tightened in Pass 2.

**Pass-2 validation outcomes** (2026-05-11, after applying 42 patches):
- `dotnet test tests/Hexalith.FrontComposer.SourceTools.Tests/Hexalith.FrontComposer.SourceTools.Tests.csproj --configuration Release --filter "FullyQualifiedName~DiagnosticRegistryTests"` → **90 passed** (up from 75 in Pass 1).
- `dotnet test ... --filter "Category=Governance|FullyQualifiedName~Diagnostic"` → **268 passed**.
- `dotnet test tests/Hexalith.FrontComposer.Shell.Tests/Hexalith.FrontComposer.Shell.Tests.csproj --filter "FullyQualifiedName~CiGovernanceTests"` → **18 passed**.
- `pwsh ./eng/validate-docs.ps1` → passed; registry/sample/stub fingerprints fresh.
- Main CI-parity lane `dotnet test Hexalith.FrontComposer.sln --filter "Category!=Performance&Category!=e2e-palette&Category!=NightlyProperty&Category!=Quarantined"` → **2,949 passed** across Contracts/Mcp/Cli/Shell/SourceTools/Testing (3 skipped pre-existing).

R6 (README package-validation prose) verified accurate on inspection — `FrontComposerPackageValidationBaselineVersion` is defined in `Directory.Build.targets:5`, matching the README's current text; no edit required.
...
- 2026-05-11 (code-review pass 2): `dotnet build tests/Hexalith.FrontComposer.SourceTools.Tests/Hexalith.FrontComposer.SourceTools.Tests.csproj --configuration Release` -> succeeded, 0 warnings, 0 errors.
- 2026-05-11 (code-review pass 2): `dotnet test tests/Hexalith.FrontComposer.SourceTools.Tests/Hexalith.FrontComposer.SourceTools.Tests.csproj --configuration Release --filter "FullyQualifiedName~DiagnosticRegistryTests"` -> passed, 90 tests (after R9 implementation).
- 2026-05-11 (code-review pass 2): `dotnet test tests/Hexalith.FrontComposer.SourceTools.Tests/Hexalith.FrontComposer.SourceTools.Tests.csproj --configuration Release --filter "Category=Governance|FullyQualifiedName~Diagnostic"` -> passed, 269 tests.
- 2026-05-11 (code-review pass 2): `dotnet test tests/Hexalith.FrontComposer.Shell.Tests/Hexalith.FrontComposer.Shell.Tests.csproj --configuration Release --filter "FullyQualifiedName~CiGovernanceTests"` -> passed, 18 tests.
- 2026-05-11 (code-review pass 2): `pwsh ./eng/validate-docs.ps1` -> passed; producer fingerprints stable (no registry edits beyond Pass 1).
- 2026-05-11 (code-review pass 2): `dotnet test Hexalith.FrontComposer.sln --configuration Release --filter "Category!=Performance&Category!=e2e-palette&Category!=NightlyProperty&Category!=Quarantined"` -> passed, 2,949 tests (3 skipped pre-existing).
...
- 2026-05-11: `/bmad-code-review 11-2` Pass 2 triaged 0 decision-needed + 42 patch + 7 defer; applied all 42 patches (R9 multi-surface determinism implemented via `GovernanceEnumerations_AreDeterministicAcrossSurfaces` covering source-files, docs-stub-rows, related-ids, suppressions, and package-groups in addition to the registry-rows shuffle). Major tightenings: severity matrix now spans registry + Roslyn descriptor + release-row + docs-stub channels (R1/R42); `mandatedStates` ledger contract rewritten with anchored regex matchers (R2); `SourceToolsAnalyzerReleaseRows_AllHaveBackingDescriptor` split into HFC-descriptor parity + HFCM-absence checks (R3); stub-body relatedIds parity test added (R4); `Story112LedgerRowRegex` broadened to hyphenated suffixes and `__bold__` variant (R5); `forbiddenSentinels` extracted into `AssertNoForbiddenSentinels` and applied to samples + docs stubs + migration-findings.json + validation reports (R7); Fisher-Yates shuffle replaces broken `OrderBy(rng.Next())` (R8); migration-findings validator extended with `hfcm-invalid-severity`/`hfcm-invalid-category`/`hfcm-malformed-slug`/`hfcm-malformed-approved-on`/`hfcm-unsupported-schema`/`hfcm-missing-findings-array` categories (R10/R12/R14); `Theory` restructured with row-index parameter, default-case throw, and 9 new mutations (R11/R22/R24/R33/R39); `TryParseVersionToken` catch-all (R13); unknown-root-field validator added (R16); unsupported-schema sample round-trip and unsafe-generated-front-matter JSON validator added (R17/R18); invisible chars in `IsConfusableOrFormatChar` replaced with explicit `\uXXXX` escapes (R19); `DiagnosticsDocsLinkUrlRegex` simplified and caller switched to structural `Uri.TryCreate` checks (R20); `uniqueRows` dedupe includes `newState` (R21); UTF-8 byte budget for samples (R23); `relatedIds` Distinct enforced (R25); CRLF + shell-pipe `|` lead + HTML-entity-decode + NFKC normalization in `ValidateGeneratedFrontMatter` (R26/R29/R31); `DescriptorCanonicalHelpLinkFormat` iterates registry skipping retired/reserved (R27/R35); `SemverShapeRegex` tightened to strict SemVer 2.0.0 shape (R37); placeholder convention unified in `reserved-retired-misuse-report.json` (R40). Validation rerun: `DiagnosticRegistryTests` 89 passed, governance/diagnostic slice 268 passed, Shell `CiGovernanceTests` 18 passed, `eng/validate-docs.ps1` passed, main CI-parity lane 2,949 passed.
