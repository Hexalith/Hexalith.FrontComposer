...
| `src/Hexalith.FrontComposer.Shell/AnalyzerReleases.Unshipped.md` | Modify — add `HFC2108_ShortcutConflict` Information row (Task 1.3). |
...
| `tests/Hexalith.FrontComposer.Shell.Tests/Shortcuts/HFC2108ShortcutConflictLogTest.cs` | Create — structured log assertion (Task 10.1e, D19). |
...
| AC | One-liner | Task(s) |
|---|---|---|
| AC1 | `IShortcutService` registers 3 shell defaults with HFC2108 duplicate-detection; `Disposable.Dispose()` removes entries (D1-D3, D19). | 1.1, 1.2, 1.3, 1.4, 10.1, 10.1e |
