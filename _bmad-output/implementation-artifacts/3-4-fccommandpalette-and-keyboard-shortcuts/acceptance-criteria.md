...
**And** **informational** shortcut rows (null `RouteUrl`) are NOT activatable on Enter — selecting does not dispatch navigation
**And** informational shortcut rows carry `aria-disabled="true"` on the `<li role="option">` element — this makes the non-activatability discoverable to screen-reader users (VoiceOver/NVDA announce "dimmed" / "unavailable") and prevents the silent-failure UX where a sighted keyboard user presses Enter with no visible / audible feedback (post-elicitation gap-fill)

> **AC6 ⇄ D11 clarification (2026-04-21 Pass 5 C1-D2 resolution):** shortcut rows with a non-null `RouteUrl` (e.g., `g h → "/"`, plus any adopter-registered shortcut that carries a navigable route) ARE activatable on Enter or click — `HandlePaletteResultActivated` navigates via `NavigationManager.NavigateTo(result.RouteUrl)` per D11, and the row does NOT carry `aria-disabled="true"`. Only informational-reference rows (null `RouteUrl` — the in-shell registrations whose invocation path is the actual chord, not navigation) are disabled. The `IsInformationalShortcut(result) => result.Category == Shortcut && string.IsNullOrEmpty(result.RouteUrl)` helper in `FcPaletteResultList.razor.cs` + `FcCommandPalette.razor.cs` is the single predicate for this distinction; covered by `ShortcutRowsCarryAriaDisabledTrue` (null-RouteUrl case) + `ShortcutRowWithRouteUrl_DoesNotCarryAriaDisabled` + `ShortcutRowWithRouteUrl_InvokesSelectionCallbackOnClick` (non-null-RouteUrl cases).
