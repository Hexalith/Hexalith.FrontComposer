...
- [x] T8. Tests and verification (AC1-AC25).
  ...
  - [ ] Code-fix tests using Microsoft.CodeAnalysis.Testing-style analyzer/code-fix verification for every automated migration. **(unchecked 2026-05-09 second-pass review — D3-FOLLOWUP; no `Microsoft.CodeAnalysis.Testing` references; tracked in Known Gaps row "T8 Microsoft.CodeAnalysis.Testing code-fix tests".)**
  ...
  - [ ] Add code-action safety tests where providers return custom operations, unsupported file operations, outside-project additions, and unsafe Fix All results; all must be rejected or reported manual-only without file writes. **(unchecked 2026-05-09 second-pass review — D3-FOLLOWUP; tracked in Known Gaps row "T8 code-action safety tests".)**
  ...
  - [ ] Add a bounded large-fixture or benchmark-style integration test proving inspect/migrate avoid unnecessary repeated full-tree work and complete within an agreed CI threshold. **(unchecked 2026-05-09 second-pass review — D3-FOLLOWUP; only an 80-file deterministic fixture exists; no perf threshold; tracked in Known Gaps row "T8 large-fixture/benchmark threshold".)**
  - [ ] Tool packaging test: `dotnet pack`, local tool install or `dotnet tool run`, and optional .NET 10 `dnx` smoke path when available in CI image. **(unchecked 2026-05-09 second-pass review — D3-FOLLOWUP; only CI smoke; in-repo packaging test absent; tracked in Known Gaps row "T8 in-repo packaging + dnx smoke".)**
...
- [ ] [Review][Patch] **P-D4 — Wire manual-only detection to a real SourceTools-emitted diagnostic.** **Not applied 2026-05-09:** requires SourceTools generator changes (new HFCM01xx ID + emit), out of scope for the second-pass review batch. Tracked in Known Gaps row "HFCM01xx new diagnostic ID for manual migration markers (P-D4)". Owner: Story 9-2 (this story, follow-up commit) coordinating with Story 9-4 governance. Replace `MigrationDiagnosticScanner.ManualApi = "ConfigureFrontComposerCustomMigration"` synthetic identifier check with a Roslyn diagnostic-driven detector: scan the project's analyzer-emitted `HFCM01xx` diagnostics (or a new `HFCMxxxx` reserved for manual-migration markers) instead of pattern-matching identifier text. Reserve the new ID in `src/Hexalith.FrontComposer.SourceTools/AnalyzerReleases.Unshipped.md` (coordinate with Story 9-4 governance). Update CLI tests to feed real diagnostic surfaces from a synthetic SourceTools generator output, not literal identifier matches. [`src/Hexalith.FrontComposer.Cli/MigrationCommand.cs:1024-1042`, `src/Hexalith.FrontComposer.SourceTools/AnalyzerReleases.Unshipped.md`]
...
- [ ] [Review][Patch] **CSPROJ `**` glob has incorrect semantics.** **Not applied 2026-05-09:** would require either adding `Microsoft.Extensions.FileSystemGlobbing` or rewriting the glob matcher. Defer to a follow-up; document limitation explicitly in CLI README. [`src/Hexalith.FrontComposer.Cli/MigrationCommand.cs`]
...
- [ ] [Review][Patch] **`Failed` apply entry path schema inconsistency.** **Partial 2026-05-09:** apply-time `Failed` calls now uniformly use `edit.RelativePath`. Plan-time `Failed` for "workspace could not be initialized" still uses `Path.GetFileName(projectPath)` because no project document is yet identified. Document the contract in JSON schema notes. [`src/Hexalith.FrontComposer.Cli/MigrationCommand.cs`]
- [ ] [Review][Patch] **`Console.CancelKeyPress` handler captures CTS that may be disposed before second Ctrl-C.** **Not applied 2026-05-09:** the `using` scope at top-level keeps the CTS alive for the entire process lifetime; the second-Ctrl-C race is theoretical. Defer. [`src/Hexalith.FrontComposer.Cli/Program.cs`]
...
- [ ] [Review][Patch] **No tests for `HasOverlappingChanges` or `TryExtractDocumentChanges` allowlist rejection paths.** **Not applied 2026-05-09:** test backfill out of scope for the second-pass review. Tracked in Known Gaps row "T8 code-action safety tests" and "T8 Microsoft.CodeAnalysis.Testing code-fix tests". [`tests/Hexalith.FrontComposer.Cli.Tests/MigrationCommandTests.cs`]
