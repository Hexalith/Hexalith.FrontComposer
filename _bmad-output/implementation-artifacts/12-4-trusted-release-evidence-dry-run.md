...
- [ ] [Review][Decision→Patch][Decision accepted; resolution pending P14] CR-12-4-D2 Same-version legitimate rerun is permanently blocked [`eng/release_evidence.py:228`] — Decision: fresh dispatch only. `rerun-review` is never publish-authorized; planned patch CR-12-4-P14 will add a typed `next_owner_action` to the decision output explaining "rerun is never publish-authorized; create a new dispatch/tag". **Status:** decision accepted, patch CR-12-4-P14 still unchecked.
- [ ] [Review][Decision→Patch][Decision accepted; resolution pending P15] CR-12-4-D3 Negative fixtures inject blocking signals directly into `checks.*` [`tests/ci-governance/fixtures/release-readiness-cases.json`] — Decision: add end-to-end fixtures that exercise real signal production from workflow steps (recursive submodule command on disk, post-seal artifact mutation on disk, concurrent run-state detection, etc.). Planned patch CR-12-4-P15. **Status:** decision accepted, patch CR-12-4-P15 still unchecked.

- [ ] [Review][Patch] CR-12-4-P1 [CRITICAL] publishCmd `||` chain writes `partial-publish-incident.json` even when no NuGet push was attempted [`.releaserc.json:12`] — operator precedence puts `(push1 && push2 || partial-incident)` inside `&& (...)`, so if `push1` fails before any artifact reaches NuGet, the fallback group still records "partial-publish" identical to the case where some packages did upload. Recovery tooling and the release-owner summary cannot distinguish "no-op failure" from "real partial publish".
- [ ] [Review][Patch] CR-12-4-P2 [CRITICAL] Repo-variable `vars.RELEASE_OWNER_APPROVED` (and `vars.RELEASE_APPROVER`) bypasses per-release approval on `push: main` [`.github/workflows/release.yml:28-32`] — env evaluates `${{ inputs.release_owner_approved || vars.RELEASE_OWNER_APPROVED || 'false' }}`. A maintainer can set `vars.RELEASE_OWNER_APPROVED=true` and `vars.RELEASE_APPROVER=release-owner` once, after which every push to main auto-publishes with no per-release human gate. Defeats AC24/AC26/D11.
- [ ] [Review][Patch] CR-12-4-P3 [CRITICAL] Four `BLOCKING_CHECKS` are default-pass in argparse and never populated by publishCmd [`eng/release_evidence.py:961-977`, `.releaserc.json:12`] — `paths_status`, `helper_state`, `semantic_release_state`, `redaction_status` all default to their passing values; `raw_evidence` defaults to `""` so `contains_dangerous_evidence` is trivially passed. The publish path never asserts these from real evidence files. AC16/AC29 implemented but not wired. Fix: classifier must fail-closed when these are absent in trusted-publish mode, AND publishCmd must read them from machine evidence (verification JSON, helper exit codes, redaction scan output).
- [ ] [Review][Patch] CR-12-4-P4 [CRITICAL] `prepare-manifest --attestation-status` defaults to `"approved-unsupported"`; not forwarded from `RELEASE_ATTESTATION_STATUS` [`eng/release_evidence.py:738`, `.releaserc.json:11`] — when `ATTESTATION_UNSUPPORTED!=true` the env says `attested`, but the sealed manifest rows say `approved-unsupported`. Either the manifest disagrees with the classifier (D17/AC31 violation), or a run authorizes as `ready` with no attestation evidence in the manifest. Fix: forward env to prepareCmd; treat absence as fail-closed.
- [ ] [Review][Patch] CR-12-4-P5 [CRITICAL] `prepare_manifest` never writes `timestamp_status` to package rows; `manifest_diagnostics` defaults missing field to `"verified"` [`eng/release_evidence.py:200, 501-512`] — AC7 requires timestamp evidence as a blocking gate. Production rows have no timestamp_status, the default-"verified" fallback silently passes. The `stale-missing-timestamp` fixture only passes because its override explicitly injects `"timestamp_status":"missing"`. Fix: write real timestamp_status from signing output, remove the default-"verified" fallback.
- [ ] [Review][Patch] CR-12-4-P6 [CRITICAL] Three readiness signals hardcoded `"false"` in publishCmd: `--release-definition-drift`, `--post-seal-artifact-mutation`, `--concurrent-same-version` [`.releaserc.json:12`] — no workflow step computes them. AC30/AC32/AC33 (D15/D16/D18) violated. Fix: classifier fail-closed when missing in trusted mode, AND add workflow steps to compute each (fingerprint compare, sha256 re-verify of `nupkgs-signed/*.nupkg` vs sealed manifest, concurrent run-state check).
- [ ] [Review][Patch] CR-12-4-P7 [HIGH] verify-manifest before publish doesn't recompute sha256 of `nupkgs-signed/*.nupkg` vs sealed manifest [`.releaserc.json:12`, `eng/release_evidence.py:780-797`] — post-seal artifact swap, repack, or rename undetected. AC32/D16 violated. Fix: add digest re-verification step that reads sealed checksums and recomputes them against the actual files about to be pushed.
- [ ] [Review][Patch] CR-12-4-P8 [HIGH] `benchmark_summary_hash` default `"candidate-benchmark-summary"` placeholder passes `manifest_diagnostics` [`eng/release_evidence.py:206-208, 737`] — only the `pending-` prefix is rejected; no `looks_like_sha256` check. AC12/AC13/D5 violated. Fix: validate as sha256, or require explicit value.
- [ ] [Review][Patch] CR-12-4-P9 [HIGH] No real attestation generation step, but env defaults `RELEASE_ATTESTATION_STATUS` to `"attested"` when `ATTESTATION_UNSUPPORTED!=true` [`.github/workflows/release.yml:30`] — workflow has `permissions: attestations: write` but no `actions/attest-build-provenance` step. AC9/D7 violated. Fix: add the attestation step, OR default to `"approved-unsupported"` until real attestation is wired (with a typed `attestation_state=missing-implementation` diagnostic).
- [ ] [Review][Patch] CR-12-4-P10 [HIGH] publishCmd hardcodes `--from-fork "false"` and omits `--dry-run` [`.releaserc.json:12`] — makes fork-PR guard unreachable; latent landmine if triggers expand to `pull_request_target` or `repository_dispatch`. AC22/AC23/D9/D10. Fix: resolve from-fork from `${{ github.event.pull_request.head.repo.fork }}` or `${{ github.repository_owner_id != github.event.repository.owner.id }}`.
- [ ] [Review][Patch] CR-12-4-P11 [HIGH] `test_status` defaults to `"passed"` and `test_count=1` if `--test-results` arg is missing [`eng/release_evidence.py:858, 968-977`] — if file path drifts or the workflow step is skipped, classifier silently passes the test gate. AC3/D20 violated. Fix: defaults must be fail-closed (`test_status="missing"`, `test_count=0`, `trx_present=false`).
- [ ] [Review][Patch] CR-12-4-P12 [HIGH] `fallback_complete` parses `expires_at` with `dt.date.fromisoformat`; rejects any ISO datetime string [`eng/release_evidence.py:644-647`] — on Python <3.11 (Ubuntu 22.04 default), only `YYYY-MM-DD` parses; an operator pasting a datetime string silently falls into a generic "missing/stale/incomplete" diagnostic with no hint of the format error. Fix: accept ISO datetime explicitly, or emit a typed diagnostic naming the invalid format.
- [ ] [Review][Patch] CR-12-4-P13 [HIGH] `int(context.get("run_attempt", 1) or 1)` and `int(os.environ.get("GITHUB_RUN_ATTEMPT", "1"))` raise unhandled `ValueError` on non-numeric input [`eng/release_evidence.py:226, 947`] — env present-but-empty (`""`) or JSON `"abc"` crashes the helper with a traceback instead of producing a typed `helper_state="partial"` diagnostic. Fix: try/except → fall back to 1 and append a typed diagnostic.
- [ ] [Review][Patch] CR-12-4-M1 [MEDIUM] `manifest_diagnostics` lockstep version drift partial enforcement [`eng/release_evidence.py:568-571`] — `version = version or row.get("version")` skips falsy first-row versions, and `None != None` is `False` so all-missing-versions never trigger drift. Fix: independently flag missing version per row, then compare strictly.
- [ ] [Review][Patch] CR-12-4-M2 [MEDIUM] `classify_context` `partial_publish_state` capitalization bug [`eng/release_evidence.py:606-607`] — `str(context.get("partial_publish_state", "none"))` yields `"None"` for a JSON `null` value, which is `!= "none"` and erroneously triggers `rerun-review`. Fix: lowercase normalize + explicit None handling.
- [ ] [Review][Patch] CR-12-4-M3 [MEDIUM] `DANGEROUS_EVIDENCE_PATTERNS` regex coverage gaps [`eng/release_evidence.py:510-517`] — Windows-path regex misses forward-slash variants (`C:/Users/...`, common on Windows Python), and GitHub token regex misses `gho_`, `ghs_`, `ghu_`, `ghr_` prefixes (only `ghp_` and `github_pat_` are present). AC29 partial. Fix: extend both regex sets.
- [ ] [Review][Patch] CR-12-4-M4 [MEDIUM] `classify-release --require-publishable` exits 1 for both "blocked release" and "tool crash" [`eng/release_evidence.py` argparse + main] — no machine-readable distinction. Fix: distinct exit codes (e.g., 2 for crash) or emit a `helper_state="crashed"` diagnostic before exit, so retry/escalation logic can differentiate.
- [ ] [Review][Patch] CR-12-4-M5 [MEDIUM] Approval truthiness only accepts literal `"true"` case-insensitive [`eng/release_evidence.py:825`] — `"yes"`, `"1"`, `"approved"` silently treated as `False`. An operator who sets `vars.RELEASE_OWNER_APPROVED="yes"` sees a blocked release with the generic "explicit approval required" reason. Fix: accept the documented truthy set OR reject anything outside `{"true","false"}` with a typed diagnostic naming the invalid value.
- [ ] [Review][Patch] CR-12-4-M6 [MEDIUM] `classify_context` priority order shadows `fork-pr` behind `rerun-review` [`eng/release_evidence.py:228-231`] — a fork-PR re-attempt is classified `rerun-review`, hiding the more relevant `fork-pr` root cause from `grouped_reasons` / `context_class`. Both are blocked, so safety holds, but the audit trail misleads. Fix: promote `from_fork` ahead of rerun in the priority chain.
- [ ] [Review][Patch] CR-12-4-M7 [MEDIUM] `release_definition_fingerprints` computed but never compared to a sealed baseline [`eng/release_evidence.py:622-627, 733`] — AC30 requires evidence binds immutable identifiers for workflow/.releaserc/helper/inventory/package versions, but the helper only records current fingerprints without asserting they equal a prior sealed copy. Fix: implement baseline-fingerprint comparison; surface drift as `release_definition_drift=true`.
- [ ] [Review][Patch] CR-12-4-L1 [LOW] Duplicate diagnostics for `sbom_hash=""` [`eng/release_evidence.py:203-207`] — both "manifest missing sbom_hash" and "manifest sbom_hash must be a concrete sha256" fire for the same empty-string root cause. Fix: suppress the second when the first already fired.
- [ ] [Review][Patch] CR-12-4-L2 [LOW] `release-readiness.json` and `test-results.json` are GitHub Release assets but not in the sealed manifest's checksum list [`.releaserc.json:24-25`] — strict reading of D16 says they should be bound, OR explicitly classified as advisory. Fix: include them in `prepare-manifest`'s checksum inputs, OR add an `advisory_artifacts` section to the decision output.
- [ ] [Review][Patch] CR-12-4-P14 [HIGH] Add typed `next_owner_action` message for rerun-review classifications [`eng/release_evidence.py` `classify_release_payload` / `next_owner_action`] — resolves CR-12-4-D2. Decision output must explain "rerun-review contexts are never publish-authorized; create a fresh dispatch or new tag to retry" instead of the current generic blocked message. Fixture `same-version-rerun-instruction` to assert the message is present in the decision JSON.
- [ ] [Review][Patch] CR-12-4-P15 [MEDIUM] Add end-to-end fixtures that exercise real workflow→helper signal production [`tests/ci-governance/fixtures/` + new test harness] — resolves CR-12-4-D3. Today's `release-readiness-cases.json` injects `checks.*` flags directly, proving only the classifier's last mile. End-to-end coverage should include: a fixture workspace with a recursive submodule command in checkout config, a sealed manifest paired with on-disk artifacts whose digests no longer match, a concurrent-run state file pair, and a workflow-fingerprint baseline that disagrees with current files. Likely sized as a separate test-harness story; this patch slot scopes the fixture surface and acceptance.
