...
- On `Reconnected`, rejoin active groups, then reconcile only currently visible projection/count lanes. Hidden pages, unrelated projections, and background prefetch remain out of scope.
...
| AC | Given | When | Then |
| --- | --- | --- | --- |
| AC1 | The EventStore projection hub reconnects after a previously connected session | The 5-3 connection state transitions to `Reconnected` | The client rejoins all currently active projection groups exactly once per `(projectionType, tenantId)`, does not duplicate group state outside `ProjectionSubscriptionService`, treats failed rejoins as degraded instead of silently active, and only starts reconciliation after rejoin attempts complete or are explicitly classified as failed. |
| AC2 | Visible projection pages, visible action-queue badge/count lanes, or other subscribed visible lanes exist | Reconnection reconciliation starts | Each visible lane issues an ETag-conditioned REST query through Story 5-2 seams, using framework-controlled tenant/user/discriminator values; hidden/unsubscribed lanes are skipped; `304 Not Modified` leaves visible state unchanged; `200 OK` updates from the response and marks the lane for one batched sweep. |
| AC3 | Catch-up queries return changed projection data | Stale rows or a changed visible lane are identified | All changed visible rows receive a single coordinated CSS sweep in the same render window; no row-by-row flash cascade occurs; if row-level identity/diff is unavailable, the visible lane receives a single lane-level sweep; `prefers-reduced-motion: reduce` makes the update instantaneous without sweep animation. |
| AC4 | Reconciliation completes and at least one visible lane changed | `FcSyncIndicator` or the equivalent Shell header status processes the result | The header reconnecting state clears and a `FluentMessageBar` with Info intent displays `Reconnected -- data refreshed`, has `role="status"` and `aria-live="polite"`, and auto-dismisses after 3 seconds without stealing focus. |
...
| AC7 | A schema evolution mismatch is detected at startup, during query deserialization, cache compatibility validation, or reconciliation | Projection type or payload shape cannot be safely consumed | A clear structured diagnostic is logged without raw tenant/user/token/payload data; all ETag cache entries for the affected projection type/discriminator family are invalidated best-effort; the affected UI renders `This section is being updated`; stale data is not shown as current and empty state is not used as a false success. |
...
- [ ] T2. Rejoin active projection groups through `ProjectionSubscriptionService` (AC1, AC9)
  ...
  - [ ] On reconnect, call `JoinGroup` once per active group after the hub reports connected/reconnected.
  - [ ] Preserve commit-after-join semantics from Story 5-1 and Story 5-3. A failed rejoin remains degraded and is retried by the next explicit reconnect/reconcile cycle or subscribe call, not marked active by optimism.
...
- [ ] T3. Build a visible-lane reconciliation coordinator (AC2, AC5, AC9)
  ...
  - [ ] Snapshot visible lanes at reconciliation start. Do not chase lanes that become visible later in the same pass; they can load normally through existing page/badge flows.
...
- [ ] T4. Route catch-up refreshes through Story 5-2 query/cache seams (AC2, AC5, AC9)
  ...
  - [ ] `200 OK` updates the normal page/badge state through existing success paths and returns enough result metadata to mark the lane changed.
...
- [ ] T5. Implement the batched sweep marker and reduced-motion styling (AC3, AC9)
  ...
  - [ ] Apply one sweep marker per changed lane in the same render cycle. Do not schedule one timer per row.
  ...
  - [ ] Add scoped CSS next to the affected component(s). Use `@media (prefers-reduced-motion: reduce)` to disable animation and render the final state immediately.
...
- [ ] T6. Add `FcSyncIndicator` / header reconnect + reconciliation UX (AC4, AC5, AC6, AC9)
  ...
  - [ ] Add EN/FR resources if the touched path already uses localization resources; otherwise keep exact copy inline and record the resource follow-up.
...
- [ ] T7. Add schema mismatch detection and cache invalidation policy (AC7, AC8, AC9)
  ...
  - [ ] Invalidate all ETag cache entries for the affected projection type/discriminator family best-effort. The invalidation operation must be bounded and must not scan unrelated LocalStorage entries without a prefix/index.
  - [ ] Render the exact user copy `This section is being updated` in the affected view/badge region; do not show empty state, stale rows, or raw exception details.
...
- [ ] T9. Tests and verification (AC1-AC9)
  - [ ] Connection/rejoin tests: reconnect event starts one pass, active groups rejoin exactly once, failed rejoin is degraded, duplicate subscribe/unsubscribe races are safe, disposal cancels the pass.
  - [ ] Reconciliation coordinator tests: visible-lane snapshot, hidden-lane skip, lane cap, dedupe, cancellation, tenant/user fail-closed, and superseded reconnect epoch cleanup.
  - [ ] Query/cache tests: ETag validators used, `304` no-churn, `200` changed-lane marker, 401/403/404/429/503 preserve visible state, protocol drift fails loudly without empty success.
  - [ ] Reducer/effect tests: sweep markers are batch-scoped, TTL clears markers, no per-row timer fan-out, reduced-motion path produces immediate final state.
  - [ ] Component/a11y tests: header `Reconnecting...`, `Refreshing data...`, Info message copy, `role="status"`, `aria-live="polite"`, 3-second auto-dismiss, silent no-change completion, no modal/overlay/focus trap.
