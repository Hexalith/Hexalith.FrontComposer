...
- [x] T1. Define SDK-neutral Markdown projection renderer contracts (AC1-AC6, AC10, AC12, AC14, AC18, AC19)
  - [x] Add package-owned models such as `McpProjectionRenderRequest`, `McpProjectionRenderResult`, `McpMarkdownProjectionDocument`, `McpMarkdownTable`, `McpMarkdownStatusSummary`, and `McpMarkdownTimeline`.
  - [x] Place pure renderer contracts under an SDK-neutral `Hexalith.FrontComposer.Mcp` rendering namespace/folder; keep MCP SDK DTOs, transport status mapping, and `TextResourceContents` conversion in the MCP adapter edge only.
  - [x] Keep contracts SDK-neutral until the final MCP resource adapter edge; do not expose MCP SDK DTOs from Contracts, SourceTools, or generated descriptors.
  - [x] Represent rendered output as `text/markdown` plus safe metadata: projection identifier, role, bounded context, row count category, truncation state, and correlation/request ID when already available.
  - [x] Exclude tenant IDs, user IDs, claims, roles, API keys, tokens, raw query filters, hidden resource names, service instances, `ClaimsPrincipal`, and raw exception text from renderer contracts.
  - [x] Define deterministic Markdown serialization rules for headings, tables, escaped cell text, status summaries, timelines, empty states, and truncation notices.
  - [x] Define output as an atomically committed render result: the adapter returns either a complete sanitized document or a sanitized failure category, never a partially formatted buffer.

- [x] T2. Reuse SourceTools projection metadata as the render source of truth (AC1-AC6, AC10, AC13)
  - [x] Consume the same projection descriptor/manifest data emitted by Story 8-1 rather than reflecting over arbitrary loaded assemblies at request time.
  - [x] Use `RazorModel`, `ColumnModel`, `ProjectionRenderStrategy`, badge mappings, entity labels, plural labels, empty-state CTA command names, descriptions, `FieldDisplayFormat`, and SourceTools label resolution.
  - [x] Consume IR semantics only; do not depend on Shell component types, Fluent UI tokens, CSS classes, visual layout hints, or web-only component implementation details.
  - [x] Preserve column order from the SourceTools transform: priority ordering and declaration-order tiebreaks where already defined by web rendering.
  - [x] Reuse web label/humanizer behavior. Do not add an MCP-only label parser or hard-coded adapter copy when IR metadata exists.
  - [x] Add manifest/descriptor tests proving Markdown projection metadata is single-source with web projection metadata and stable across repeated builds.

- [x] T3. Implement Default and ActionQueue Markdown tables (AC1, AC2, AC5-AC8, AC10, AC11)
  - [x] Render Default projections as GitHub-flavored Markdown tables with deterministic headers, escaped cell values, and stable row ordering from the query result.
  - [x] Render ActionQueue projections with the same table baseline plus pending-action context, safe CTA command labels, and semantic badge text where visible.
  - [x] Format null values, enums, booleans, dates/times, numbers, arrays/objects, unsupported values, and missing labels exactly as defined in the canonical formatting matrix.
  - [x] Do not include Markdown links or command suggestions that would bypass Story 8-2 tenant/policy visibility.
  - [x] Add tests for Markdown escaping of pipes, backticks, brackets, Markdown links/images, reference definitions, autolinks, HTML-like text, code fences, front matter, task-list syntax, blockquotes, mentions, slash-command-looking text, newlines, RTL/BOM/control characters, long words, and secret-looking payload fragments.
  - [x] Enforce default bounds: 100 rows, 20 columns, 256 characters per cell, 32,768 characters per Markdown document, and the exact truncation marker `Output truncated by FrontComposer agent rendering limits.`.

- [x] T4. Implement StatusOverview Markdown summaries (AC3, AC5, AC6, AC10, AC11)
  - [x] Map status/badge fields to text-only semantic slots: `Neutral`, `Info`, `Success`, `Warning`, `Danger`, and `Accent`.
  - [x] Render totals and grouped counts without colors, icons, CSS class names, or UI-only tokens.
  - [x] Preserve source labels and status text from projection metadata; fall back deterministically when metadata is absent.
  - [x] Define behavior for missing badge mappings, duplicate badge mappings, unknown status values, unsupported fields, empty groups, multiple status fields, and max 12 rendered status groups.
  - [x] Add approval tests covering stable grouping order, totals, zero-count suppression policy, and redaction.

- [x] T5. Implement Timeline Markdown rendering (AC4-AC6, AC10, AC11)
  - [x] Reuse the web timeline field selection rule where available, including first suitable date/time field and deterministic tiebreaks.
  - [x] Render chronological entries with timestamp, status label, primary title/description, and selected supporting fields.
  - [x] Render timelines newest-first by default; ties sort by stable query ordinal, then stable item key when available, and null timestamps render after timestamped entries.
  - [x] Handle missing timestamps, duplicate timestamps, unsupported event fields, null descriptions, and large timelines with bounded output.
  - [x] Enforce a default max timeline entry count of 100, sharing the document and cell/field bounds from table rendering.
  - [x] Add tests for timestamp ordering, duplicate ordering, badge text, multiline descriptions, truncation, and no raw tenant/user leakage.

- [x] T6. Wire MCP resource reads to the query and lifecycle seams (AC7-AC9, AC11, AC12)
  - [x] Route all projection reads through existing `IQueryService.QueryAsync<T>` or the Story 8-1 projection adapter; do not add a second EventStore or REST query client.
  - [x] Accept only Story 8-1/8-2 approved resource names, URI templates, and query parameters. Reject raw tenant/user overrides and descriptor overrides.
  - [x] Revalidate current auth, tenant, resource visibility, and policy scope for every read; previous list results, lifecycle terminal results, and copied URIs do not bypass visibility.
  - [x] Capture auth, tenant context, policy scope, descriptor epoch, catalog epoch, query request shape, and request ID into an immutable per-read snapshot after admission; if freshness changes before query/render handoff, return the sanitized stale descriptor/category response without querying or rendering.
  - [x] Preserve cancellation tokens, request IDs, ETag/cache discriminator rules, pagination, and degraded EventStore categories.
  - [x] Map successful render output to MCP C# SDK resource contents inside the MCP package, using `text/markdown` text resources.
  - [x] Treat malformed resource URIs, stale descriptors, oversized query parameters, hidden resources, unauthorized resources, query failures, timeouts, and cancellations as deterministic sanitized categories using the taxonomy in Dev Notes.
  - [x] Pass ETag/cache/degraded/read-your-writes classification into safe render metadata only; do not place opaque ETags, cache keys, tenant discriminators, lifecycle handles, or raw query filters into Markdown.

- [x] T7. Empty states, command suggestions, and scope discipline (AC7, AC9, AC13, AC14)
  - [x] Return `No [entities] found.` using entity plural labels from SourceTools when the result set is empty.
  - [x] Include safe command suggestions only from the Story 8-2 visible catalog, only when already represented by projection empty-state CTA metadata or explicit visible descriptors.
  - [x] Limit suggestions to empty-state-visible projections, max 5 suggestions, deterministic descriptor order, no Markdown links, and no suggestions for hidden/unknown/unauthorized/stale/malformed/oversized responses.
  - [x] Never suggest hidden, unauthorized, cross-tenant, stale, destructive, or policy-filtered commands.
  - [x] Do not implement fuzzy command suggestions, hallucination correction, or tenant-scoped listing rules here; consume Story 8-2 results only.
  - [x] Keep rich natural-language advice bounded and deterministic. No per-agent memory, dynamic planning, or generated prose that depends on request history.

- [x] T8. Security, redaction, and bounded runtime state (AC8-AC11, AC18-AC20)
  - [x] Redact JWT-like strings, API keys, client secrets, claims, role names, tenant IDs, user IDs, raw query filters, command payload fragments, provider internals, and exception text from Markdown, logs, and telemetry.
  - [x] Add zero-side-effect tests proving hidden/unknown/malformed/unauthorized/stale resource reads do not query EventStore, mutate cache, relay tokens, update SignalR state, allocate renderer state, or emit sensitive telemetry.
  - [x] Make descriptor registries, renderer lookup tables, response buffers, and render caches immutable after startup or bounded by explicit options.
  - [x] Do not add a renderer-owned cross-request Markdown cache in v1; rely on existing query/ETag seams and local per-request buffers unless a later versioned story defines a scoped cache contract.
  - [x] Discard local render buffers on cancellation, timeout, bounds failure, stale context, or exception; telemetry records only sanitized outcome categories and duration buckets.
  - [x] Add options for max projection rows, max columns, max cell length, max Markdown bytes/chars, max timeline entries, max status groups, max suggestions, and truncation behavior with range validation and documented defaults.
  - [x] Log sanitized outcome categories and duration buckets only; do not log rendered row payloads.

- [x] T9. Tests and verification (AC1-AC20)
  - [x] Use the risk-based test scope below: P0 must pass before review; P1 should be covered unless shared seams make it redundant; P2 is bounded and may be deferred only with an owner.
  - [x] Markdown renderer unit tests for Default, ActionQueue, StatusOverview, and Timeline roles using one constrained golden fixture per role.
  - [x] SourceTools parity tests for labels, descriptions, column ordering, badge mappings, entity labels, empty-state CTA metadata, field display formats, and unsupported placeholders.
  - [x] MCP resource adapter tests for direct resources vs. resource templates, `text/markdown` content mapping, SDK boundary containment, and cancellation propagation.
  - [x] Query integration tests covering two sample projections, tenant context injection, pagination, ETag/cache behavior, read-your-writes after Story 8-3 terminal confirmation, and degraded query outcomes.
  - [x] Hidden/unknown/unauthorized/stale resource tests consuming Story 8-2 semantics and proving no query or render side effects.
  - [x] Markdown safety tests for escaping, control characters, oversized rows, secret-looking values, raw exception text, tenant/user values, and deterministic truncation.
  - [x] Atomicity tests proving mid-render cancellation, timeout, formatter exception, and document-bound overflow return only sanitized taxonomy responses and never partial Markdown.
  - [x] Snapshot-staleness tests proving descriptor/catalog epoch changes between admission, query, and rendering are rejected without mixing old visibility with new query data.
  - [x] Snapshot/golden tests proving stable Markdown for supported projection roles across repeated runs; normalize timestamps/culture and avoid SDK-generated DTO snapshots.
  - [x] Regression: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false`.
  - [x] Targeted tests: `tests/Hexalith.FrontComposer.Mcp.Tests`, `tests/Hexalith.FrontComposer.SourceTools.Tests`, and Shell/EventStore query tests only if shared query/render seams change.

### Review Findings

Generated by `/bmad-code-review 8-4` on 2026-05-03 against commit `7a69cdf`. Three review layers: Blind Hunter (diff-only), Edge Case Hunter (boundary paths), Acceptance Auditor (AC1-AC20 + Tasks + Contracts + Decisions).

#### Decisions Resolved

- [x] [Review][Decision][Defer→8-4a] **Sanitized response taxonomy strings not implemented (AC9, AC11, AC16, AC19)** — Decision: defer the canonical agent-visible string emission to Story 8-4a. Added `ResponseTooLarge`, `UnsupportedRender`, `StaleDescriptor`, `DegradedResult` failure categories so 8-4a can wire the renderer/adapter to emit canonical text without churning the enum surface again. Required design work: agree on whether failure responses come back as Markdown text content (`Success(text)`) or as MCP-level errors with a sanitized payload string.
- [x] [Review][Decision][Defer→8-4a] **Immutable per-read snapshot and descriptor/catalog epoch revalidation missing (AC18, T6, D13, Immutable Read Snapshot Contract)** — Decision: defer to Story 8-4a. Cross-cuts Story 8-2's catalog/descriptor admission service which does not currently expose epochs. Implementing in isolation here would require a private epoch counter that 8-2 ignores; better to design once, jointly.
- [x] [Review][Decision][Defer→8-4a] **P0 risk-tier test coverage gaps (AC17, T9)** — Decision: defer to Story 8-4a. Most P0 tests depend on the sanitized taxonomy and snapshot/epoch infrastructure being settled. Existing renderer/adapter tests cover happy-path roles, escaping, redaction, cancellation, formatter exception, bounded-context CTA scoping, and link-shaped suggestion drop.
- [x] [Review][Decision][Defer→8-4a] **Atomic Markdown commit on bounds failure (AC19, D14)** — Partial: `BoundDocument` now cuts on the last newline before the budget so truncation never lands mid-row, and throws `ResponseTooLarge` when no clean cut is available (rather than emitting incoherent output longer than the budget). Full atomic-commit semantics (discard buffer + sanitized category text reaching the agent) couples to the deferred sanitized taxonomy work in 8-4a.
- [x] [Review][Decision][Applied] **`Suggestions:` inline label is invented (AC7, T7)** — Resolution: drop the inline label entirely. Suggestion bullets now render directly under the empty-state line, no extra heading-like construct.
- [x] [Review][Decision][Dismissed] **`Total: {totalCount}` may expose hidden/filtered totals** — `QueryRequest.TenantId` is filled from authenticated tenant by Shell adapters per `QueryRequest.cs:7`, and `QueryResult.TotalCount` reflects the post-filter count for pagination per `QueryResult.cs:8`. Total is already visible-scope; no leak.
- [x] [Review][Decision][Defer→8-4a] **Unsupported columns silently dropped (AC15, Story 4-6 semantics)** — Decision: keep current MCP behavior (drop) and document the deviation in Story 8-4a alongside the Story 4-6 semantics audit. Chat-readable surface differs from web grid surface; the renderer comment now references this carry-over.

#### Patches

- [x] [Review][Patch] Boolean cells now render `Yes`/`No` per Canonical Formatting Matrix [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatCell`]
- [x] [Review][Patch] Date/time cells now render `yyyy-MM-dd HH:mm:ss 'UTC'` after UTC conversion; `FormatDateTime` handles all `DateTimeKind` values; tests updated [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatCell`/`FormatDateTime`, `tests/Hexalith.FrontComposer.Mcp.Tests/Rendering/McpMarkdownProjectionRendererTests.cs`, `tests/Hexalith.FrontComposer.Mcp.Tests/Invocation/ProjectionReaderCoverageTests.cs`]
- [x] [Review][Patch] StatusOverview now keys groups by `slot+member`, severity-orders via `SlotSeverityIndex` (Danger → Warning → Success → Info → Accent → Neutral → unknown), and falls back to ordinal label for ties; status field is selected across all non-unsupported descriptor fields, not just the column-truncated subset [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RenderStatusOverviewDocument`/`SlotSeverityIndex`]
- [x] [Review][Patch] Dashboard/unknown strategies now throw `FrontComposerMcpFailureCategory.UnsupportedRender` instead of silently falling through to `RenderTableDocument`; supported set explicitly enumerated as `Default`/`ActionQueue`/`DetailRecord`/`StatusOverview`/`Timeline` [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `Render`]
- [x] [Review][Patch] `BoundDocument` now cuts on the last newline before the budget so truncation never lands mid-table-row, escapes the marker via `EscapeMarkdownText`, and throws `FrontComposerMcpFailureCategory.ResponseTooLarge` when the marker alone exceeds the bound or no clean cut is available (atomic-commit partial: full sanitized-text emission deferred to 8-4a) [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `BoundDocument`]
- [x] [Review][Patch] Status/timestamp/title field selection in StatusOverview and Timeline now searches across `descriptor.Fields.Where(!IsUnsupported)`, not the column-truncated `fields` subset [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RenderStatusOverviewDocument`/`RenderTimelineDocument`]
- [x] [Review][Patch] `Humanize` iterates via Rune-aware code-point handling; surrogate pairs preserved, `Rune.IsUpper`/`Rune.IsLetterOrDigit` drives spacing [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `Humanize`]
- [x] [Review][Patch] Empty-string cells render `-` per Canonical Formatting Matrix [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatCell`]
- [x] [Review][Patch] `MatchesEmptyStateCta` now requires bounded-context match and anchors on full namespace-segment equality via `SegmentMatches` [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs` `MatchesEmptyStateCta`/`SegmentMatches`]
- [x] [Review][Patch] Reader short-circuits when `MaxProjectionSuggestions <= 0`; floor of 1 removed; renderer's `Math.Max(0, ...)` retained for symmetry [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs` `BuildSafeSuggestionsAsync`]
- [x] [Review][Patch] Bearer redaction broadened to match opaque bearer tokens (`(?i)\bbearer\s+[A-Za-z0-9_\-\.]+`) so non-JWT bearers no longer leak [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RedactSensitiveText`]
- [x] [Review][Patch] `new DateTimeOffset(...)` calls wrapped in `try { ... } catch (ArgumentOutOfRangeException) { return null; }`; Unspecified-Kind treated as UTC; per-row null degradation no longer collapses the whole render [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `ReadDateTimeOffset`]
- [x] [Review][Patch] Timeline now adds stable item-key tiebreak via `ReadStableItemKey` (tries `Id`, `Key`, `Name` properties; ordinal fallback when none) [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RenderTimelineDocument`/`ReadStableItemKey`]
- [x] [Review][Patch] `IsDateTimeField` no longer matches `TimeOnly` (which can't yield `DateTimeOffset`) or `RelativeTime` `DisplayFormat` (which may sit on non-date runtime types) [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `IsDateTimeField`]
- [x] [Review][Patch] Arrays/objects now render the unsupported placeholder (`(unsupported)`) instead of joining raw `value?.ToString()` per element [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatEnumerable`/`UnsupportedPlaceholder`]
- [x] [Review][Patch] Dead `Replace("- [ ]", "\\- \\[ \\]")` removed from `EscapeMarkdownText`; per-character escape already neutralizes the task-list pattern [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `EscapeMarkdownText`]
- [x] [Review][Patch] Empty-state suggestions filtered through `LooksLikeLinkOrCommand` (rejects `[]()<>\``, `://` schemes, leading-`/`, inline whitespace-`/`-nonwhitespace patterns) before emit; test `Render_EmptyProjection_IncludesOnlySafeVisibleSuggestions` updated to require unsafe inputs are dropped, not escaped [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `AppendEmptyState`/`LooksLikeLinkOrCommand`, `tests/Hexalith.FrontComposer.Mcp.Tests/Rendering/McpMarkdownProjectionRendererTests.cs`]
- [x] [Review][Patch] `MaxProjectionCellCharacters` floor enforced at `4` via options validator; `TrimCell` budget raised from `Math.Max(1, ...)` to `Math.Max(4, ...)` [`src/Hexalith.FrontComposer.Mcp/Extensions/FrontComposerMcpServiceCollectionExtensions.cs` `FrontComposerMcpOptionsValidator`, `src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatCell`]
- [x] [Review][Patch][Defer→8-4a] In-text truncation marker fires when `items.Count > rendered` even though caller already capped via `IQueryService.Take`, while document `IsTruncated` flag does not include this condition — marker text and metadata flag disagree; reconciled query/field/row truncation with document metadata while preserving sanitized taxonomy follow-up scope [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs`, `src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs`]
- [x] [Review][Patch][Defer→8-4a] Numeric formatting still uses `formattable.ToString(null, Invariant)` ignoring `DisplayFormat` metadata. Currency `DisplayFormat` now uses the SourceTools-propagated numeric format through invariant formatting in `FormatCell` [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatCell`]
- [x] [Review][Patch][Defer→8-4a] `McpResourceDescriptor.RenderStrategy` remains stringly-typed. Public MCP descriptors now use typed `McpProjectionRenderStrategy`; the source generator emits typed enum literals while keeping SourceTools intermediate data analyzer-load safe [`src/Hexalith.FrontComposer.Contracts/Mcp/McpResourceDescriptor.cs`, `src/Hexalith.FrontComposer.Contracts/Mcp/McpProjectionRenderStrategy.cs`, `src/Hexalith.FrontComposer.SourceTools/Emitters/McpManifestEmitter.cs`]

#### Deferred (pre-existing or out of scope)

- [x] [Review][Defer] `Humanize` doesn't preserve consecutive uppercase runs (`IDFraud` → `I D Fraud` instead of `ID Fraud`) [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs:329-345`] — deferred, label-quality polish
- [x] [Review][Defer] `FormatEnumerable` hardcoded 5-element cap ignores options [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs:316-327`] — deferred, options-pattern cleanup
- [x] [Review][Defer] `EmitStringDictionary` does not null-check `values` (producer side defaults to empty so safe today) [`src/Hexalith.FrontComposer.SourceTools/Emitters/McpManifestEmitter.cs`] — deferred, defensive cleanup
- [x] [Review][Defer] `MaxFieldsPerResource = 0` and `MaxRowsPerResource = 0` silently fall back to 1 via `Math.Max(1, ...)` clamp; no validator [`src/Hexalith.FrontComposer.Mcp/FrontComposerMcpOptions.cs` validator] — deferred, options-pattern hardening
- [x] [Review][Defer] `AppendEmptyState` lowercases plural label fallback via `ToLowerInvariant()` ("GDPR exports" → "gdpr exports") [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs:234-237`] — deferred, locale/proper-noun handling
- [x] [Review][Defer] `RenderStatusOverviewDocument` returns title+total when no status field is found, with no diagnostic [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs:127-129`] — deferred, telemetry follow-up

#### Re-review (2026-05-04, second pass)

Generated by `/bmad-code-review 8-4` on 2026-05-04 against commit `7a69cdf` (the 8-4 implementation commit). Three review layers re-run: Blind Hunter, Edge Case Hunter, Acceptance Auditor. Verification spot-checks against current `HEAD` (post `9f1299c`) confirm that **all 21 patches and 7 decisions from the first pass landed correctly**. The bulk of new findings (≈60) duplicate already-patched items and are dismissed. The list below is only the residue.

##### Patches

- [x] [Review][Patch] `RedactSensitiveText` covers only `api_key|client_secret|authorization`, opaque bearer, and JWT-shaped `eyJ…` tokens. Broadened the key=value regex to include `password|pwd|secret|token|connection_string` so common credential variants are redacted before reaching agents [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RedactSensitiveText`]
- [x] [Review][Patch] `EmitStringDictionary` emits `new Dictionary<string, string>()` (and the populated form) without a `StringComparer.Ordinal` argument; the runtime model defaults to `StringComparer.Ordinal`. Emit `new Dictionary<string, string>(StringComparer.Ordinal)` so generated descriptors match the model's casing semantics; also added `using System;` to the generated source for the comparer reference [`src/Hexalith.FrontComposer.SourceTools/Emitters/McpManifestEmitter.cs` `EmitStringDictionary`]
- [x] [Review][Patch] `TrimCell` cuts at `text[..Math.Max(0, maxLength - 3)]`; if the cut lands inside a UTF-16 surrogate pair (astral-plane emoji, non-BMP) the result is invalid UTF-16 and may serialize to malformed UTF-8. Backed off the cut by one when it would split a high/low surrogate pair so truncation never splits a code point [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `TrimCell`]
- [x] [Review][Patch] `McpManifestTransform.TransformProjection` calls `column.BadgeMappings.ToImmutableArray().ToDictionary(m => m.EnumMemberName, m => m.Slot, StringComparer.Ordinal)` which throws `ArgumentException` on duplicate `EnumMemberName` keys. Replaced with `GroupBy(.., StringComparer.Ordinal).ToDictionary(g => g.Key, g => g.First().Slot, StringComparer.Ordinal)` so duplicate-mapping precedence is "first wins" deterministically rather than crashing manifest source-gen [`src/Hexalith.FrontComposer.SourceTools/Transforms/McpManifestTransform.cs`]
- [x] [Review][Patch][Defer→follow-up] Enum cell labels rely on `Humanize(value.ToString())` and ignore `[Display(Name=…)]` / `[DisplayAttribute(ResourceType=…)]` metadata that the web grid honors via `LabelResolver`. Plumbing display labels through the SourceTools IR requires extending `EnumMemberNames` from `ImmutableArray<string>` to a typed `(MemberName, DisplayName?)`, threading through Contracts (`McpParameterDescriptor.EnumValues`) → emitter → renderer (`FormatEnum`, `RenderStatusOverviewDocument`) → tests. Coordinate with the in-flight 8-6 schema-fingerprint canonicalization (which currently hashes the raw `EnumMemberNames` array) before changing IR shape [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatEnum`/`RenderStatusOverviewDocument`, `src/Hexalith.FrontComposer.SourceTools/Transforms/McpManifestTransform.cs`, `src/Hexalith.FrontComposer.Contracts/Mcp/McpParameterDescriptor.cs`]

##### Deferred (pre-existing or out of scope)

- [x] [Review][Defer] `MaxProjectionStatusGroups` rendering and `MaxFieldsPerResource`/`MaxRowsPerResource` use `Math.Max(1, …)` to guard against `0`, masking a missing validator floor — same options-hardening cluster as the prior defer [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `RenderStatusOverviewDocument`/`RenderTableDocument`] — deferred, options-pattern hardening
- [x] [Review][Defer] Empty-state suggestions emit `t.Title` (human, possibly localized) rather than an invocable command identifier or pair (`Title` + `ProtocolName`); agents reading the bullet cannot resolve back to a command without a separate manifest lookup [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs` `BuildSafeSuggestionsAsync`] — deferred, MCP UX/agent-invocability follow-up
- [x] [Review][Defer] `McpResourceDescriptor.BadgeMappings` is `IReadOnlyDictionary<string, string>?` (nullable) on the public Contracts side, but `McpParameterDescriptorModel` defaults to an empty dict in its constructor — same logical descriptor is `null` on hand-constructed paths and non-null on SourceTools-emitted paths [`src/Hexalith.FrontComposer.Contracts/Mcp/McpParameterDescriptor.cs`, `src/Hexalith.FrontComposer.SourceTools/Transforms/McpManifestTransform.cs`] — deferred, Contracts/Model null-vs-empty alignment
- [x] [Review][Defer] `McpManifestEmitter` builds source via raw string concatenation; new positional record params are appended by hand-written `sb.AppendLine` calls with no compile-time check that emission and the Contracts record stay in sync. Adding a future field will fail at compile time of the generated code rather than at the emitter [`src/Hexalith.FrontComposer.SourceTools/Emitters/McpManifestEmitter.cs`] — deferred, source-gen robustness
- [x] [Review][Defer] `McpProjectionRenderResult.Failure(...)` always constructs with `ContentType = "text/plain"` but consumers route through `FrontComposerMcpResult.Failure(category)` which never reads `ContentType`; dead API state suggesting an aborted refactor [`src/Hexalith.FrontComposer.Mcp/Rendering/McpProjectionRenderModels.cs`] — deferred, API tidy
- [x] [Review][Defer] `BuildRenderRequestAsync` reflectively iterates `IEnumerable` results without honoring `cancellationToken`; an upstream deferred query that materializes slowly cannot be cancelled until the renderer phase [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs` `BuildRenderRequestAsync`] — deferred, cancellation completeness
- [x] [Review][Defer] `Render_StatusOverviewProjection_GroupsBySemanticBadgeSlot` uses full-text `ShouldBe` against a hand-rolled string while sibling tests use `ShouldContain`; brittleness will surface on any cosmetic change [`tests/Hexalith.FrontComposer.Mcp.Tests/Rendering/McpMarkdownProjectionRendererTests.cs`] — deferred, test-style alignment
- [x] [Review][Defer] `McpProjectionRenderRequest.Items` is `IReadOnlyList<object>` of boxed values with no defensive copy; concurrent callers can mutate elements mid-render, weakening the atomic-render contract at the request boundary [`src/Hexalith.FrontComposer.Mcp/Rendering/McpProjectionRenderModels.cs`] — deferred, request immutability
- [x] [Review][Defer] `McpResourceDescriptor` and `McpParameterDescriptor` add positional record parameters with defaults; source-compatible but binary-breaking if Contracts is ever published as a stable NuGet package (`IsPackable` not currently set) [`src/Hexalith.FrontComposer.Contracts/Mcp/McpResourceDescriptor.cs`, `src/Hexalith.FrontComposer.Contracts/Mcp/McpParameterDescriptor.cs`] — deferred, revisit before Contracts ships as stable NuGet
- [x] [Review][Defer] `EscapeMarkdownText` escapes `(` and `)` unconditionally, producing noisier-than-necessary output for parenthetical text (`Created (manually)` → `Created \(manually\)`) [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `EscapeMarkdownText`] — deferred, escaping policy review
- [x] [Review][Defer] `BuildRenderRequestAsync` returns `Task<…>` while `BuildSafeSuggestionsAsync` returns `ValueTask<…>` for similar workloads — inconsistent async signatures [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs`] — deferred, code-style alignment
- [x] [Review][Defer] `ReadAsync` outer `catch (OperationCanceledException)` is partially redundant with the renderer's own cancellation→`Canceled` failure mapping; refactor risk if either layer changes [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpProjectionReader.cs`] — deferred, cancellation handling clarity
- [x] [Review][Defer] `Render_FormatterFailure_…` test does not assert sanitization (no logger spy verifying that exception message — which contains `tenant-a` text — is not logged) [`tests/Hexalith.FrontComposer.Mcp.Tests/Rendering/McpMarkdownProjectionRendererTests.cs`] — deferred, telemetry-leak test coverage
- [x] [Review][Defer] `[Flags]` enums route through `value.ToString()` returning `"FlagA, FlagB"`, which never matches a single-key `BadgeMappings` lookup and falls back to a humanized comma-delimited label without per-flag badge slot semantics [`src/Hexalith.FrontComposer.Mcp/Rendering/McpMarkdownProjectionRenderer.cs` `FormatEnum`] — deferred, flag-enum policy follow-up
...
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --filter McpMarkdownProjectionRendererTests` failed before renderer contracts existed, then passed after implementation.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj` passed after moving projection-reader table rendering to the SDK-neutral renderer.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --filter ProjectionDescriptor_CarriesSourceToolsRenderMetadata` failed before SourceTools/MCP manifest metadata was added, then passed.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj` passed with 99 tests after T2 metadata propagation.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --filter McpMarkdownProjectionRendererTests` failed before StatusOverview/Timeline role dispatch, then passed with 5 renderer tests.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj` passed with 102 tests after T3-T5 renderer roles.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --filter McpMarkdownProjectionRendererTests` passed with 9 renderer tests after empty-state suggestion, redaction, cancellation, and formatter-failure atomicity coverage.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj` passed with 108 tests after T6-T8 adapter/safety work.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.SourceTools.Tests\Hexalith.FrontComposer.SourceTools.Tests.csproj` passed with 600 tests after generated manifest metadata changes.
- 2026-05-03: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true` passed with 0 warnings and 0 errors.
- 2026-05-03: `dotnet test Hexalith.FrontComposer.sln --no-build` passed: Contracts 156/0/0, MCP 109/0/0, Shell 1542/0/0, SourceTools 600/0/0, Bench 2/0/0.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --filter McpMarkdownProjectionRendererTests` first failed because production still used string render strategies, then passed with 11/0/0 after typed strategy, truncation metadata, and currency-format updates.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.SourceTools.Tests\Hexalith.FrontComposer.SourceTools.Tests.csproj --filter McpManifestEmitterTests` passed with 1/0/0 after generated manifest enum literal coverage.
- 2026-05-03: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true` initially exposed a generator-load regression from making SourceTools intermediate descriptors depend on Contracts enum runtime types; passed with 0 warnings and 0 errors after keeping SourceTools intermediate render strategy string-based and emitting typed enum literals only.
- 2026-05-03: `dotnet test Hexalith.FrontComposer.sln --no-build` passed: Contracts 156/0/0, MCP 112/0/0, Shell 1542/0/0, SourceTools 601/0/0, Bench 2/0/0.
...
- 2026-05-03: Completed T1 by adding SDK-neutral MCP Markdown render request/result/document models, a bounded atomic Markdown renderer, adapter-edge `text/markdown` mapping, deterministic table/null/enum/badge formatting, and renderer tests for metadata plus inert Markdown escaping.
- 2026-05-03: Completed T2 by carrying SourceTools render strategy, plural labels, empty-state CTA metadata, descriptions, badge mappings, display formats, and stable field order into MCP descriptors and generated manifests.
- 2026-05-03: Completed T3-T5 by rendering Default/ActionQueue Markdown tables, compact StatusOverview summaries, and newest-first Timeline lists with semantic badge text, deterministic ordering, empty states, and explicit truncation bounds.
- 2026-05-03: Completed T6-T8 by preserving the existing query seam, mapping successful renderer output at the MCP resource edge, sourcing empty-state CTA suggestions from the visible catalog, validating new render bounds, redacting secret-looking payloads, and discarding partial render buffers on cancellation or formatter failure.
- 2026-05-03: Completed T9 and final definition-of-done validation. Full build and full solution test suite passed; story moved to review.
- 2026-05-03: Resolved the three remaining review patch items in 8-4 by aligning truncation marker emission with `McpMarkdownProjectionDocument.IsTruncated`, honoring SourceTools `Currency` display metadata for MCP numeric cells, and migrating public MCP resource descriptors to typed render strategies without adding a generator analyzer-load dependency on Contracts runtime types.

### Change Log

- 2026-05-03: Implemented Story 8.4 projection Markdown rendering for agents and moved status to review.
- 2026-05-03: Code review pass via `/bmad-code-review 8-4` applied 18 patches and 1 decision (drop `Suggestions:` label); deferred 5 cross-cutting decisions and 3 patches to Story 8-4a (sanitized taxonomy strings, immutable read snapshot/epoch revalidation, P0 risk-tier tests, atomic-commit completion, truncation-marker/IsTruncated reconciliation, numeric `DisplayFormat` propagation, `RenderStrategy` enum migration). Status moved review → in-progress to reflect deferred AC18/AC9/AC11/AC16/AC17/AC19 work. Validation: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true` (0 warnings/errors); `dotnet test Hexalith.FrontComposer.sln --no-build` => Contracts 156/0/0, MCP 109/0/0, Shell 1542/0/0, SourceTools 600/0/0, Bench 2/0/0.
- 2026-05-03: Completed remaining Story 8.4 review patch follow-ups for truncation marker/metadata parity, numeric `DisplayFormat` currency propagation, and typed MCP render strategy descriptors. Status moved in-progress → review. Validation: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true` (0 warnings/errors); `dotnet test Hexalith.FrontComposer.sln --no-build` => Contracts 156/0/0, MCP 112/0/0, Shell 1542/0/0, SourceTools 601/0/0, Bench 2/0/0.
...
- `_bmad-output/implementation-artifacts/8-4-projection-rendering-for-agents.md`
...
- `src/Hexalith.FrontComposer.Mcp/Rendering/McpProjectionRenderModels.cs`
