...
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --no-restore --filter "FullyQualifiedName~ReadAsync_InProgressPastConfiguredTimeout|FullyQualifiedName~ReadAsync_ActiveCapacityEviction" -v:minimal` passed: 2/0/0.
- 2026-05-03: `dotnet test tests\Hexalith.FrontComposer.Mcp.Tests\Hexalith.FrontComposer.Mcp.Tests.csproj --no-restore -v:minimal` passed: 78/0/0.
- 2026-05-03: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false -v:minimal` passed: 0 warnings, 0 errors.
- 2026-05-03: `dotnet test Hexalith.FrontComposer.sln --no-build -p:UseSharedCompilation=false -v:minimal` passed: Contracts 156/0/0, MCP 78/0/0, Shell 1542/0/0, SourceTools 600/0/0, Bench 2/0/0.
- 2026-05-03: `dotnet build Hexalith.FrontComposer.sln -m:1 -nr:false -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false` passed: 0 warnings, 0 errors. Initial parallel build attempt hit a Windows file lock on `SourceTools.dll` in `obj`, then passed under single-process MSBuild.
- 2026-05-03: `dotnet test Hexalith.FrontComposer.sln --no-build -m:1` passed: SourceTools 600/0/0, Contracts 156/0/0, Shell 1542/0/0, Bench 2/0/0, MCP 80/0/0.
...
- 2026-05-03: Resolved review decision D2 by adding MCP lifecycle timeout and eviction terminalization. In-progress entries now emit sanitized `timed_out` terminal snapshots after `MaxLifecycleInProgressMs`; active-capacity eviction retains a bounded `needs_review` terminal snapshot instead of making an acknowledged handle disappear as hidden/unknown.
- 2026-05-03: Applied D2 follow-up code-review patches (D2.1 + P33-P40). Synthetic terminals carry structured `LIFECYCLE_TIMED_OUT`/`LIFECYCLE_NEEDS_REVIEW` rejection payload satisfying AC6 envelope contract; eliminated timer-after-Dispose race, deferred timer arming until after registration, replaced O(n²) eviction counts with `Interlocked` counters, replaced `IsTerminal` lock with `Volatile.Read` flag, switched timeout test to `FakeTimeProvider`, added monotonicity + idempotent-read regression tests.
- 2026-05-03: Completion validation reran after D2 follow-up edits. All story tasks and review follow-ups are checked, warning-as-error build passes, full solution tests pass, and story status is ready for review.
...
- 2026-05-03: Resolved D2 review follow-up. Added `MaxLifecycleInProgressMs` and `MaxRetainedTerminalLifecycleEntries`, per-entry timeout terminalization to `timed_out`, bounded retained terminal snapshots for capacity eviction as `needs_review`, and MCP regression tests for both paths. Status `in-progress` -> `review`.
- 2026-05-03: D2 follow-up code-review pass via `/bmad-code-review 8-3` applied 1 decision-resolution and 8 patches. **D2.1 (option a):** synthetic terminals now carry structured `McpRejectionPayload` (`LIFECYCLE_TIMED_OUT`/`LIFECYCLE_NEEDS_REVIEW` codes, `retry`/`poll` suggested actions, `HFC-MCP-LIFECYCLE-*` docs codes); `state="Rejected"` preserved. **Concurrency hardening:** P33 timer-after-Dispose guard via `_entryDisposed` flag + `ObjectDisposedException` swallowing on `_timeoutTimer.Change` (3 sites); P34 deferred timer arming via new `LifecycleEntry.Start()` invoked after full registration (constructor uses `Timeout.InfiniteTimeSpan`); P35 lock-free `IsTerminal` via `Volatile.Read(ref _isTerminal)` flag; P36 O(1) `_activeCount`/`_terminalCount` via `Interlocked` mutations under `_capacityGate` (replaces O(n²) per-iteration dictionary scans). **Test hardening:** P37 deterministic timeout test via injected `FakeTimeProvider` (added `Microsoft.Extensions.TimeProvider.Testing` package); P38 new `ReadAsync_RealConfirmedAfterSyntheticTimedOut_TerminalStaysTimedOut` (AC18); P39 new `ReadAsync_RepeatedReadsOfSyntheticTerminal_ReturnIdenticalSnapshots` (AC5/AC18); P40 `CounterUlidFactory` throws on overflow instead of emitting 25-char invalid ULIDs. **Deferred:** AC22 cancellation/disconnect/restart paths (Story 8-3a), AC4 history truncation under synthetic terminal, `MaxLifecycleInProgressMs` infinite sentinel, `Math.Max(1, …)` clamp clarity, eviction-loop ordering refactor. Status `review` -> `in-progress`. Validation: `dotnet build Hexalith.FrontComposer.sln -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false` (0 warnings, 0 errors); `dotnet test Hexalith.FrontComposer.sln --no-build` => Contracts 156/0/0, **MCP 80/0/0** (+2 new), Shell 1542/0/0, SourceTools 600/0/0, Bench 2/0/0.
- 2026-05-03: Completion gate passed after D2 follow-up. Status `in-progress` -> `review`. Validation: `dotnet build Hexalith.FrontComposer.sln -m:1 -nr:false -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false` (0 warnings, 0 errors); `dotnet test Hexalith.FrontComposer.sln --no-build -m:1` => SourceTools 600/0/0, Contracts 156/0/0, Shell 1542/0/0, Bench 2/0/0, MCP 80/0/0.
- 2026-05-03: Round 3 code-review pass via `/bmad-code-review 8-3` applied 4 decisions and 12 patches plus 16 new MCP tests. **Decisions:** D7(b) added missing AC18/AC19/AC21/AC23 test coverage; D8(a) rejection envelope now carries `messageId`/`correlationId`; D9(a) ULID factory throws `InvalidOperationException` on monotonic overflow per spec section 4; D10(c) corrected May-2 ledger P12 wording to align with AC2. **Patches:** P41 NormalizeIdentifier rejects leading/trailing ASCII whitespace; P42 CallToolAsync broadens exception catch around `GetContext()`; P43 MatchesLifecycleSignature pins `parameters[1]` as the generic command parameter; P44 `IdempotentSuccess` caps and sanitizes `Descriptor.Title` (60-char bound + control-char strip); P45 routes lifecycle hidden-unknown through shared `BuildHiddenUnknownStructuredContent`; P46 ReadCoreAsync emits sanitized `LoggerMessage`-source-generated warning before returning DownstreamFailed; P47 `LifecycleUriPrefix` validator rejects userinfo (`user:pass@host`); P48 dispatcher RetryAfter persisted on entry and returned consistently from snapshots; P49 validation failures use `outcome.category="validation"` distinct from domain `"rejected"`; P50 `_byMessage` collision now fail-closes with `UnsupportedSchema`; P51 ULID factory clamps `milliseconds` to `_lastTimestampMs` to defend against clock backward; P52 duplicate-Confirmed re-delivery skips `AppendHistory` so the bounded ring no longer evicts the original Acknowledged frame. Status `review` -> `in-progress` -> `review` (after validation). Validation: `dotnet build Hexalith.FrontComposer.sln -m:1 -nr:false -p:TreatWarningsAsErrors=true -p:UseSharedCompilation=false` (0 warnings, 0 errors); `dotnet test Hexalith.FrontComposer.sln --no-build -m:1 -p:UseSharedCompilation=false` => SourceTools 600/0/0, Contracts 156/0/0, Shell 1542/0/0, Bench 2/0/0, **MCP 96/0/0** (+16 new). **Deferred:** R3-D1/R3-D2 (durable allocation order — Story 8-3a), R3-D3 (per-invocation IUlidFactory DI), R3-D4 (IUlidFactory in Contracts), R3-D5 (_byMessage collision tracking), R3-D6 (OCE→Timeout classification), R3-D7 (DF8.3-D2-5 stale terminals in _insertionOrder), R3-D8 (lifecycle handle entropy / rate-limiting — Story 10-6).
...
- [x] [Review][Decision] D2. **Resolved (option b, 2026-05-03).** MCP lifecycle entries now use a per-entry timeout timer to emit a sanitized `timed_out` terminal snapshot after `MaxLifecycleInProgressMs`, and active-capacity eviction terminalizes the oldest active entry as bounded `needs_review` instead of making an acknowledged handle disappear. The implementation stays inside the MCP package boundary and retains terminal snapshots with `MaxRetainedTerminalLifecycleEntries`; deeper Shell `IPendingCommandStateService` reconciliation remains covered by the D1/full shared-source follow-up.
...
- [x] [Review][Patch] P12. **Resolution corrected (D10(c), 2026-05-03 Round 3).** Original wording "Return the entry's current state at ack time" was misaligned with AC2 and the Proposed Two-Call Flow step 6, which both prescribe `state="Acknowledged"` on the first call regardless of synchronous dispatch progress (the agent uses the lifecycle tracking surface to learn current state). Code at `FrontComposerMcpLifecycleTracker.cs:61-68` correctly hard-codes `Acknowledged` per AC2; no code change required. [`Invocation/FrontComposerMcpLifecycleTracker.cs:49-57`]
...
### Review Findings — D2 Follow-up (2026-05-03)

Generated 2026-05-03 via `/bmad-code-review 8-3`. Three layers ran in parallel against the uncommitted D2 follow-up patch (per-entry timeout terminalization to `timed_out`, bounded retained terminal snapshots for active-capacity eviction as `needs_review`, options `MaxLifecycleInProgressMs` / `MaxRetainedTerminalLifecycleEntries`, two MCP regression tests). Source codes: `blind` = adversarial; `edge` = edge-case; `auditor` = acceptance auditor.

#### Decision-needed

- [x] [Review][Decision] D2.1. **Resolved (option a, 2026-05-03).** `McpTerminalOutcome.TimedOut()` and `NeedsReview()` now construct `McpRejectionPayload` instances with `errorCode="LIFECYCLE_TIMED_OUT"`/`"LIFECYCLE_NEEDS_REVIEW"`, `reasonCategory="timed_out"`/`"needs_review"`, `suggestedAction="retry"`/`"poll"`, `retryAppropriate=true`, `dataImpact="Outcome is unconfirmed at the framework boundary."`, and `docsCode="HFC-MCP-LIFECYCLE-TIMED-OUT"`/`"HFC-MCP-LIFECYCLE-NEEDS-REVIEW"`. Synthetic-terminal envelope now satisfies AC6 structure while keeping `state="Rejected"` (no cross-package enum change). Original analysis: **Synthetic-terminal envelope shape: `state="Rejected"` with no `outcome.rejection` block** [`b+e+a`]. `MarkSyntheticTerminal` sets `_state = CommandLifecycleState.Rejected` for both `TimedOut` and `NeedsReview`; `McpTerminalOutcome.TimedOut()`/`NeedsReview()` carry no `Rejection` payload, so the agent JSON envelope has `state="Rejected"`, `outcome.category="timed_out"|"needs_review"`, `outcome.retryAppropriate=true`, but neither `outcome.rejection` (AC6 fields) nor `outcome.success`. An agent that branches on `state=="Rejected"` will treat synthetic categories as domain rejections and miss the structured remediation hint AC6 implies for that envelope. Three options:
   - **(a)** Add a synthetic remediation payload: `McpTerminalOutcome.TimedOut()`/`NeedsReview()` produce a `McpRejectionPayload` with `errorCode="LIFECYCLE_TIMED_OUT"|"LIFECYCLE_NEEDS_REVIEW"`, `reasonCategory="timed_out"|"needs_review"`, `suggestedAction="retry"|"poll"`, `retryAppropriate=true`, `dataImpact="Outcome is unconfirmed at the framework boundary."`, `docsCode="HFC-MCP-LIFECYCLE-TIMED-OUT"|"HFC-MCP-LIFECYCLE-NEEDS-REVIEW"`. Keeps `state="Rejected"`, fills the AC6 gap.
   - **(b)** Document the contract: agents must read `outcome.category` to disambiguate and treat `timed_out`/`needs_review` as bounded-failure categories distinct from `domain_conflict` rejections; update the spec's Structured Result Shapes to show a synthetic-terminal example. No code change.
   - **(c)** Defer to the broader Story 8-3a `IPendingCommandStateService` reconciliation work — under that story the synthetic kind would be obsolete because real state would surface.

#### Patches (unambiguous fixes)

- [x] [Review][Patch] P33. Timer-after-Dispose race resolved: added `_entryDisposed` flag set atomically in `LifecycleEntry.Dispose()`; `MarkSyntheticTerminal` short-circuits when set; all three `_timeoutTimer.Change(...)` callsites wrapped in `try/catch (ObjectDisposedException)`. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs`]
- [x] [Review][Patch] P34. Timer arming deferred to `LifecycleEntry.Start()`. Constructor creates timer in `Timeout.InfiniteTimeSpan` quiescent state; tracker calls `created.Start()` only after `_byMessage.TryAdd` and `_insertionOrder.Enqueue`. Loser path's `Dispose()` runs before any callback can fire. [`Invocation/FrontComposerMcpLifecycleTracker.cs`]
- [x] [Review][Patch] P35. `IsTerminal` getter replaced with `Volatile.Read(ref _isTerminal) != 0`; `_isTerminal` flag set under `_gate` whenever `_state` transitions to `Confirmed`/`Rejected` in `Observe` or `MarkSyntheticTerminal`. Eliminates `_gate` ↔ `_capacityGate` lock-ordering hazard from count helpers. [`Invocation/FrontComposerMcpLifecycleTracker.cs`]
- [x] [Review][Patch] P36. `CountActiveEntries`/`CountTerminalEntries` removed; replaced with `Interlocked.Increment(ref _activeCount)` (in `GetOrCreateEntry`), `Interlocked.Decrement(ref _activeCount); Interlocked.Increment(ref _terminalCount)` (in `OnEntryTerminalized` under `_capacityGate`), and `Interlocked.Decrement(ref _terminalCount)` (in `EnforceTerminalRetention`). Eviction loops now use `Volatile.Read` of counters. O(1) per check. [`Invocation/FrontComposerMcpLifecycleTracker.cs`]
- [x] [Review][Patch] P37. Timeout test now uses `FakeTimeProvider` injected via the new `timeProvider` parameter on `Build`; `fakeTime.Advance(TimeSpan.FromMilliseconds(50))` deterministically fires the 10ms timer. No wall-clock dependency. Added `Microsoft.Extensions.TimeProvider.Testing` package reference. [`tests/Hexalith.FrontComposer.Mcp.Tests/Invocation/CommandLifecycleTests.cs`]
- [x] [Review][Patch] P38. Added `ReadAsync_RealConfirmedAfterSyntheticTimedOut_TerminalStaysTimedOut` — fires `MarkTimedOut` via fake time advance, then drives a real `Confirmed` transition through `ILifecycleStateService`, asserts `state="Rejected"`, `outcome.category="timed_out"`, and exactly one terminal row in history. [`tests/.../CommandLifecycleTests.cs`]
- [x] [Review][Patch] P39. Added `ReadAsync_RepeatedReadsOfSyntheticTerminal_ReturnIdenticalSnapshots` — calls `ReadAsync` twice after timeout and asserts byte-equal JSON snapshots. [`tests/.../CommandLifecycleTests.cs`]
- [x] [Review][Patch] P40. `CounterUlidFactory` default branch now throws `InvalidOperationException` instead of silently emitting 25-char invalid ULIDs. [`tests/.../CommandLifecycleTests.cs`]

#### Deferred (pre-existing, scoped, or paired with a follow-up story)

- [x] [Review][Defer] AC22 — cancellation/disconnect/process-restart paths beyond the configured-timeout case. The patch addresses two of the four AC22 trigger conditions (configured timeout via per-entry timer, capacity eviction). Cancellation-mid-dispatch, transport-disconnect, and adapter-exception-after-allocation reconciliation remain coupled to the deferred Shell `IPendingCommandStateService` recovery work tracked in the Story 8-3a follow-up.
- [x] [Review][Defer] AC4 — bounded-history truncation behavior under synthetic terminal append at history cap. The synthetic terminal calls `AppendHistory` which honors `_maxHistory`, but no test pins the truncation flag/order under the new path. Pair with the broader bounded-state tests in T8 line 139.
- [x] [Review][Defer] `MaxLifecycleInProgressMs` typed as `int` with no "infinite" sentinel; validator allows `int.MaxValue` (~24.8 days). Acceptable in practice given the 30 s default. Re-typing to `TimeSpan?` with `null` = disabled is a follow-up options API change.
- [x] [Review][Defer] `Math.Max(1, opts.MaxLifecycleInProgressMs)` belt-and-suspenders clamp in `GetOrCreateEntry` masks any future validator regression but is not a current correctness issue. Track as a clarity/hardening note alongside an `Monitor.IsEntered(_capacityGate)` assert in `EnforceTerminalRetention`.
- [x] [Review][Defer] `EnforceCapacity` drains `_insertionOrder` past already-terminal entries, doing wasted no-op work on each. Algorithm is correct (the bound `MaxRetainedTerminalLifecycleEntries=1000` enforces the active+retained ceiling), but a small refactor — re-enqueue active entries that survive the eviction loop or maintain a dedicated active-only queue — would tighten ordering invariants.

---

### Review Findings — Round 3 (2026-05-03)

Generated 2026-05-03 via `/bmad-code-review 8-3` after the D2 follow-up landed. Three layers ran in parallel against the diff `d5f93ca..HEAD` (10 files, 1,683 insertions, 38 deletions). Source codes: `blind` = adversarial; `edge` = edge-case; `auditor` = acceptance auditor.

This round did NOT re-litigate items already marked `[x]` resolved or `[x]` deferred in the May-2 / D2-follow-up ledgers above. Round-3 findings are either (a) spec/code drift discovered when verifying prior `[x]` claims, or (b) genuinely new patches/decisions surfaced by the parallel review layers.

#### Decision-needed

- [x] [Review][Decision] D7. **Resolved (option b, 2026-05-03 Round 3).** Added 16 new MCP tests in `tests/Hexalith.FrontComposer.Mcp.Tests/Invocation/CommandLifecycleTests.cs` covering AC23 malformed-handle expansion (oversized 1024-char, percent-encoded, near-match 24/25-char, leading/trailing whitespace, escaped tab, both-keys-present), AC18 parallel-read concurrency (16 concurrent readers vs single dispatch), AC21 out-of-order observer determinism, AC19 replay matrix (after-Rejected, IdempotentConfirmed re-delivery), P52 history non-growth, D8 envelope correlation, P49 validation category, P48 dispatcher RetryAfter carry-through. T8 lines 131/135/136/138/141 now have direct test coverage. **Original analysis follows.** **Several May-2 ledger items marked `[x]` resolved are only partially closed in code; the test work was actually deferred to Story 8-3a.** P4 (parallel reads / parallel duplicate Confirmed observations / out-of-order observation), P5 (replay/idempotency matrix), P17 (oversized / percent-encoded / near-match / whitespace-padded / both-keys malformed-handle InlineData expansion), and P18 (out-of-order observer determinism + parallel-read determinism) all kept their `[x]` markings because limited single-threaded tests were added and the rest was tracked under Story 8-3a in the follow-up reservation paragraph. T8 lines 131/135/136/138/141/144 remain `[x]` despite tests being absent. Decision options:
  - **(a)** Re-mark P4/P5/P17/P18 and the affected T8 entries as `[~]` partial in the ledgers above, with explicit `(deferred to Story 8-3a)` annotations. No test work in this round.
  - **(b)** Add the missing tests now (~10–15 new tests across replay matrix, parallel-read concurrency, out-of-order delivery, malformed-handle expansion). Closes the AC18/AC19/AC21/AC23/AC24 gaps inside Story 8-3.
  - **(c)** Open Story 8-3a as a hard prerequisite and gate Story 8-3 `done` on its completion.
  Sources: blind+edge+auditor.

- [x] [Review][Decision] D8. **Resolved (option a, 2026-05-03 Round 3).** `BuildRejectionPayload` now accepts `messageId`/`correlationId` parameters and emits them on the rejection envelope alongside `state="Rejected"`. `ApplyDerivableValues` returns the allocated IDs to the caller for capture across the catch boundary. New test `InvokeAsync_CommandRejected_RejectionEnvelopeIncludesMessageAndCorrelationId` pins the AC6 correlation contract. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpCommandInvoker.cs:34, 291-307, 392-447`] **Original analysis follows.** **Synchronous-rejection envelope omits `messageId`/`correlationId`** despite IDs being allocated up-front in `ApplyDerivableValues` [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpCommandInvoker.cs:291-295, 305-328`]. The May-2 P29 was marked `[x]` resolved with "agent cannot correlate the rejection with its dispatched handle", but `BuildRejectionPayload` does not actually emit either ID. Decision options:
  - **(a)** Add `messageId`/`correlationId` to the rejection envelope alongside `state="Rejected"`. Mechanical; closes the AC6 correlation gap and matches the lifecycle-snapshot envelope.
  - **(b)** Document the contract: synchronous rejection arrives without correlation; agents must consume the rejection from the same call/response pair as the command submission and never poll for it.
  - **(c)** Defer to telemetry hardening, accepting AC6 spec/code drift in v1.
  Sources: auditor+blind.

- [x] [Review][Decision] D9. **Resolved (option a, 2026-05-03 Round 3).** `FrontComposerMcpUlidFactory.NewUlid` now throws `InvalidOperationException("ULID monotonic overflow within the same millisecond.")` when `_lastRandomLo` wraps to 0 AND `_lastRandomHi == 0xFFFFUL`, per canonical ULID spec section 4. [`src/Hexalith.FrontComposer.Mcp/FrontComposerMcpUlidFactory.cs:24-49`] **Original analysis follows.** **ULID factory monotonic overflow handling diverges from canonical ULID spec section 4** [`src/Hexalith.FrontComposer.Mcp/FrontComposerMcpUlidFactory.cs:24-29`]. When same-millisecond increments roll the 80-bit random component AND overflow the 16-bit `_lastRandomHi`, the code masks `_lastRandomHi & 0xFFFFUL`, silently wrapping to zero. The May-2 D5 resolution explicitly cited spec section 4, which prescribes throwing on overflow. Decision options:
  - **(a)** Throw `InvalidOperationException("ULID monotonic overflow within same millisecond")` per canonical spec.
  - **(b)** Spin-sleep until the next millisecond and reseed the random component (preserves call-site contract; minor latency cost).
  - **(c)** Accept silent wrap (current); document the spec deviation in the file header.
  Sources: blind+edge.

- [x] [Review][Decision] D10. **Resolved (option c, 2026-05-03 Round 3).** Updated the May-2 ledger P12 entry above to clarify that AC2 mandates `state="Acknowledged"` on the first call regardless of synchronous dispatch progress (the agent uses the lifecycle tracking surface to learn current state). Code at `FrontComposerMcpLifecycleTracker.cs:61-68` is correct as written; the original P12 wording was misaligned with AC2 and the Proposed Two-Call Flow step 6. **Original analysis follows.** **Acknowledgement state contradiction with ledger P12** [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:61-68`]. Code hard-codes `state = Acknowledged` and the inline comment cites AC2. The May-2 P12 was marked `[x]` resolved as "Return the entry's current state at ack time" — the implementation does not. AC2 wording matches the code; the spec ledger contradicts it. Decision options:
  - **(a)** Fix code to return entry's current state when the dispatcher pushed Confirmed/Rejected synchronously (matches P12 ledger; potential AC2 wording clash).
  - **(b)** Accept current code; re-mark P12 as `[~]` partial — "AC2 wording took precedence; current-state surface deferred".
  - **(c)** Update spec to remove the contradiction and clarify AC2 trumps any optimistic state surfacing.
  Sources: auditor.

#### Patches (unambiguous fixes)

- [x] [Review][Patch] P41. `NormalizeIdentifier` accepts leading/trailing ASCII whitespace before `Trim()` — two distinct caller inputs (`" 01J..."` vs `"01J..."`) collapse onto the same canonical handle. Reject when `value != value.Trim()` before normalization. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:144-156`]
- [x] [Review][Patch] P42. `CallToolAsync` only catches `FrontComposerMcpException` from `GetContext()` — non-FrontComposerMcpException (`InvalidOperationException`, `ArgumentException`) bubbles unhandled and may leak stack trace through the MCP transport. Broaden to a generic `catch (Exception)` that returns the unauthorized/unsupported failure category. [`src/Hexalith.FrontComposer.Mcp/Extensions/FrontComposerMcpServiceCollectionExtensions.cs:107-112`]
- [x] [Review][Patch] P43. `MatchesLifecycleSignature` does not check `parameters[1]` type — a future 4-arg `DispatchAsync<TCommand>` overload whose second parameter is not the command (e.g. an additional logger/context slot) will silently match. Add an explicit assertion that `parameters[1]` is the generic method type parameter. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpCommandInvoker.cs:148-156`]
- [x] [Review][Patch] P44. `IdempotentSuccess(entityLabel)` interpolates `Descriptor.Title` directly into agent-visible message without length bound or control-character stripping — a malformed/oversized descriptor title flows into the snapshot JSON. Cap to ~60 chars and strip control chars. [`src/Hexalith.FrontComposer.Mcp/Invocation/McpLifecycleModels.cs:106-113`]
- [x] [Review][Patch] P45. `HiddenUnknown` payload is hand-rolled inside the lifecycle tracker rather than routed through `FrontComposerMcpToolAdmissionService.BuildUnknownToolStructuredContent`. AC9 mandates "indistinguishable external response shape" with Story 8-2 unknown semantics. Any future change to the Story 8-2 payload (size, ordering, suggestion behavior) creates an externally observable divergence. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:259-267`]
- [x] [Review][Patch] P46. Generic `catch { return Failure(DownstreamFailed) }` in `ReadCoreAsync` swallows every exception with no logging, no telemetry, no rethrow of fatal categories — `NullReferenceException` from a bug in `ToSnapshot` becomes "DownstreamFailed", indistinguishable from a transient external failure. AC13 requires sanitized failure-category telemetry. Replace bare catch or emit an `ILogger` warning with sanitized correlation/category before returning. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:114-116`]
- [x] [Review][Patch] P47. `LifecycleUriPrefix` validator now accepts absolute URIs without checking userinfo. A configured `https://user:pass@host/` would emit credentials inside lifecycle response strings. Add `string.IsNullOrEmpty(prefixUri.UserInfo)` to the validator condition. [`src/Hexalith.FrontComposer.Mcp/Extensions/FrontComposerMcpServiceCollectionExtensions.cs:187-193`]
- [x] [Review][Patch] P48. `ToSnapshot` ignores `result.RetryAfter` (the dispatcher-supplied hint persisted on the acknowledgement) and always emits `Math.Clamp(opts.DefaultLifecycleRetryAfterMs, ...)`. The acknowledgement and every subsequent snapshot for the same command therefore disagree on retry guidance. Persist the dispatcher's `RetryAfter` on the entry and surface it from `ToSnapshot`, OR fix the acknowledgement to also use `DefaultLifecycleRetryAfterMs` (the latter is consistent with AC24 retry-guidance discipline). [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:488-490` vs `:67-68`]
- [x] [Review][Patch] P49. `CommandValidationException` falls into `BuildRejectionPayload` with `outcome.category="rejected"` (the `BuildRejectionPayload` helper hardcodes the category), conflating protocol-layer validation failure with domain-layer rejection at the envelope-level discriminator agents would branch on. The structured fields (`errorCode`, `reasonCategory="validation"`) carry the right info, but the outer category is wrong. Either parameterize the category in `BuildRejectionPayload` or use `"validation"` for the validation path. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpCommandInvoker.cs:91-101, 305-328`]
- [x] [Review][Patch] P50. `_byMessage.TryAdd(messageId, created)` silently drops on collision — a same-message-ID second registration is unreachable by `messageId` lookup forever. Either fail-closed at registration (throw/return `UnsupportedSchema`) or log a misconfiguration warning. The current `// TryAdd so a message-id collision does not silently overwrite` comment misses that `TryAdd` failure is itself silent. [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:194`]
- [x] [Review][Patch] P51. ULID factory does not defend against clock-backward (NTP step / VM migration / container clock skew). The `else` branch unconditionally writes `_lastTimestampMs = milliseconds` even when `milliseconds < _lastTimestampMs`, breaking the lexicographic monotonicity guarantee the lifecycle tracker depends on for sequence ordering. Clamp via `milliseconds = Math.Max(milliseconds, _lastTimestampMs)` before the branch. [`src/Hexalith.FrontComposer.Mcp/FrontComposerMcpUlidFactory.cs:17, 30-31`]
- [x] [Review][Patch] P52. Duplicate Confirmed observations (idempotent re-delivery) fall through `Observe` and call `AppendHistory` for an extra row each time. The `currentTerminal && transition.NewState != _state` guard does not catch same-state re-delivery, and the truncation loop will eventually evict the original Acknowledged frame from the agent-visible history. Skip `AppendHistory` when `currentTerminal && transition.NewState == _state` (still allow the IdempotencyResolved upgrade in `_outcome`). [`src/Hexalith.FrontComposer.Mcp/Invocation/FrontComposerMcpLifecycleTracker.cs:376-411, 461-477`]

#### Deferred (already covered by prior ledgers, follow-up stories, or scope guards)

- [x] [Review][Defer] R3-D1 — AC17/D11 durable shared lifecycle allocation precedes acknowledgment is still violated (order is `dispatch → register`). Already deferred under May-2 D1 to Story 8-3a (`[~]` partially resolved with "register-before-dispatch deferred"). T2 task checkboxes remain inconsistent with this gap; flag as part of D7 spec-hygiene resolution.
- [x] [Review][Defer] R3-D2 — `TrackAcknowledged` throwing after `DispatchAsync` succeeded leaves an acknowledged command untrackable. Same root cause as R3-D1; resolved by Story 8-3a register-before-dispatch reshape.
- [x] [Review][Defer] R3-D3 — Per-invocation `services.GetService<IUlidFactory>()` DI lookup in `ApplyDerivableValues` (CommandInvoker:291) is on the hot path. Perf concern, not correctness; v1.x perf sweep.
- [x] [Review][Defer] R3-D4 — `IUlidFactory` lives in `Hexalith.FrontComposer.Contracts` and is registered by `AddFrontComposerMcp`. Architectural coupling concern; only re-shape if Contracts surface widens further. Cross-Story Contract: Story 8-1.
- [x] [Review][Defer] R3-D5 — `_byMessage.TryAdd` collision (P50) and `_byMessage.TryRemove` mistargeting risk are mitigated by the framework-issued ULID factory's collision resistance under correct config. Ladder to fail-closed only if external messageId injection becomes a supported flow.
- [x] [Review][Defer] R3-D6 — `OperationCanceledException` re-mapping to `Timeout` in `InvokeAsync` (CommandInvoker:102-106) is potentially over-eager for non-timer OCE causes (link-CTS, `HttpClient` network drop). Behavior preserved from prior reviews; deferral pending end-to-end OCE classification audit.
- [x] [Review][Defer] R3-D7 — `_insertionOrder` accumulates stale terminal correlationIds (already DF8.3-D2-5 in deferred-work.md).
- [x] [Review][Defer] R3-D8 — Lifecycle handle entropy / rate-limiting / cross-agent same-tenant enumeration concerns. Capacity-scoped enumeration is bounded by `MaxActiveLifecycleEntries`; per-handle revalidation runs in `ReadAsync`; deeper rate-limiting paired with Story 10-6 timing-oracle hardening.

#### Dismissed (false positives / handled / noise)

- Phantom missing `using` for `CommandValidationException` — type lives in `Hexalith.FrontComposer.Contracts.Lifecycle`, already imported.
- `RecordingLifecycleStateService` test fake uses `Dictionary` without locks — single-threaded by test design; production uses `ConcurrentDictionary`.
- JSON Schema `oneOf` permits both keys present — under `additionalProperties: false`, both `required` blocks matching means BOTH subschemas match → `oneOf` correctly fails. Runtime `arguments.Count != 1` is consistent with schema.
- `Math.Max(1, ...)` belt-and-suspenders defenses already deferred (DF8.3-D2-4).
- `IdempotencyResolved=true` re-delivery to already-IdempotentSuccess outcome — agent itself describes as "harmless"; idempotent.
