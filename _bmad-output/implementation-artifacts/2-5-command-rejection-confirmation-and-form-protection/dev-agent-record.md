...
| 2026-04-17 | **Review findings closed — AC authority drift reconciled (doc-only).** Addressed the two open `[Review][Decision]` items in tasks-subtasks.md by aligning AC prose with the binding critical decisions: **AC8** now ratifies HFC1020 Info severity + expanded verb regex per **D20** (Warning promotion + `[SuppressHFC1020]` escape hatch forward-referenced to Story 9-4; TreatWarningsAsErrors does not fail on Info in v0.1). **AC6** narrowed to Submitting-only suppression per **D13** (Syncing with ActionPrompt FIRES; `WrapperInitiatedNavigation` cascade suppresses wrapper Start-over). Zero code / test changes — code and descriptors were already authoritative. Regression re-run: 0 warnings / 0 errors under `-warnaserror`; **553 tests passing / 2 skipped** (pre-existing E2E latency). Story Status: in-progress → review. | bmad-dev-story |
