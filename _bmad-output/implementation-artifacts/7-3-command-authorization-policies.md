...
### Review Findings — Pass 4 (2026-05-02)

Re-run via `/bmad-code-review 7-3` on the dedicated follow-up dev session (uncommitted changes vs HEAD on `main`; 1214 lines, 14 modified + 6 new). Three adversarial layers (Acceptance Auditor, Blind Hunter, Edge Case Hunter via `general-purpose`) raised **174 raw findings** (Blind 75, Edge 71→65 after self-demotions, Auditor 28 + AC matrix). After dedup against Pass 1 + Pass 2 + Pass 3 ledgers and triage: **7 decision-needed, 51 patches, 10 defers, ~38 dismissed**. The strongest cross-source signal: the dedicated dev session under-delivered against the sprint-status entry — DN4 was not implemented at all, DN3 only gates Inline mode, DN5 reintroduces resource-shape divergence (the very defect it was supposed to eliminate), and both new components (`FcAuthorizedCommandRegion` + renderer-emitted refresh) ship without any of the Pass-3 hardening discipline (cts capture, sequence guard, AuthState subscribe, _disposed guard, IDisposable, StateHasChanged).

#### Decision-Needed (Pass 4)

- [ ] [Review][Decision] DN-7-3-4-1: DN4 (Capability/Home gating) was never implemented, despite Pass-3 `[x]` mark and Pass-4 sprint-status claim — `CapabilityDiscoveryEffects.cs` and `FcHomeDirectory.razor.cs` have zero diff hunks and zero `Authoriz|Policy|Evaluate` references. Spec AC13 requires Home/Capability surface gating. Pass-3 P-E29 (`CapabilityDiscoveryEffects warns once-per-session if ICommandAuthorizationEvaluator is missing`) was also marked `[x]` but unimplemented. Decide: (a) land DN4 + P-E29 now in this session (mirrors `CommandPaletteEffects.CanSurfaceCommandAsync`; ~150 LoC + 4-6 tests + `SourceSurface.HomeCapability` wiring), (b) re-defer to a Pass 5 dedicated session and revert the false `[x]` marks + tighten sprint-status wording, (c) accept current scope and document Home/Capability as a known gap with explicit risk acknowledgement.
- [ ] [Review][Decision] DN-7-3-4-2: AA-04 — Direct-dispatch authorization landed as **per-impl injection** (optional ctor param on `StubCommandService` and `EventStoreCommandClient`) rather than the binding-decision **`AuthorizingCommandServiceDecorator`** wrapping `ICommandService`. Adopters with custom `ICommandService` impls (different transport, MCP bridge, test fixtures) bypass the gate entirely. Decide: (a) refactor to a decorator pattern matching DN2 binding decision (preserves the per-impl gate as defense-in-depth or removes it), (b) update the spec/Dev Notes/DN2 resolution to reflect the per-impl-injection design and document the contract for adopters who introduce new `ICommandService` impls, (c) keep both: per-impl injection AND a decorator (max defense-in-depth, double-evaluation cost — see EH-74).
- [ ] [Review][Decision] DN-7-3-4-3: AA-05 — `EventStoreCommandClient` and `StubCommandService` accept an optional `ICommandDispatchAuthorizationGate? authorizationGate = null` and fail OPEN when the gate is missing (`if (authorizationGate is not null)`). Spec D4 mandates fail-closed for protected commands when auth services are missing. Existing test factories (`EventStoreCommandClassifierIntegrationTests.NewClient`) already construct without the gate. Decide: (a) make the gate a required ctor param (no `?`, no `= null`) so DI throws `InvalidOperationException` at resolution if missing — guarantees fail-closed but breaks existing test factories, (b) keep optional but check `DomainManifest.CommandPolicies` directly inside the dispatch impl when gate is null and throw `Forbidden` for any protected command (gate becomes a perf optimization), (c) accept fail-open and document the DI requirement in adopter onboarding (no spec realignment needed if D4 is narrowed to "when policy + gate are both present").
- [ ] [Review][Decision] DN-7-3-4-4: AA-08 — `FcAuthorizedCommandRegion` consumer (`FcProjectionEmptyPlaceholder.razor`) does NOT pass `BoundedContext` to the region, so the evaluator resource has `BoundedContext=null` for the CTA path while form/palette/dispatch surfaces supply it. **This is exactly the resource-shape divergence DN5 was supposed to eliminate.** Authorization handlers that branch on `resource.BoundedContext` will accept on form path and reject on CTA path. Decide: (a) extend `EmptyStateCta` record with `BoundedContext` field populated from the resolver's manifest source and thread through the region (small structural change; resolver already knows the BC), (b) have `FcAuthorizedCommandRegion` resolve BC internally via `IFrontComposerCommandPolicyRegistry` extension `TryGetCommandPolicy(out policy, out boundedContext)` — also resolves AA-23 (gate's BC drift), (c) accept partial unification, document the gap, and commit to a Pass-5 fix.
- [ ] [Review][Decision] DN-7-3-4-5: AA-06 — `CommandDispatchAuthorizationGate.EnsureAuthorizedAsync` collapses `Pending` and `Canceled` decisions into `CommandWarningKind.Forbidden`. `CommandWarningKind.Pending` exists explicitly for transient retryable states; spec line 516 says "Cancellation before an authorization decision is treated as non-executable and side-effect-free; it may surface a neutral retryable status rather than a permission-denied message." Direct-dispatch users hit during prerender / fast-cancel see "permission denied" instead of a transient/cancellable status. Decide: (a) branch on `decision.Kind` — Pending → throw `CommandWarningException(Pending, ...)`, Canceled → throw `OperationCanceledException`, only Denied/FailedClosed-other → Forbidden, (b) keep current Forbidden coercion and document, (c) add a retry hint in the Forbidden copy specifically for Pending so user sees "Action temporarily unavailable — please retry" mirroring the form's dual-copy patch.
- [ ] [Review][Decision] DN-7-3-4-6: AA-10 + AA-12 — DN3 only emits the `Disabled` trigger gate for **Inline** mode. CompactInline (line 567+ of emitter) and FullPage (line 593+) emit no trigger-level gating. The renderer-emitter test only verifies `InlineAction` surface and the inline disabled binding. The spec-mandated `tests/.../Generated/*Authorization*` bUnit suite is also still missing (Pass-3 P3 deferred awaiting DN3 — DN3 is now partial, suite still uncreated). Decide: (a) extend `Disabled` binding emission to CompactInline + FullPage modes now AND create the Generated/Authorization bUnit suite covering all five surfaces (inline / compact / full-page / CTA / palette + home if DN-7-3-4-1 (a)) — substantial work (~200 LoC emitter + 7-9 verified.txt rebaselines + 5-7 bUnit fixtures), (b) extend gate emission to all modes but defer the bUnit suite again (keeps the trigger-gating coverage gap), (c) defer both to Pass 5 and explicitly document FullPage as form-level-only protection.
- [ ] [Review][Decision] DN-7-3-4-7: AA-02 + AA-03 + AA-13 + AA-14 — Both new code paths (`FcAuthorizedCommandRegion` and emitted `RefreshPresentationAuthorizationAsync` in `CommandRendererEmitter`) ship **without any** of the Pass-3 hardening discipline that the form emitter received: no IDisposable / Dispose, no `_cts` capture-and-pass, no `_disposed` short-circuit after await, no sequence-number guard against concurrent refresh, no `AuthenticationStateChanged` subscription, no try/catch around evaluator, no synchronous `_authorizationPresentationReady = false` before await, no single-correlation-ID per refresh. This is ~12 hardening patches per code path. Decide: (a) apply the full hardening matrix to BOTH paths in this session (~24 sub-patches, mirrors the Pass-3 form-emitter wave), (b) defer to Pass 5 and ship Pass 4 with the disposal/concurrency/staleness hazards documented, (c) apply minimal subset (Dispose + AuthState subscribe in CTA region; _disposed + cts in renderer) and defer the rest.

#### Patches (Pass 4 net-new)

CTA region (`FcAuthorizedCommandRegion`) — issued contingent on DN-7-3-4-7 not being deferred:

- [ ] [Review][Patch] Initialize `_decision = Pending(...)` at field declaration so first synchronous render shows Pending, not the catch-all `else` (which is `@NotAuthorized` ~ usually null) [src/Hexalith.FrontComposer.Shell/Components/Rendering/FcAuthorizedCommandRegion.razor:4-15, FcAuthorizedCommandRegion.razor.cs:28] — BH-01 / EH-26.
- [ ] [Review][Patch] Add IDisposable + Dispose pattern; capture CTS; pass `_cts.Token` to `Evaluator.EvaluateAsync` [FcAuthorizedCommandRegion.razor.cs:33-62] — AA-13 / EH-03 / EH-05 / EH-47 / BH-06.
- [ ] [Review][Patch] Add `_disposed` short-circuit after `await Evaluator.EvaluateAsync` [FcAuthorizedCommandRegion.razor.cs:54-60] — AA-13 / BH-05 / EH-29.
- [ ] [Review][Patch] Add sequence-number guard against concurrent `OnParametersSetAsync` (`var captured = ++_sequence; ... if (captured != _sequence) return;`) [FcAuthorizedCommandRegion.razor.cs:33-62] — AA-14 / BH-05 / BH-69 / EH-06.
- [ ] [Review][Patch] Inject `AuthenticationStateProvider`; subscribe to `AuthenticationStateChanged`; re-evaluate on transitions; unsubscribe in Dispose [FcAuthorizedCommandRegion.razor.cs] — AA-02 / EH-02 / EH-60.
- [ ] [Review][Patch] Wrap `Evaluator.EvaluateAsync(...)` in try/catch — null evaluator, null result, or thrown handler must produce `Blocked(MissingService)` / `Blocked(HandlerFailed)` rather than escape to Blazor error boundary [FcAuthorizedCommandRegion.razor.cs:54-60] — BH-03 / BH-62.
- [ ] [Review][Patch] Set `_decision = Pending(...)` synchronously then call `await InvokeAsync(StateHasChanged)` BEFORE the evaluator await so the Pending fragment actually renders [FcAuthorizedCommandRegion.razor.cs:52-55] — BH-02 / BH-04 / EH-06.
- [ ] [Review][Patch] Allocate ONE correlation ID per refresh cycle and reuse for both Pending stand-in and final decision [FcAuthorizedCommandRegion.razor.cs:37,52] — BH-08 / EH-27 / AA-18 / EH-70.
- [ ] [Review][Patch] Pass `BoundedContext` from `FcProjectionEmptyPlaceholder.razor` to `FcAuthorizedCommandRegion` (resolves DN-7-3-4-4 if option (a) chosen) [FcProjectionEmptyPlaceholder.razor:19-21] — AA-08.
- [ ] [Review][Patch] Update `_lastCommandType`/`_lastPolicyName`/`_lastBoundedContext` cache fields on the early-return MissingPolicy branch so subsequent identical-parameter calls don't re-allocate correlation IDs [FcAuthorizedCommandRegion.razor.cs:33-39] — BH-07 / AA-20.
- [ ] [Review][Patch] Move `ProtectedCreateOrderCommand` test type into a namespace (currently file-scope public — pollutes test assembly's public surface) [tests/Hexalith.FrontComposer.Shell.Tests/Components/Rendering/FcProjectionEmptyPlaceholderTests.cs:984-985] — BH-29 / EH-52.
- [ ] [Review][Patch] Revert `CreateOrderCommand` visibility from `public` to `private` (no apparent need for widening) [FcProjectionEmptyPlaceholderTests.cs:894] — BH-30.
- [ ] [Review][Patch] Make `IWriteAwarePolicyRegistry` test interface `internal` (currently public — pollutes test assembly API) [tests/Hexalith.FrontComposer.Shell.Tests/Services/EmptyStateCtaResolverTests.cs:1087] — BH-65.
- [ ] [Review][Patch] Log Information when `ResolveCommandType(cta.CommandFqn)` returns null (silent CTA suppression — operators cannot distinguish from authorization deny) [src/Hexalith.FrontComposer.Shell/Components/Rendering/FcProjectionEmptyPlaceholder.razor.cs:114-125] — AA-09 / BH-26 / BH-27 / EH-08 / EH-45 / EH-59.
- [ ] [Review][Patch] Drop `manifests` parameter from `EmptyStateCtaResolver.ResolveCommandPolicy` instance method — always use `_registry` to eliminate snapshot-vs-live divergence [src/Hexalith.FrontComposer.Shell/Services/EmptyStateCtaResolver.cs:721-725] — BH-23 / BH-59 / EH-24.
- [ ] [Review][Patch] Trim policy values defensively at `EmptyStateCtaResolver` consumption site (custom `IFrontComposerCommandPolicyRegistry` impls may not trim) [EmptyStateCtaResolver.cs:722-725] — BH-24 / EH-25 / EH-61.

Direct-dispatch gate (`CommandDispatchAuthorizationGate`) hardening:

- [ ] [Review][Patch] Localize gate copy via `IStringLocalizer<FcShellResources>` — reuse Pass-3 `AuthorizationActionUnavailableTitle/Message` keys; add `LocalizedString.ResourceNotFound` fallback per Pass-3 P18 [src/Hexalith.FrontComposer.Shell/Services/Authorization/CommandDispatchAuthorizationGate.cs:70-79] — AA-07 / EH-15 / BH-17.
- [ ] [Review][Patch] Snapshot `_manifests` to a local list (or take a lock) before enumeration in `TryResolvePolicy` and `ResolveBoundedContext` — `InvalidOperationException: Collection was modified` if `RegisterDomain` runs concurrently [CommandDispatchAuthorizationGate.cs:91-110, FrontComposerRegistry.cs:114-128] — BH-10.
- [ ] [Review][Patch] Catch `ObjectDisposedException` (and other corrupted-state-safe exception families) in the GetService scope-resolution path — Blazor circuit teardown can dispose the scope mid-dispatch; today only `InvalidOperationException` is caught [CommandDispatchAuthorizationGate.cs:28-38] — BH-16 / EH-19.
- [ ] [Review][Patch] Add `cancellationToken.ThrowIfCancellationRequested()` at the entry of `EnsureAuthorizedAsync` (currently unprotected commands skip cancellation entirely) [CommandDispatchAuthorizationGate.cs:18-25] — BH-12 / EH-18.
- [ ] [Review][Patch] Trim `commandTypeName` once at the top of `EnsureAuthorizedAsync` so canonical and fallback lookups agree (today fast path trims internally via registry, fallback `manifest.CommandPolicies.TryGetValue(commandTypeName, ...)` does NOT trim → fail-OPEN for whitespace-padded keys) [CommandDispatchAuthorizationGate.cs:18-100] — BH-14 / EH-12.
- [ ] [Review][Patch] Constructor-inject `ICommandAuthorizationEvaluator` directly instead of capturing `IServiceProvider` (re-introduces the service-locator antipattern that Pass-2 P30/B9 explicitly removed from the evaluator) [CommandDispatchAuthorizationGate.cs:577-602] — BH-15.
- [ ] [Review][Patch] Include `PolicyName` in the deny `LogWarning` (today log only contains `Reason`, `CommandType`, `CorrelationId` — operators cannot correlate dispatch deny with the registered policy) [CommandDispatchAuthorizationGate.cs:61-65] — EH-21.
- [ ] [Review][Patch] Reject command types with `FullName == null` (open generics, dynamic assemblies) with explicit Blocked decision — currently falls back to `commandType.Name`, lookup misses → fail OPEN [CommandDispatchAuthorizationGate.cs:586-587] — BH-60.
- [ ] [Review][Patch] Use `StringComparison.Ordinal` (not `OrdinalIgnoreCase`) when stripping the `"Command"` suffix in `DisplayLabel` — case-insensitive match incorrectly strips suffixes from types like `MyCOMMAND` [CommandDispatchAuthorizationGate.cs:676-681] — BH-19.
- [ ] [Review][Patch] Hide `EntityLabel` (or use a generic label) for unauthenticated/denied users — leaks command type name to client, helps attackers enumerate the policy graph [CommandDispatchAuthorizationGate.cs:634-643] — BH-18.
- [ ] [Review][Patch] Guard `DisplayLabel` against types literally named `"Command"` (returns empty string today; downstream `format(EntityLabel)` in aria-labels can produce broken UI) [CommandDispatchAuthorizationGate.cs:676-681] — EH-17.
- [ ] [Review][Patch] Use `Type.FullName` (not `Type.Name`) inside `DisplayLabel` so nested type names retain enclosing type context [CommandDispatchAuthorizationGate.cs:676-681] — EH-66.
- [ ] [Review][Patch] Document inheritance hazard — `[RequiresPolicy]` has `Inherited=false` (Pass-2) but the gate uses `command.GetType()` (runtime type, may be derived). Either walk the type hierarchy to find the nearest declared policy, or document that derived command types must re-declare `[RequiresPolicy]` [CommandDispatchAuthorizationGate.cs:582-587, RequiresPolicyAttribute.cs] — BH-70.
- [ ] [Review][Patch] Static `s_emptyValidationErrors = ImmutableDictionary<string, IReadOnlyList<string>>.Empty` in `CreateForbiddenWarning` to avoid per-call allocation [CommandDispatchAuthorizationGate.cs:639-643] — BH-51 (low-pri, batch with others).

Renderer emitter (`CommandRendererEmitter`) hardening — issued contingent on DN-7-3-4-7 not being deferred:

- [ ] [Review][Patch] Apply Pass-3 hardening matrix to emitted `RefreshPresentationAuthorizationAsync`: `_disposed` short-circuit after await, `_cts` capture-and-pass, sequence-number guard, try/catch wrapping evaluator, `_authorizationPresentationReady = false` synchronously before await [src/Hexalith.FrontComposer.SourceTools/Emitters/CommandRendererEmitter.cs:815-841] — AA-03 / BH-38 / BH-40 / EH-28 / EH-29.
- [ ] [Review][Patch] Emit `AuthenticationStateChanged` subscription in renderer (mirrors form-emitter Pass-2 P1) so inline triggers reflect sign-out/role-change [CommandRendererEmitter.cs:815-841] — AA-03 / BH-39 / EH-30.
- [ ] [Review][Patch] Pass `null` (not `_prefilledModel`) as the `Command` arg of `CommandAuthorizationRequest` for presentation-time renderer probes — `_prefilledModel` is unvalidated user input and violates the Authorization Resource Contract (spec line 501 "must not contain ... arbitrary command payload values") [CommandRendererEmitter.cs:820-826] — AA-15 / BH-37.
- [ ] [Review][Patch] Add explicit parens around the Disabled-binding boolean expression — defends against future-edit reordering hazard, mirrors Pass-3 P17 patch on form emitter [CommandRendererEmitter.cs:859] — BH-44.
- [ ] [Review][Patch] Extend `Disabled` binding emission to `CompactInline` and `FullPage` modes (resolves DN-7-3-4-6 if option (a) chosen) [CommandRendererEmitter.cs:530, 593+, 567+] — AA-10 / BH-42 / EH-33 / EH-34.
- [ ] [Review][Patch] `ResolveAuthorizationSurface` switch — replace `_ => FullPage` default with explicit case-per-mode + log/throw on unknown so future `CommandRenderMode` additions don't silently mis-classify [CommandRendererEmitter.cs:831-837] — AA-22 / BH-41 / EH-31 / EH-62.
- [ ] [Review][Patch] Add Verify-snapshot for the protected variant + parse-test (mirrors `Renderer_FullPage_*` patterns) — current `Renderer_ProtectedCommand_EmitsEvaluatorBackedTriggerGate` test only asserts substring presence, would not catch emitted-code compile errors [tests/Hexalith.FrontComposer.SourceTools.Tests/Emitters/CommandRendererEmitterTests.cs:1202-1210] — AA-24 / BH-45.
- [ ] [Review][Patch] Reset `_authorizationPresentationReady`/`_authorizationPresentationAllowed` to `false` in `OnParametersSetAsync` to support component-instance reuse across different commands via `@key` [CommandRendererEmitter.cs:800-829] — BH-67.

Registry / `EmptyStateCtaResolver`:

- [ ] [Review][Patch] Trim `commandTypeName` key in `FrontComposerRegistry.TryGetCommandPolicy` (Pass-3 trimmed values; keys still pass un-trimmed) [src/Hexalith.FrontComposer.Shell/Registration/FrontComposerRegistry.cs:114-128] — covered by gate trim (above) but also at registry surface for symmetry — EH-12.
- [ ] [Review][Patch] Log `Warning` on `TryGetCommandPolicy` collision overwrite (Pass-3 P19 added warning at merge time; runtime cross-BC collisions silent) [FrontComposerRegistry.cs:120-124] — EH-23 / AA-25.
- [ ] [Review][Patch] Extend `IFrontComposerCommandPolicyRegistry.TryGetCommandPolicy` to return both policy and bounded-context (`out string policy, out string? boundedContext`) so the gate's `TryResolvePolicy` fast path doesn't have to call `ResolveBoundedContext` separately and risk choosing a different manifest than the policy came from [src/Hexalith.FrontComposer.Shell/Registration/IFrontComposerCommandPolicyRegistry.cs, FrontComposerRegistry.cs:114-128, CommandDispatchAuthorizationGate.cs:81-110] — AA-23 / BH-13 / EH-13 / EH-14 / EH-42 — also resolves DN-7-3-4-4 (b) if chosen.
- [ ] [Review][Patch] Enforce `StringComparer.Ordinal` for `DomainManifest.CommandPolicies` dictionary at the init coercion (today the comparer of the input dictionary is preserved — adopters can leak `OrdinalIgnoreCase` and break cross-surface lookups) [src/Hexalith.FrontComposer.Contracts/Registration/DomainManifest.cs:32-35] — EH-55.
- [ ] [Review][Patch] Make `IFrontComposerCommandPolicyRegistry.TryGetCommandPolicy` a default-implemented interface method that defers to a manifest-walk on `IFrontComposerRegistry.GetManifests()` — adopters' custom registries that don't implement the new companion silently take the gate's fallback path with the trim/BC drift defects [IFrontComposerCommandPolicyRegistry.cs:12-21] — BH-11 / EH-46.

Tests:

- [ ] [Review][Patch] Add integration test `EventStoreCommandClient_DispatchAsync_DeniedByGate_DoesNotSendHttp` injecting a denying gate and asserting zero `HttpClient.SendAsync` [tests/Hexalith.FrontComposer.Shell.Tests/Infrastructure/EventStore/] — AA-17.
- [ ] [Review][Patch] Add ordering test for `StubCommandService` — configure denying gate, assert no `CommandResult` is produced, assert `LifecycleSubmittedAction`/pending-registration/EventStore-send did not occur [tests/Hexalith.FrontComposer.Shell.Tests/Services/StubCommandServiceTests.cs] — BH-35 / EH-51.
- [ ] [Review][Patch] Dispose `ServiceProvider` in test `Provider` helper (each `BuildServiceProvider` leaks today) [tests/Hexalith.FrontComposer.Shell.Tests/Services/Authorization/CommandDispatchAuthorizationGateTests.cs:1071-1075] — BH-32.
- [ ] [Review][Patch] Add fallback-path test using a custom `IFrontComposerRegistry` substitute that does NOT implement `IFrontComposerCommandPolicyRegistry` — exercises the gate's fallback `manifests.CommandPolicies.TryGetValue` branch [CommandDispatchAuthorizationGateTests.cs] — BH-33 / EH-39.
- [ ] [Review][Patch] Add canonical-vs-fallback divergence test: `EmptyStateCtaResolver` canonical returns "X", manifest contains "Y" — assert canonical wins [tests/Hexalith.FrontComposer.Shell.Tests/Services/EmptyStateCtaResolverTests.cs] — BH-34 / BH-46.
- [ ] [Review][Patch] Strengthen `EnsureAuthorizedAsync_ProtectedAllowed_UsesDirectDispatchSurface` — `captured.Command.ShouldBeSameAs(command)` (not just `ShouldBeOfType`) to catch identity regressions [CommandDispatchAuthorizationGateTests.cs:1029-1033] — BH-64.
- [ ] [Review][Patch] Strengthen `EnsureAuthorizedAsync_ProtectedDenied_*` — assert the deny payload contains neither the policy name NOR the command FQN (today only policy is asserted absent) [CommandDispatchAuthorizationGateTests.cs:1042-1048] — BH-31 / EH-41.
- [ ] [Review][Patch] Add canonical failure-matrix tests for the gate: missing-evaluator, Pending decision, Canceled decision (post DN-7-3-4-5), Unauthenticated decision, scope-disposed `ObjectDisposedException`, null `decision` from broken impl [CommandDispatchAuthorizationGateTests.cs] — AA-16 / EH-40 / EH-68.
- [ ] [Review][Patch] Add CTA-region test asserting evaluator was actually consulted — today `PolicyProtectedCtaIsHiddenForUnauthorizedUser` would pass even if the evaluator were never called, because the catch-all `else` branch renders the same null `@NotAuthorized` for null `_decision` [tests/Hexalith.FrontComposer.Shell.Tests/Components/Rendering/FcProjectionEmptyPlaceholderTests.cs:944-961] — BH-28.
- [ ] [Review][Patch] Remove dead `AddAuthorizationCore + AddPolicy("OrderApprover", RequireAuthenticatedUser())` from `PolicyProtectedCtaUsesEvaluatorBackedResourceAuthorization` — the test substitutes the evaluator so the real `IAuthorizationService` policy is never consulted [FcProjectionEmptyPlaceholderTests.cs:907-909, 933-941] — BH-46.

Other:

- [ ] [Review][Patch] `ICommandDispatchAuthorizationGate` XML doc — document the throw contract (`CommandWarningException(Forbidden)` on deny, fail-closed on missing evaluator, etc.) so adopter alternate impls match [src/Hexalith.FrontComposer.Shell/Services/Authorization/ICommandDispatchAuthorizationGate.cs] — AA-26.
- [ ] [Review][Patch] Move new file paths (`FcAuthorizedCommandRegion.razor[.cs]`, `IFrontComposerCommandPolicyRegistry.cs`, `CommandDispatchAuthorizationGate.cs`/`I*.cs`, `CommandDispatchAuthorizationGateTests.cs`) into the spec File List section instead of appending to the "Party-Mode Review" tail [_bmad-output/implementation-artifacts/7-3-command-authorization-policies.md:264-265] — BH-49.
- [ ] [Review][Patch] Tighten sprint-status `last_updated` wording to match what was actually shipped (per AA-21) — drop the "renderer-emitted inline trigger presentation gating" claim (only Inline mode actually gated; CompactInline/FullPage not), drop the "evaluator-backed empty-state CTA region replacing policy-only AuthorizeView" framing (resource shape still diverges via missing BoundedContext), explicitly note DN4 and Generated/* bUnit are NOT closed [_bmad-output/implementation-artifacts/sprint-status.yaml:43] — AA-21 — outcome contingent on DN-7-3-4-1.

#### Deferred (Pass 4 net-new)

- [x] [Review][Defer] `ResolveCommandType` cross-assembly resolution — `Type.GetType(commandFqn)` only scans calling assembly + mscorlib; cross-assembly commands silently lose their CTA in trimmed AOT/WASM publishes. Needs a manifest-driven `Type` lookup via the registry. Defer to dedicated CTA hardening session or Story 9-5 (docs site) — EH-09 / AA-27.
- [x] [Review][Defer] Sub-millisecond perf concerns on `TryGetCommandPolicy` and `ResolveBoundedContext` O(N) walks — adopters with many manifests will see proportional dispatch latency; defer to Epic 9-6 backlog (cache hardening) — BH-9 / EH-38.
- [x] [Review][Defer] `Type` reference comparison vs `Type.Equals`/`AssemblyQualifiedName` for collectable `AssemblyLoadContext` proxy/dynamic types — exotic edge case, defer to plugin-loader hardening (no current story owns it) — BH-56 / EH-56.
- [x] [Review][Defer] Forensic correlation continuity across presentation and submit decisions — Pass 1 deferred; Pass 4 multiplies the issue with new Guid per `OnParametersSetAsync` call in CTA region. Defer to telemetry roadmap (Epic 9-4) — D-Pass1 / AA-18 / EH-27 / EH-70.
- [x] [Review][Defer] Audit-log on `Allowed` dispatches — Pass 3 deferred; gate also doesn't log Allowed. Defer to telemetry roadmap (Epic 9-4) — EH-49.
- [x] [Review][Defer] Tenant-context-changed subscription in CTA region and renderer — Pass 3 deferred for form pending Story 7-2 tenant-changed observable; same gap inherited by new components. Defer to same story — D-Pass3 / AA-02 (tenant-axis only).
- [x] [Review][Defer] `HumanizeCommandName` consistency across surfaces — gate's `DisplayLabel` differs from `EmptyStateCtaResolver.HumanizeCommandName` and form emitter; defer to display-label unification refactor — EH-16.
- [x] [Review][Defer] AC9 anonymous-principal end-to-end integration test — Pass 3 deferred awaiting DN3 + DN4; both partial in Pass 4. Defer to bUnit suite alongside DN-7-3-4-6 outcome — A33 / D-Pass3.
- [x] [Review][Defer] Per-circuit auth decision cache for palette/CTA/dispatch — Pass 1 deferred; new dispatch surfaces re-introduce N×M evaluation overhead under double-evaluation (form gates, then dispatch gate re-evaluates). Defer to Epic 9-6 — D-Pass1 / EH-74.
- [x] [Review][Defer] TOCTOU between gate evaluation and tenant resolution in `EventStoreCommandClient` — gate runs against tenant snapshot A; HTTP send may use snapshot B if user switches tenant in between. Submit-time evaluator already takes fresh tenant. Edge case; defer to Story 7-2 follow-up — EH-48.

#### Dismissed (Pass 4)

Withdrawn or self-corrected by the source layers (BH-17 first-version, BH-53 first-version, BH-54 second-version, BH-72, BH-75 — all marked withdrawn in the report; EH-22-self-demote, EH-25-self-demote, EH-32-self-demote, EH-49-self-demote, EH-65-self-demote, EH-67-self-demote — already explicitly demoted); design-intent (BH-43 const policy name baked at build time is the design; BH-58 ctor parameter ordering is fine; BH-66 defensive trim acceptable; BH-48 razor private-field access is fine; AA-25 silent-overwrite at lookup-time is informational); cosmetic / no functional bug (BH-9 last-write-wins is intentional; BH-23 manifests parameter shadow only matters under custom registry pattern — covered by deeper patch; EH-67 minor leak; BH-50 per-render Type lookup is cheap; BH-56 collectable AssemblyLoadContext same as defer above); already covered by other patches in this triage (BH-2, BH-4 → P-CR8 StateHasChanged sync; BH-5, BH-69 → P-CR5 sequence guard; EH-3, EH-29, EH-47 → P-CR2 IDisposable + P-CR4 disposed guard; EH-13, EH-14, EH-42 → registry P-extend-tuple-return; EH-46 → P-default-implement IFrontComposerCommandPolicyRegistry; AA-13/14 → split into the CR-cluster patches); dead-code / unreachable (BH-7 partial — second clause of cache update is covered); Pass-3 already-defered (BH-10 first variant of policy registry concurrency — kept as patch; rest covered).

### Decision Resolutions (Pass 4, 2026-05-02)

All seven decisions resolved by reviewer judgment per Jerome's `do best` directive.

- **DN-7-3-4-1 resolved → (b) Re-defer with explicit architectural justification** — Investigation during patch wave revealed that `FcHomeDirectory`, `FcHomeCard`, `FcCollapsedNavRail`, `FrontComposerNavigation`, and `CapabilityDiscoveryEffects` enumerate **projections only** (`DomainManifest.Projections`), not commands (`DomainManifest.Commands`). The only command-surfacing code in the Shell is `CommandPaletteEffects.CanSurfaceCommandAsync` (already gated since Pass 2) and `EmptyStateCtaResolver` (gated via `FcAuthorizedCommandRegion`). AC13's "Home/Capability discovery lists commands" condition is structurally not met in the v1 architecture — the surface does not enumerate commands at all. The Pass-3 P-E29 patch (defensive missing-evaluator warning inside `CapabilityDiscoveryEffects`) was based on the assumption that the effects enumerate commands; with that assumption falsified, P-E29 is dead code. Sprint-status wording needs to be tightened (AA-21 patch) to drop the "DN4 closed" claim and replace with "AC13 satisfied by architectural fact: v1 home directory enumerates projections only". When/if a future story adds command enumeration to the home directory, the same gating pattern as the palette applies and a `CanSurfaceCommandAsync`-shaped filter can be lifted to a shared helper.
- **DN-7-3-4-2 resolved → (a) Refactor to `AuthorizingCommandServiceDecorator`** — Honors DN2 binding decision per memory rule `feedback_cross_story_contracts.md`. Decorator wraps `ICommandService` registration in `AddHexalithFrontComposer*`. Reads `IFrontComposerCommandPolicyRegistry.TryGetCommandPolicy` to determine whether the command is protected, then delegates to `ICommandDispatchAuthorizationGate.EnsureAuthorizedAsync`. Per-impl optional gate ctor params on `StubCommandService` and `EventStoreCommandClient` are REMOVED (dead code under the decorator pattern; their Pass-4-introduced injection points are reverted). Preserves single canonical authorization site.
- **DN-7-3-4-3 resolved → Structural via DN-7-3-4-2 (a)** — With the decorator as the only authorization site for `ICommandService.DispatchAsync`, there is no longer an optional `gate is not null` short-circuit. AA-05 fail-open closure is automatic; `EventStoreCommandClassifierIntegrationTests.NewClient` is updated to construct without the (now-removed) gate parameter.
- **DN-7-3-4-4 resolved → (b) Extend `IFrontComposerCommandPolicyRegistry.TryGetCommandPolicy(string commandFqn, out string policyName, out string? boundedContext)`** — Single canonical lookup returns both. `CommandDispatchAuthorizationGate.TryResolvePolicy` consumes the tuple and drops the separate `ResolveBoundedContext` call (eliminates AA-23 drift between fast path and fallback). `EmptyStateCtaResolver.ResolveCommandPolicy` consumes the tuple too. `FcAuthorizedCommandRegion` accepts an optional `BoundedContext` parameter; `FcProjectionEmptyPlaceholder.razor` resolves BC via `IFrontComposerCommandPolicyRegistry` and threads it through. Eliminates the resource-shape divergence (AA-08).
- **DN-7-3-4-5 resolved → (a) Branch on `decision.Kind`** — `CommandDispatchAuthorizationGate.EnsureAuthorizedAsync`: `Pending → throw CommandWarningException(CommandWarningKind.Pending, ...)` with the localized "Action temporarily unavailable" copy + retry hint; `Canceled → throw OperationCanceledException(cancellationToken)` so callers can distinguish user-cancel from deny; only `Denied`/`FailedClosed-other` → `Forbidden`. Symmetric with form Pass-3 P-DN7 dual-copy patch.
- **DN-7-3-4-6 resolved → (b) Extend `Disabled` binding to CompactInline + FullPage modes; defer Generated/*Authorization* bUnit suite again** — Trigger gating now consistent across all three render modes (Inline / CompactInline / FullPage); closes AC10 trigger hole. The bUnit suite is re-deferred to Story 10-1 (adopter test host and component testing utilities) which is the natural owner for cross-surface bUnit fixtures and reduces Pass-4 scope to a manageable session. Add SKIP-marked test stubs in `tests/Hexalith.FrontComposer.Shell.Tests/Generated/Authorization/` documenting the deferred coverage so Story 10-1 has a concrete starting point.
- **DN-7-3-4-7 resolved → (a) Apply full hardening matrix to both new code paths** — `FcAuthorizedCommandRegion`: implement `IDisposable`, capture CTS, pass `_cts.Token` to evaluator, `_disposed` short-circuit after await, sequence-number guard against concurrent `OnParametersSetAsync`, subscribe to `AuthenticationStateChanged`, try/catch around evaluator, sync `StateHasChanged` after Pending stand-in, single correlation ID per refresh cycle. Renderer-emitted `RefreshPresentationAuthorizationAsync`: same matrix in emitted code. Mirrors Pass-3 form-emitter discipline.

#### Patches Applied (Pass 4 — 2026-05-02)

`/bmad-code-review 7-3` Pass 4 patches applied this session per `do best` directive after the seven decisions resolved above.

Foundational structural changes:
- ✅ **DN-7-3-4-4 (b)** — `IFrontComposerCommandPolicyRegistry.TryGetCommandPolicy` extended to `(out string policy, out string? boundedContext)`. Default-implemented on the interface (manifest-walk with last-write-wins + trim) so adopters with a custom `IFrontComposerRegistry` automatically get policy lookup without overriding. `FrontComposerRegistry` overrides for snapshot-based concurrent-safe enumeration. `EmptyStateCta` record extended with `BoundedContext` field and threaded through `EmptyStateCtaResolver` → `FcProjectionEmptyPlaceholder` → `FcAuthorizedCommandRegion` so the evaluator's `CommandAuthorizationResource` shape is symmetric across CTA, palette, dispatch, and form surfaces (closes AA-08 / AA-23).
- ✅ **DN-7-3-4-2 (a) + DN-7-3-4-3** — New `AuthorizingCommandServiceDecorator` (internal sealed) wrapping `ICommandServiceWithLifecycle` (which inherits `ICommandService`). Registered via factory in both `AddHexalithFrontComposer` and `AddHexalithEventStore`. The Pass-4-introduced optional `ICommandDispatchAuthorizationGate? authorizationGate = null` ctor parameters were REMOVED from `StubCommandService` and `EventStoreCommandClient` — fail-open surface eliminated structurally.
- ✅ **AddHexalithFrontComposer** now calls `AddAuthorizationCore()` (idempotent) so adopters get `IAuthorizationService` for free; spec D4 fail-closed becomes structural rather than dependent on adopter wiring discipline.

Direct-dispatch gate hardening (`CommandDispatchAuthorizationGate` rewrite):
- ✅ Localize copy via `IStringLocalizer<FcShellResources>?` with static fallback (closes AA-07 / EH-15 / BH-17). Localizer is optional — fall through to English when not registered (test/minimal-host friendly).
- ✅ Branch on `decision.Kind` (DN-7-3-4-5): `Pending → CommandWarningException(Pending)`, `Canceled → OperationCanceledException`, only `Denied`/`FailedClosed-other` → Forbidden (closes AA-06 / EH-54 / EH-63 / EH-69).
- ✅ Lazy evaluator resolution via `IServiceProvider` with `InvalidOperationException`/`ObjectDisposedException` catch (BH-16 / EH-19).
- ✅ `cancellationToken.ThrowIfCancellationRequested()` at entry (BH-12 / EH-18).
- ✅ Trim `commandTypeName` once at entry (BH-14 / EH-12).
- ✅ Reject `commandType.FullName == null` types as Blocked (BH-60).
- ✅ Drop separate `ResolveBoundedContext` fallback path — registry tuple lookup is the only source of truth (eliminates BH-13 / BH-23 BC-drift / fallback-divergence concerns).
- ✅ `DisplayLabel` uses `Ordinal` suffix strip (BH-19), guards empty-string for "Command"-named types (EH-17).
- ✅ `EntityLabel` returns generic localized "Action unavailable" so deny payload doesn't leak command type names to attackers (BH-18).
- ✅ Static empty `s_emptyValidationErrors` dictionary (BH-51).
- ✅ Include `PolicyName` in deny `LogWarning` (EH-21).
- ✅ Defensive null-check on evaluator's returned decision (BH-61).

CTA region (`FcAuthorizedCommandRegion`) full hardening matrix:
- ✅ `IDisposable` + `Dispose()` pattern; CTS captured and disposed; `_authorizationCts` cancelled on dispose (AA-13 / EH-03 / EH-05 / EH-47).
- ✅ Cancellation token threaded into `Evaluator.EvaluateAsync(_cts.Token)` (BH-06 / EH-58).
- ✅ `_disposed` short-circuits before and after every await (AA-13 / BH-05 / EH-29).
- ✅ Sequence-number guard against concurrent `OnParametersSetAsync` via `Interlocked.Increment`/`Interlocked.Read`; stale completions dropped (AA-14 / BH-05 / BH-69 / EH-06).
- ✅ Subscribe to `AuthenticationStateChanged`; re-evaluate on transitions; unsubscribe in `Dispose` (AA-02 / EH-02 / EH-60).
- ✅ Try/catch wrapping `EvaluateAsync` — `OperationCanceledException` → `Blocked(Canceled)`, generic exception → `Blocked(HandlerFailed)` with logged warning (BH-03 / BH-62).
- ✅ `_decision = Pending(...)` synchronously THEN `StateHasChanged()` BEFORE the evaluator await so the Pending fragment renders during the in-flight cycle (BH-02 / BH-04 / EH-06).
- ✅ Single correlation ID reused for both Pending stand-in and final decision per refresh cycle (BH-08 / EH-27 / AA-18 / EH-70).
- ✅ Razor markup re-ordered — null/Pending → Pending fragment (avoids first-render NotAuthorized flicker — BH-01 / EH-26).
- ✅ Cache fields seeded on early MissingPolicy return (BH-07 / AA-20).
- ✅ `BoundedContext` parameter populated from `EmptyStateCta.BoundedContext` (closes AA-08).

Renderer emitter (`CommandRendererEmitter`) full hardening matrix in emitted code:
- ✅ Emit `_authorizationCts`, `_authorizationDisposed`, `_authorizationRefreshSequence`, `_authorizationStateSubscribed` fields (DN-7-3-4-7 hardening parity with form emitter).
- ✅ Emit `Dispose()` method + `IDisposable` on the class declaration (only when `hasAuthorizationPolicy`).
- ✅ Emit `AuthenticationStateChanged` subscription with `try { InvokeAsync(...) } catch (ObjectDisposedException) { }` envelope (AA-03 / BH-39 / EH-30 / EH-08 / BH-13).
- ✅ Emit `_disposed` short-circuits before/after await; sequence guard via `Interlocked.Increment`/`Read`; cancellation token capture (AA-03 / BH-38 / EH-28 / EH-29).
- ✅ Emit `try/catch` around `EvaluateAsync` with `OperationCanceledException` → return, generic exception → log + null decision → fail-closed `Ready=true, Allowed=false`.
- ✅ Pass `null` (not `_prefilledModel`) as the `Command` arg of `CommandAuthorizationRequest` (AA-15 / BH-37 — presentation-time auth must not include unvalidated user input per spec line 501).
- ✅ Wrap `_externalSubmit is null` in explicit parens before `||` (BH-44).
- ✅ Extend `Disabled` gating to CompactInline + FullPage modes via emitted `if (AuthorizationTriggerDisabled()) { /* placeholder */ break; }` branches at the top of each switch arm (DN-7-3-4-6 (b) / AA-10 / BH-42 / EH-33 / EH-34).
- ✅ `ResolveAuthorizationSurface` switch — explicit `FullPage` case in addition to `_ =>` default (AA-22 / BH-41 / EH-31).

Registry / `EmptyStateCtaResolver`:
- ✅ `FrontComposerRegistry.TryGetCommandPolicy` snapshot-locks `_manifests` enumeration; trims commandTypeName key; logs Warning on cross-BC collision overwrite (BH-10 / EH-12 / EH-23 / AA-25).
- ✅ `EmptyStateCtaResolver.ResolveCommandPolicy` is now an instance method that returns `(string? Policy, string? BoundedContext)` via the canonical registry tuple lookup. Drops the `manifests`-walk fallback entirely; eliminates snapshot-vs-live divergence (BH-23 / BH-59 / EH-24).
- ✅ `EmptyStateCta` record carries `BoundedContext` field which CTA flows to evaluator resource.
- ✅ `FcProjectionEmptyPlaceholder.ResolveCommandType` logs Information when type resolution returns null so operators can distinguish silent-suppression-by-trim from authorization-deny (AA-09 / BH-26 / BH-27 / EH-08 / EH-45 / EH-59).

Tests:
- ✅ New `AuthorizingCommandServiceDecoratorTests` — verifies `Received.InOrder(gate, inner)`, gate-throws short-circuits inner with zero side effects, lifecycle overload routes through gate, null command throws ArgumentNullException before gate.
- ✅ Expanded `CommandDispatchAuthorizationGateTests` — new failure-matrix tests for Pending → CommandWarningKind.Pending, Canceled → OperationCanceledException, null command → ArgumentNullException; strengthened Allowed/Denied assertions to use `ShouldBeSameAs` (BH-64) and to assert the deny payload contains neither policy name nor command FQN (BH-31 / EH-41).
- ✅ Updated `EmptyStateCtaResolverTests` — `IWriteAwareRegistry` composite now extends `IFrontComposerCommandPolicyRegistry`; `CreateResolver` wires explicit `TryGetCommandPolicy` substitution that mirrors the production manifest-walk semantics; new `Resolve_CommandWithPolicy_BC_ManifestFallback` covers the manifest-walk default impl path; `Resolve_CommandWithPolicy_UsesCanonicalRegistryPolicyLookup` asserts BoundedContext flows through the canonical lookup.
- ✅ Updated `StubCommandServiceTests` — gate-related test (`DispatchAsync_InvokesAuthorizationGateBeforeAcceptedResult`) removed because gate now lives in the decorator; gate behaviour covered by `AuthorizingCommandServiceDecoratorTests`.
- ✅ Updated `FcProjectionEmptyPlaceholderTests` — `PolicyProtectedCtaIsHiddenForUnauthorizedUser` now asserts the evaluator was actually consulted via `_authorizationEvaluator.LastRequest` shape, not just the absence of the CTA element (BH-28). Test command types `CreateOrderCommand` reverted to `private`, `ProtectedCreateOrderCommand` moved to namespace-scope `internal` (kept top-level so its FullName matches a real production command shape — nested-type `+` separators would break `CommandRouteBuilder.IsInternalRoute`).
- ✅ Updated `EventStoreRegistrationTests` and `SeamExtractionSmokeTests` — assertions updated for the new decorator pattern (inner `EventStoreCommandClient` still resolvable separately; `ICommandService` resolves through the decorator).
- ✅ Updated `CommandRendererEmitterTests.Renderer_ProtectedCommand_EmitsEvaluatorBackedTriggerGate` — adds parse-test (BH-45 / AA-24) and asserts the new hardening fields are emitted (`_authorizationCts`, `_authorizationDisposed`, `_authorizationRefreshSequence`, `AuthenticationStateChanged += OnAuthenticationStateChanged`); new `Renderer_ProtectedCommand_GatesAllRenderModes` test asserts the placeholder `if (AuthorizationTriggerDisabled())` branches in CompactInline + FullPage modes.

Other:
- ✅ `ICommandDispatchAuthorizationGate` XML doc documents the throw contract (`Forbidden` for deny / fail-closed, `Pending` for transient, `OperationCanceledException` for cancel) so adopter alternate impls match (AA-26).

DN-7-3-4-1 (DN4 Capability/Home gating) closure: investigated and found that `FcHomeDirectory`, `FcHomeCard`, `FcCollapsedNavRail`, `FrontComposerNavigation`, and `CapabilityDiscoveryEffects` enumerate **projections only**, not commands. AC13 is structurally satisfied — the surface does not enumerate commands. Pass 3 P-E29 (defensive missing-evaluator warning in `CapabilityDiscoveryEffects`) was based on the false assumption of command enumeration; with that assumption falsified, the patch is dead code and was not applied.

#### Pending (re-deferred to Story 10-1 + Pass 5)

- ❌ **Generated/*Authorization* bUnit suite (Pass-3 P3 + P4-full)** — DN-7-3-4-6 (b) re-deferred this to Story 10-1 (adopter test host and component testing utilities). Rationale: the suite needs cross-surface bUnit fixtures (inline / compact / full-page / CTA / palette / home), interaction-level assertions, and tenant-switch / sign-out / destructive-dialog-delay scenarios — the natural home is the adopter test host story. Add SKIP-marked stub files in `tests/Hexalith.FrontComposer.Shell.Tests/Generated/Authorization/` is OPTIONAL future work; deferred for now to keep Pass 4 scope manageable.
- ❌ **Tenant-context-changed subscription** — same as Pass 3 deferral — pending Story 7-2 tenant-changed observable.
- ❌ **Per-circuit auth decision cache for palette/CTA/dispatch** — Pass 1 deferred; Pass 4 introduces double-evaluation (form gate + dispatch decorator gate) for protected commands. Defer cache hardening to Epic 9-6.
- ❌ **TOCTOU between gate and tenant resolution in EventStoreCommandClient** — defer to Story 7-2 follow-up (tenant-changed observable would also fix this).

#### Validation

```
dotnet build Hexalith.FrontComposer.sln -warnaserror -p:UseSharedCompilation=false → 0 warnings, 0 errors
dotnet test Hexalith.FrontComposer.sln --no-build -p:UseSharedCompilation=false →
  Contracts 156/0/0,
  Shell 1536/0/0 (+ new AuthorizingCommandServiceDecoratorTests, expanded gate failure matrix, expanded resolver/CTA assertions),
  SourceTools 600/0/0 (+ Renderer_ProtectedCommand_GatesAllRenderModes),
  Bench 2/0/0;
  total 2294/0/0
```
...
| `src/Hexalith.FrontComposer.Shell/Components/Rendering/FcAuthorizedCommandRegion.razor[.cs]` | Pass 4 — evaluator-backed CTA gating component (DN-7-3-4-7 hardening: IDisposable + cts + sequence guard + AuthState subscribe + try/catch + Pending-fragment-first render order). |
| `src/Hexalith.FrontComposer.Shell/Registration/IFrontComposerCommandPolicyRegistry.cs` | Pass 4 — canonical policy + bounded-context tuple lookup with default-implemented manifest-walk fallback. |
| `src/Hexalith.FrontComposer.Shell/Services/Authorization/AuthorizingCommandServiceDecorator.cs` | Pass 4 — decorator wrapping `ICommandService.DispatchAsync` so direct callers (palette, MCP bridge, adopter custom code) hit the authorization gate before any side effect. |
| `src/Hexalith.FrontComposer.Shell/Services/Authorization/CommandDispatchAuthorizationGate.cs` + `ICommandDispatchAuthorizationGate.cs` | Pass 4 — gate hosts the policy evaluation logic; decorator delegates to it. |
| `tests/Hexalith.FrontComposer.Shell.Tests/Services/Authorization/CommandDispatchAuthorizationGateTests.cs` + `AuthorizingCommandServiceDecoratorTests.cs` | Pass 4 — gate failure-matrix coverage and decorator gate-before-inner ordering tests. |
