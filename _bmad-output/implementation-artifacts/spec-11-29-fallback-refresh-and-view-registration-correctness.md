...
Chunk 2 review (2026-09-20). Layers: Blind Hunter, Edge Case Hunter, Verification Gap Reviewer, Acceptance Auditor.

- [x] [Review][Patch] `BadgeCountService.InitializeAsync` is not safe under concurrent invocation, so `RegisterLane`'s new full-contract equivalence check can throw and abort badge-count seeding — `LaneContractsAreEquivalent` requires `ReferenceEquals(first.RefreshAsync, second.RefreshAsync)`. `BadgeCountService.RegisterReconciliationLane` (`src/Hexalith.FrontComposer.Shell/Badges/BadgeCountService.cs:268-279`) builds a fresh `token => RefreshCountLaneAsync(type, token)` closure on every call into `_reconciliationRegistrations.GetOrAdd`. `BadgeCountService.InitializeAsync` is invoked from two independent Fluxor effects, `CapabilityDiscoveryEffects.HandleAppInitialized` (unconditional) and `HandleStorageReady` (guarded only by `HydrationState == Idle`); `HydrationState` does not leave `Idle` until `BadgeCountsSeededAction` is dispatched at the very end of `InitializeAsync`, so both effects can call `InitializeAsync` concurrently on the same instance before either completes. `ConcurrentDictionary.GetOrAdd`'s factory can then run twice for the same `Type` key, producing two distinct `RefreshAsync` closures for an otherwise-identical lane contract; the second `RegisterLane` call now throws `InvalidOperationException` from inside a Fluxor `[EffectMethod]`, aborting the remaining catalog registrations/fetches in `InitializeAsync`'s loop. The same pre-existing race also silently discards one of the two returned `IDisposable` registrations even when no exception occurs, permanently leaking a refcounted scheduler entry for the process lifetime (`RefCount` never returns to 0). No test in `BadgeCountServiceTests.cs` wires a real scheduler + user-context-accessor or exercises concurrent `InitializeAsync` calls. **Decision (owner, 2026-09-20): fix `BadgeCountService`/`CapabilityDiscoveryEffects` with a single-flight guard on `InitializeAsync` so it cannot run twice concurrently on the same instance; do not weaken the scheduler's conflict-detection contract.** [ProjectionFallbackRefreshScheduler.cs:414-424; BadgeCountService.cs:119-161,268-279; CapabilityDiscoveryEffects.cs:125-151] — **Applied:** added an `_initializeGate`/`_initializeTask` single-flight guard around a new private `InitializeCoreAsync` so overlapping callers share one in-flight fetch; the guard resets once the shared task completes so a later, independent re-initialization (e.g. a reconnect re-poll) still runs a fresh fetch instead of replaying a stale result forever. First patch draft cached the completed task permanently, which silently broke `BadgeCountServiceNoChurnTests.UpdateCount_ChangedValue_EmitsAgain` (sequential re-`InitializeAsync` no longer re-ran); caught by running the full focused suite after patching and fixed before landing. `SecurityLoggingGovernanceTests.ExpectedResidualWarningAndAboveLocations` updated from `InitializeAsync` to `InitializeCoreAsync` to track the moved `FrontComposerWarningLog` call site.
- [x] [Review][Patch] `ProjectionFallbackLane`'s doc remark is now stale/misleading [ProjectionFallbackLane.cs:6-10] — the `<remarks>` still says "`ViewKey` remains the dedupe identity for refcounted registrations," but registration now requires the full contract (tenant, page window, filters, sort, search, callback) to match, and a mismatch throws. Update the remark to describe the new equivalence/conflict rule. — **Applied.**
- [x] [Review][Patch] `DecrementLane` does not clear `_pendingRetry` on final disposal [ProjectionFallbackRefreshScheduler.cs:381-396] — it removes `_lastEtagByLane`/`_lastNoEtagSignatureByLane` for the view key but not a matching `_pendingRetry` entry. If a nudge sets `_pendingRetry[viewKey]` and the owning `RefreshLaneAsync` call then exits via an uncaught `OperationCanceledException` (skipping its own end-of-method `_pendingRetry.TryRemove` cleanup) around the same time as final disposal, the disposed `LaneEntry` stays referenced indefinitely, contradicting the "disposal-safe cache cleanup" invariant. Add `_pendingRetry.TryRemove(viewKey, out _)` alongside the two existing cache removals. — **Applied.**
- [x] [Review][Patch] Unconditional 16 KB scratch-buffer allocation for zero-row pages [ProjectionFallbackRowSignature.cs:38-39] — `Create` allocates `serializationBuffer` before checking `rowsToHash`, wasting an allocation on every empty/filtered-to-zero no-ETag poll. Move the allocation inside the `rowsToHash > 0` path. — **Applied.**
- [x] [Review][Defer] `IProjectionFallbackRefreshScheduler` has no XML documentation on any member [IProjectionFallbackRefreshScheduler.cs:1-16] — deferred: pre-existing repository-wide gap predating this story (the interface file is unchanged by this diff); the new `InvalidOperationException` on `RegisterLane` inherits the same undocumented state. Documenting the whole interface is a broader hygiene task outside Story 11.29's Code Map.

#### Rejected (Chunk 2)

- BH `GetActiveLane` is unused — false: reconfirmed from Chunk 1; the private helper is never invoked and produces no user- or developer-facing defect.
- BH `RegisterLane`'s `TryAdd`-failure branch is unreachable — low: every `_lanes` mutation happens under `_laneGate`, and the branch is reached only immediately after confirming under that same lock that the key is absent; harmless defensive dead code.
- BH no regression test covers `IsNotModified=true` with a null/empty `ETag`, and `TryRecordValidatorState`'s no-ETag fallback silently discards `isReliable` — false: `ProjectionFallbackRefreshSchedulerTests.cs` already constructs `ProjectionPageResult(..., null, IsNotModified: true)` in multiple fixtures (e.g. lines 39, 142, 347, 407, 441, 537, 692, 724, 903); discarding `isReliable` at that call site is safe because every future comparison always re-derives `isReliable` fresh at comparison time — the stored signature is only ever used as a raw equality baseline, never as a trust flag.
- BH recursive depth guard in `AppendElement` overlaps with `JsonDocumentOptions.MaxDepth` — low: both bounds are consistent defense-in-depth; no demonstrated behavior difference, purely a documentation/clarity nit.
- BH `DispatchGate`-before-`_laneGate` lock-ordering invariant is undocumented — low: real gap but a maintenance-risk documentation nit with no live defect today.
- EC `ProjectionFallbackRowSignature.Create`'s per-row loop does not check `CancellationToken` — low: the work is already tightly bounded (≤128 rows × 16 KiB), so wasted CPU after cancellation is negligible; the fix would add a new parameter across three call sites.
- EC/BH no-ETag lanes with `itemCount > MaxRows (128)` always set `isReliable=false`, forcing `Changed` every reconciliation pass — false: this is the documented, deliberately conservative "bound signature work/material" trade-off, already proven correct by the existing oversized/over-count fixtures (VG-02 in the Review Triage Log).
- Acceptance Auditor: no new acceptance-criteria violations, spec-intent deviations, missing behavior, or spec/code contradictions found.
...
- 2026-09-20: Applied all four Chunk 2 review patches (single-flight `BadgeCountService.InitializeAsync` guard, `ProjectionFallbackLane` doc remark, `DecrementLane` `_pendingRetry` cleanup, conditional row-signature buffer allocation) and verified with focused and full Shell test runs.
...
**Chunk 2 patch verification (2026-09-20):**
- `dotnet build src/Hexalith.FrontComposer.Shell/Hexalith.FrontComposer.Shell.csproj -c Release -p:NuGetAudit=false -p:MinVerVersionOverride=1.0.0 -nodeReuse:false` — Build succeeded, 0 Warning(s), 0 Error(s).
- `dotnet build tests/Hexalith.FrontComposer.Shell.Tests/Hexalith.FrontComposer.Shell.Tests.csproj -c Release -p:NuGetAudit=false -p:MinVerVersionOverride=1.0.0 -nodeReuse:false` — Build succeeded, 0 Warning(s), 0 Error(s).
- xUnit v3 in-process runner, full `Hexalith.FrontComposer.Shell.Tests` assembly (`-parallel` default) — Total: 2739, Failed: 0 (after fixing the single-flight regression and the governance ledger entry it moved). One `CommandTargetGeneratedFormTests.BlockingCloneGetter_UsesSingleBoundedWorkerAndLaterSubmissionsFailTargetFast` timeout observed on the first full run under heavy parallel CPU contention; reran in isolation and it passed in 0.8s, confirming pre-existing timing flakiness unrelated to this diff (not caused by the four patches).
- Focused rerun (`-class` for `BadgeCountServiceTests`, `BadgeCountServiceNoChurnTests`, `ProjectionFallbackRefreshSchedulerTests`, `ProjectionFallbackPollingDriverTests`, `CapabilityDiscoveryEffectsScopeTests`, `CapabilityDiscoveryConsumerIntegrationTests`, `NudgeToSchedulerLaneRefreshIntegrationTests`, `EventStoreActionQueueCountReaderTests`, `SecurityLoggingGovernanceTests`, `ShellLayeringTests`, `ShellTypeOrganizationGovernanceTests`) — Total: 116, Failed: 0.
