...
- [ ] **Task 4 — Add release-blocking graph diff and affected-module gates (AC: #2, #4)**
  - [x] **Local work is unblocked by the approved 2026-08-02 correction.** The local graph, policy, materialization, closure, handoff schema, and caller-side governance are implemented. **2026-08-08:** AD-13 `create-ci` is wired into `.github/workflows/ci.yml` and always invokes `create_ci_handoff`; successful sealed emission still requires a non-empty active-policy `evaluator_authorizations.ci` entry (AD-16 external gate below).
  ...
  - [x] Implement the authenticated `dependency-release-handoff` producer/consumer contract and local fixtures. For every successful eligible `push`, emit exactly one `hexalith.dependency-release-handoff.v1` artifact binding repository, workflow/event/run/attempt, immutable base and exact candidate, normalized graph/diff, bounded affected-module proof, active policy coordinates/digest, evaluator closure/conclusion, and raw handoff digest. The authenticated handoff candidate is the sole downstream release-candidate authority. **2026-08-08:** `.github/workflows/ci.yml` drafts a CI evaluator and calls `dependency_handoff.py create-ci` (always invokes `create_ci_handoff`); on authorization success it uploads `dependency-release-handoff-<run>-<attempt>`. Empty/`@main` evaluator state fails closed with exit 2 and a deferred diagnostic without failing the graph gate. `.github/workflows/release.yml` downloads that artifact, runs `verify-ci --live`, and treats its candidate as sole release authority via `DEPENDENCY_RELEASE_HANDOFF`/`RELEASE_EVALUATOR`. Exact-source proof remains uploaded for forensics only.
...
- [ ] **Task 5 — Extend the existing sealed release manifest (AC: #5, #6)**
  - [x] **Local work is unblocked by the approved 2026-08-02 correction.** The local manifest, policy, handoff contracts, verifier, post-release consumer, and fixtures are implemented. **2026-08-08:** Release caller wiring for AD-13 consume + AD-15 emit is in place; successful sealed handoffs still require active-policy evaluator authorization (AD-16 external gate below).
  ...
  - [x] Update `.github/workflows/release.yml` and its local handoff consumer so the caller authenticates the triggering run ID/attempt and event head through read-only Actions APIs, verifies the named successful-CI handoff, and treats that handoff's matching candidate as the sole release-candidate authority. Reload the recorded policy from its exact commit and consume the authenticated candidate everywhere. Preserve the REL-4 freeze and `.releaserc.json` publication ownership; a tag, ambient/default branch, second-hop event SHA, or mutable `@main` seam cannot authorize publication. **2026-08-08:** `verify-source` now downloads `dependency-release-handoff-<run>-<attempt>`, runs `verify-ci --live`, and threads `DEPENDENCY_RELEASE_HANDOFF`/`RELEASE_EVALUATOR` into prepare. Missing AD-13 artifacts fail closed (release eligibility remains AD-16-gated while `evaluator_authorizations` is empty / CI reusable stays `@main`).
  - [x] Under `if: always()`, make every governed Release attempt upload exactly one `release-verification-handoff` artifact conforming to AD-15, including the authenticated CI run/attempt/raw handoff hash, exact policy projection, and failure/partial null representations. Update `release-evidence.yml` to authenticate both runs/artifacts, require matching candidate/policy projections even on pre-manifest failure, reload the exact base/before policy, derive the root only from the original CI candidate plus sealed manifest, require a policy-authorized post-release closure, and verify/record success, failure, or partial publication without using second-hop `workflow_run.head_sha` as the candidate or green-no-oping. **2026-08-08:** `emit-verification-handoff` (`if: always()`) calls `create-release` → `create_release_handoff` and uploads `release-verification-handoff-<run>-<attempt>` (bundling the raw AD-13 CI handoff when sealed). Unauthorized evaluators or missing AD-13 input produce a deferred diagnostic rather than a fake seal. `release-evidence.yml` authenticates that artifact first, runs `verify-release` when the sealed pair is present, prefers the AD-15 candidate over second-hop `head_sha`, and retains the existing disposition/ledger forensics.
...
- 2026-08-08: Implemented the deferred-work option (b) — wire AD-13/AD-15 into production workflows — without fabricating evaluator authorization or pinning `domain-ci.yml@main`.
  - Added `draft-evaluator` / `create-ci` / `create-release` CLIs to `eng/dependency_handoff.py`. `create-ci` always calls `create_ci_handoff`; unauthorized/empty registry returns exit 2 with a deferred diagnostic.
  - `.github/workflows/ci.yml` drafts a CI evaluator and runs `create-ci` on successful push graph gates; uploads `dependency-release-handoff-*` only when sealed.
  - `.github/workflows/release.yml` requires the AD-13 artifact (`verify-ci --live`), prepares via `DEPENDENCY_RELEASE_HANDOFF`/`RELEASE_EVALUATOR`, and adds `emit-verification-handoff` (`if: always()`) calling `create_release_handoff`.
  - `.github/workflows/release-evidence.yml` authenticates the AD-15 artifact first, runs `verify-release` when the sealed pair is present, and prefers the AD-15 candidate over second-hop `head_sha`.
  - `eng/release_prepublish.py` forwards `--ci-handoff` / `--release-evaluator` when those env vars are set.
  - Focused validation: `python3 -m unittest tests/eng/test_dependency_graph.py tests/eng/test_workflow_source_closure.py tests/eng/test_dependency_handoff.py tests/eng/test_release_evidence_v2.py` = 98/98; `CiGovernanceTests` = 61/61; `py_compile` clean. Preserved `baseline_commit` `3786330d241c2d87449fa3e01afc95fc832552df`. No push/remote/submodule ops. External AD-16 gate unchanged (issue 17 still closed; empty `evaluator_authorizations`; CI still `@main`).
...
- **The story remains incomplete for the external AD-16 seam.** The active policy intentionally authorizes no production evaluator while Hexalith.Builds issue 17/successor has no owner-accepted immutable reusable-workflow revision. CI/Release now invoke the AD-13/AD-15 constructors but defer sealed emission (exit 2) rather than fabricating authorization; no end-to-end publication/evidence claim is made. Story 11.17d completed separately on 2026-08-02 and is not reopened.
...
- Status stays `in-progress`, not `review`: local AD-13/AD-15 producer/consumer wiring is in place, but sealed successful-push/Release handoff emission, immutable reusable-workflow integration, and the final end-to-end evidence chain still require the qualifying Hexalith.Builds revision and active-policy authorization.
- **2026-08-08:** Wired AD-13/AD-15 into production workflows. `eng/dependency_handoff.py` gained `draft-evaluator`/`create-ci`/`create-release` CLIs; `ci.yml` always invokes `create_ci_handoff` via `create-ci`; `release.yml` consumes AD-13 as sole release-candidate authority and emits AD-15 under `if: always()` via `create_release_handoff`; `release-evidence.yml` authenticates the AD-15 artifact and prefers its candidate over second-hop `head_sha`. Local Task 4/5 handoff bullets are checked. Task 4/5 parent rows and both **External integration gate only** bullets remain open: Hexalith.Builds issue 17 is still closed without a qualifying revision, `evaluator_authorizations` remains empty, and `ci.yml` still references `domain-ci.yml@main`, so sealed handoff emission stays deferred (exit 2) and release eligibility remains blocked per AD-16.
