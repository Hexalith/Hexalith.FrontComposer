...
_Generated: 2026-05-13 | Reviewer: Claude Sonnet 4.6 via bmad-code-review (Blind Hunter + Edge Case Hunter + Acceptance Auditor)_

#### Decision-Needed

- [x] [Review][Decision] D1: `FindRegisteredEnvironment` uses `BuildServiceProvider()` inside DI registration loop — resolved: keep with doc-comment warning (Option 1 applied) — The new factory-registered `IHostEnvironment` support calls `services.BuildServiceProvider()` inside the discovery loop (`AddFrontComposerDevModeExtensions.cs`). This is the "build ServiceProvider inside ConfigureServices" anti-pattern: it allocates and immediately disposes a full secondary container, triggering any `IDisposable` singleton registrations already in `services` and running `ValidateOnBuild` validators eagerly. If any factory dependency is not yet registered at the call site, the build throws and leaves dev-mode configuration incomplete with no user-facing error. Decision: keep the current approach with documented limitations (a comment warning adopters not to call before logging/IHostEnvironment registration), or refactor to a `Lazy<>` or keyed-service resolution that avoids the premature build.
- [x] [Review][Decision] D2: `#if DEBUG` guard on factory-registration test leaves new `BuildServiceProvider` code path untested in Release CI — resolved: keep guard (Option 1 confirmed; production code is also `#if DEBUG`) — `AddFrontComposerDevMode_RegistersDevModeServicesWithFactoryRegisteredDevelopmentEnvironment` is wrapped in `#if DEBUG` (`AddFrontComposerDevModeExtensionsTests.cs`). Since the production code path (`FindRegisteredEnvironment`) is also conditionally compiled via the same `#if DEBUG` guard in the extension method, this may be intentional. Decision: confirm the guard is intentional, or move the test to a non-DEBUG block to ensure Release builds exercise the new discovery logic.

#### Patch

- [x] [Review][Patch] P1: `ReconciliationSweepReducer.ReduceMark` uses `break` instead of `continue` — silently drops updates to already-tracked keys when the dictionary is at capacity [`src/Hexalith.FrontComposer.Shell/State/ReconnectionReconciliation/ReconciliationSweepState.cs`]. When `MaxSweepMarkers` is reached and a new-key check fires (`!next.ContainsKey(viewKey) && next.Count >= MaxSweepMarkers`), `break` exits the outer loop entirely, silently discarding all remaining `viewKey` entries in the action even if subsequent keys already exist in the dictionary and would be updates (not insertions). Fix: change `break` to `continue`.
- [x] [Review][Patch] P2: French `CurrentLevelLabel` test is a false positive — dismissed: files already use U+00A0; tool preview rendered non-breaking space as plain space — `ShouldBe(' ')` (U+0020) does not enforce the intended non-breaking space [`tests/Hexalith.FrontComposer.Shell.Tests/Resources/DevModeStringsTests.cs`]. The test name says "UsesNonBreakingSpaceBeforeColon" but `frValue[colonIndex - 1].ShouldBe(' ')` passes for any space character. The `fr.resx` value `"Niveau actuel :"` uses a plain ASCII space (U+0020), not a non-breaking space (U+00A0) as French typography requires. Fix: change the assertion to `ShouldBe(' ')` and update the resource value to use a literal U+00A0.
- [x] [Review][Patch] P3: `Identifier` in `CounterCountSlot` can produce empty string or leading-hyphen IDs for edge-case field names [`samples/Counter/Counter.Web/Components/Slots/CounterCountSlot.razor`]. A field name of all-punctuation characters (e.g. `"---"`) passes the `IsNullOrWhiteSpace` guard but every character maps to `-` and then is filtered by the `Where` clause, yielding an empty `char[]`. Result: `new string([])` is empty string, producing `counter-count-slot--label` (double-dash). A leading non-alphanumeric character (e.g. `"-Count"`) survives the `Where` clause and creates an id beginning with a hyphen, which breaks CSS `querySelector` selectors. Fix: strip leading/trailing hyphens from the result and fall back to `"field"` when the result is empty.
- [x] [Review][Patch] P4: `ReadBoundedResponseBodyAsync` catches `ArgumentException` but not `NotSupportedException` from `Encoding.GetEncoding` [`src/Hexalith.FrontComposer.Shell/Infrastructure/EventStore/EventStoreQueryClient.cs`]. On trimmed or WASM runtimes, `Encoding.GetEncoding(charset)` throws `NotSupportedException` for valid but unregistered charset names. This propagates unhandled and surfaces as a query failure. Fix: add `catch (NotSupportedException)` alongside the existing `catch (ArgumentException)`.

#### Deferred

- [x] [Review][Defer] W1: AC5 FcDevModeAnnotation nested button traps — `FcDevModeAnnotation.razor` was not modified in this story; no executable bUnit test proves absence of nested interactive role traps. Split to Story 11.3/11.4 per spec row classification. — deferred, pre-existing
- [x] [Review][Defer] W2: AC17 Counter stale/accessibility fixture gaps — Counter sample lacks an explicit stale-contract-drift path test and an accessibility-warning path test; row matrix classifies these as accepted-with-risk with historical evidence. — deferred, pre-existing
- [x] [Review][Defer] W3: AC18 `FcDevModeToggleButton.razor.css` missing `forced-colors` and `prefers-reduced-motion` media queries — no High Contrast visibility guard for the FluentIcon. Row matrix accepts per D16 (representative evidence only). — deferred, pre-existing
- [x] [Review][Defer] W4: AC28 WASM/Blazor Auto `IHostEnvironment` registration remaining gaps — factory-registered env is now supported; ImplementationType-based WASM registrations (`AddSingleton<IHostEnvironment, BlazorWebAssemblyEnvironment>()`) are silently skipped by `FindRegisteredEnvironment`. Split to Blazor Auto hosting story per spec. — deferred, pre-existing
- [x] [Review][Defer] W5: AC32 acceptance matrix boilerplate — all 62 accepted rows share identical evidence text; per-row likelihood and downstream consumer impact are not individually recorded per AC3. Evidence quality issue, not a code correctness defect. — deferred, pre-existing
- [x] [Review][Defer] W6: `ReconciliationSweepReducer` expired-marker comment vs code mismatch — comment says "skip markers already expired at insertion" but no pre-insertion expiry check is present; this is a pre-existing comment accuracy issue not introduced by this story. — deferred, pre-existing
- [x] [Review][Defer] W7: `RazorEmitter.AppendNode` blank-AnnotationKey dedup display — when `AnnotationKey` is blank the cycle-detection key uses `SourceComponentIdentity` but the emitted comment still shows blank parentheses; cosmetic dev-tool output only. — deferred, pre-existing
- [x] [Review][Defer] W8: `DevModeOverlayController.Register` `_nodes` write outside `_selectionLock` — dict write is not atomic with the selection refresh; Blazor scoped service design intent and not a production correctness defect in the single-threaded circuit model. — deferred, pre-existing
- [x] [Review][Defer] W9: `TryGetClrGenericStarterName` regex `[A-Za-z0-9_.]` excludes `+` and version tokens from assembly-qualified nested-type arguments — CLR `Type+Nested` syntax is not matched; falls back to `{baseName}_Arity{n}` which is consistent with accepted constraints. — deferred, pre-existing
