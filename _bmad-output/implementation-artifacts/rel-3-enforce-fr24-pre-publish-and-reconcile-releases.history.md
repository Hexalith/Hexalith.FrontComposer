---
archived_from: rel-3-enforce-fr24-pre-publish-and-reconcile-releases.md
archived_on: 2026-08-09
reason: Split from active residual plan (bmad-build token gate [S]); historical epic body + Option 1 full AC amend retained here.
active_spec: rel-3-enforce-fr24-pre-publish-and-reconcile-releases.md
---

# ARCHIVED — REL-3 historical epic body (not the active implementer surface)

The active residual plan lives in `rel-3-enforce-fr24-pre-publish-and-reconcile-releases.md`.
Do not implement from this archive unless a human reopens a deferred goal.
...
created: 2026-07-15
updated: 2026-08-09
amended: 2026-08-09 (Option 1 — unsigned-author / NuGet.org repository-signature FR24 model; residual review re-plan)
...
signingModel: _bmad-output/implementation-artifacts/spec-remove-author-signing-requirement.md
status: draft
...
Status: draft (2026-08-09 re-plan after Option 1 intent resolution — unsigned FR24 model + residual review fixes).

Approval: approved by Administrator on 2026-07-15. Intent renegotiated 2026-08-09 (Option 1)
to align with done `spec-remove-author-signing-requirement` and REL-5.

Status correction (2026-08-03, amended 2026-08-09): repository-local exact-artifact
orchestration and non-publishing validation exist and are under residual review. Author
signing / timestamp / `nupkgs-signed` are not REL-3 requirements. A real governed release,
NuGet.org owner signer-policy confirmation, upstream/protected authorization seam, durable
GitHub Release evidence closure, and REL-AI-1 closure remain transferred REL-5 / GOV-1
conditions and are not implied by this story's implementation status.
...
**Problem:** REL-3's approved text still conflicted with the done unsigned-author model, and the
2026-08-09 adversarial review found residual fail-closed gaps in publication verify, AD-15
handoff, orchestrator docs/contracts, and overstated governance pins.

**Approach:** Keep the unsigned-author / NuGet.org repository-signature FR24 contract; close only
the residual fail-closed and verification gaps named below without restoring author signing.

## Boundaries & Constraints

**Always:** Preserve pack-once sealed unsigned `nupkgs/*` candidates; sealed manifest + readiness
authorization before push; independent post-publication GitHub checksum + NuGet.org repository-
signature + normalized content equality (exclude only root `.signature.p7s`); REL-4/Builds
publication freeze contract; fail-closed partial-publish incidents; legacy signed-v2 audit
readability.

**Ask First:** Real release dispatch/publication; NuGet.org owner signer-policy changes; secret
or variable deletion; branch/commit/push/PR; submodule mutation; reintroducing author signing.

**Never:** Require author-signing certificate/password/timestamp, `dotnet nuget sign`, trust-store
mutation, or `nupkgs-signed`. Never weaken raw GitHub checksum verification, treat NuGet.org
repository signature as an author signature, ignore archive differences beyond `.signature.p7s`,
or claim REL-AI-1 closed without REL-5 durable evidence.

## I/O & Edge-Case Matrix

| Scenario | Input / State | Expected Output / Behavior | Error Handling |
|----------|---------------|----------------------------|----------------|
| Publish verify | GitHub Release `immutable=false` or empty assets | `verify-publication` fails | Fail closed before AD-15 success |
| AD-15 published | `published=true` but candidate/manifest missing | Job fails (exit 1), no soft-deferred seal | No exit-2 success path |
| Stale readiness | Sealed readiness still authorized but classify would now block | Documented contract: either re-classify before push or sealed-readiness-only with explicit pin | Must not silently drift |
| Disposition | Completed `release / release` job topology | `governed-attempt=true` under runtime/fixture proof | Misclassify fails test |
| Retroactive auth | Downloaded readiness `publish_authorized=false` | Verifier fails closed before compliant ledger | Incident recorded |
...
- **Attestation before classification:** the pre-publication chain must attest provenance over the
  exact unsigned candidates and bind the bundle path into the manifest before sealing and
...
- **Upstream scope:** the Hexalith.Builds dependency is the full BUILD-REL-1 opt-in governed
  contract (environment, attestation permissions, candidate phase, bundle handoff, no-repack
  publish). Author-signing certificate/password/timestamp forwarding is not a FrontComposer
  production requirement as of 2026-08-04.
- **Operational authority split:** NuGet.org unsigned-upload policy confirmation, protected
  production approval, first-release authorization, and REL-AI-1 closure are executed by the
  Release Owner under REL-5; REL-3 retains the exact-artifact technical implementation and
  verification tooling.
- **Signing model (2026-08-09 Option 1):** Author signing, PFX/password, RFC 3161 author
  timestamp, and `nupkgs-signed` are out of REL-3 scope. Supply-chain identity is sealed
  unsigned GitHub candidates plus NuGet.org repository-signature verification and normalized
  content equality (exclude only root `.signature.p7s`), per
  `spec-remove-author-signing-requirement`.
...
5. Given the exact `.nupkg` candidates intended for publication, when preparation seals them, then
   every candidate remains an unsigned-author package (no author signature / no pre-publication
   `dotnet nuget sign`); signed or unsafe NuGet archive content fails closed. Author-signing
   credentials, passwords, and RFC 3161 author timestamps are not required.
...
7. Given the evidence bundle is prepared, when checksums run, then they cover packages, symbols, SBOM,
   inventory, test results, consumer-validation results, and supporting evidence (no author-signing
...
10. Given any evidence is blocked, invalid, unverified, incomplete, or unauthorized, when
...
11. Given publication is authorized, when NuGet and GitHub Release publishing runs, then both receive
    the same sealed unsigned candidate bytes covered by the verified manifest (`nupkgs/*.nupkg` and
    matching symbols); the publisher does not rebuild, repack, author-sign, or substitute artifacts.

12. Given the initial GitHub Release is created, when assets are inspected, then packages, symbols,
    SBOM, inventory, checksums, tests, consumer results, sealed/verified manifest, and release
    readiness are durable release assets on an immutable release; a short-retention Actions artifact
    alone fails the criterion.

13. Given publication completes, when independent post-publication verification runs, then it downloads
    the NuGet and GitHub assets, verifies each NuGet.org repository signature, compares GitHub asset
    hashes to the sealed manifest, and proves normalized package content equality with the unsigned
    GitHub candidate excluding only the root `.signature.p7s` entry NuGet.org adds.
...
15. Given releases created under G1, when historical reconciliation runs, then the controlled ledger
    records at least v3.2.1 and v3.2.2 with release/run URLs, package inventory, observed
    signing/repository-signature state, manifest/readiness state, consumer evidence, durable paths,
    disposition, owner, and remediation.

16. Given governance tests run, when they inspect the release model, then they reject G1
    record-and-proceed, missing `--require-publishable`, reconstructed-package authorization,
    non-authorized publish paths, post-publication authorization, and any active requirement for
    author signing or a `nupkgs-signed` tree.

17. Given a real release completes, when the Release Owner reviews REL-AI-1, then REL-AI-1 closes only
    if every FR24 artifact is durable, `classification=ready`, `publish_authorized=true`, GitHub
    bytes match the authorized manifest, and NuGet.org repository signatures plus normalized content
    equality hold (REL-5-owned execution).

18. Given unsigned candidate packages exist, when provenance attestation runs in the governed release
...
- `.github/workflows/release.yml` `verify-publication` jq ~353-356 -- currently non-draft + assets array only; must match evidence immutable + non-empty.
- `.github/workflows/release.yml` `emit-verification-handoff` ~445-518 -- `assets.json` always `[]`; candidate download `|| true`; exit 2 soft-deferred on missing manifest when published.
- `.github/workflows/release-evidence.yml` ~352-388, ~463-464 -- KEEP: `immutable==true`, non-empty assets, `--require-immutable`, `publish_authorized` gate.
- `eng/release_contract.py` `validate_publication` ~276-292 -- immutable only with `--require-immutable`.
- `eng/dependency_handoff.py` `validate_release_handoff` ~326-335 / `create-release` ~650-663 -- sealed handoff rejects null manifest; GraphError → exit 2 deferred.
- `eng/release_prepublish.py` docstring 1-31; `cmd_prepare`/`cmd_bundle` via `release.yml` prepare-candidate; `.releaserc.json` 20-22 `restore`/`verify-prepared`/`publish`.
- `eng/release_prepublish.py` `cmd_verify_prepared` 650-664 / `cmd_publish` 718-738 -- sealed readiness read only (no re-classify today).
- `eng/release_prepublish.py` `_validate_unsigned_candidates` 176-203, 766-772 -- KEEP unsigned pack-once contract.
- `tests/.../CiGovernanceTests.cs` `ReleaseEvidenceWorkflow_IndependentlyVerifiesPublishedArtifacts` ~1364+ -- disposition string pin only.
- `tests/.../ReleaseModelGovernanceTests.cs` `VerificationWorkflow_DownloadedEvidence_CannotAuthorizeReleaseRetroactively` ~149-166 -- source-text pin only.
- `tests/ci-governance/stage_release_state.py` -- reuse for orchestrator runtime negatives; extend pattern for disposition/auth if extracted.

## Required Artifact Invariant

```text
Pack once (unsigned-author candidates under nupkgs/)
  → validate inventory, tests, and package consumers
  → generate SBOM and symbol evidence
  → attest build provenance over the exact unsigned packages (bundle bound into the manifest,
  ...
  → publish those same authorized unsigned bytes
  → verify published GitHub checksums + NuGet.org repository signatures and normalized content
```

## Implementation Boundary

- **Hexalith.Builds:** its owner must land the BUILD-REL-1 opt-in governed release contract —
  protected release environment, `id-token: write` + `attestations: write` scoped to the governed
  steps, a version-aware pre-publication candidate phase, `actions/attest-build-provenance` over
  the exact candidates with the bundle path handed to manifest finalization, and no-repack
  publication — per the G2 request document. Author-signing certificate secrets are not a
  FrontComposer production requirement. Do not directly modify or commit the shared submodule
  from FrontComposer.
- **FrontComposer:** owns the semantic-release lifecycle command, artifact/evidence paths,
  classification, same-byte publication of sealed unsigned candidates, GitHub assets, downloaded-
  artifact verification (repository signature + content equality), tests, and ledger tooling.
...
- [ ] T1 — Confirm the release freeze and upstream dependency. **(owner-blocked residuals; freeze
  ...
  - [ ] Record the Hexalith.Builds owner-approved issue/story URL and accepted revision in the G2
        ...
        attestation permissions, candidate phase, bundle handoff), not a signing-secret-only
        request (filing itself is REL-5 owner work).
  ...
  - [x] Confirm author-signing certificate/password/timestamp secrets are **not** production
        prerequisites (2026-08-04 / Option 1). *Historical 2026-07-18 note about awaiting
        BUILD-REL-1 signing-secret forwarding is superseded.*
...
- [x] T2 — Build a repository-owned pre-publication orchestration command.
  ...
  - [x] Run inventory, tests, package-only consumer validation, SBOM, checksums, manifest
        preparation/sealing/verification, and publishable classification in order over unsigned
        candidates (no author-sign phase).
  - [x] Bind the provenance attestation-bundle path over the exact unsigned candidates into the
  ...
  - [x] Never require or log author-signing material; keep secrets out of artifacts, logs,
        summaries, and manifest fields.
...
- [x] T3 — Make semantic-release publish only authorized artifacts.
  - [x] Wire `verifyReleaseCmd`/`prepareCmd`/`publishCmd` to restore → verify-prepared → publish
        over the workflow-prepared sealed candidate (pack/classify runs in `prepare-candidate`).
  - [x] Make `publishCmd` re-verify the sealed manifest/readiness immediately before pushing.
  - [x] Push only sealed unsigned manifest-authorized `nupkgs/*.nupkg` paths and matching `.snupkg`
        paths.
...
- [x] T5 — Reverse G1 governance and add negative coverage.
  - [x] Prove signed or unsafe candidate archives fail closed under the unsigned-author contract
        (supersedes the historical missing-credential / invalid-timestamp negatives from the
        author-signing era).
  - [x] Require `classify-release --require-publishable` and authorized publish paths.
  - [x] Prove the publisher cannot repack, substitute, or consume non-manifest paths.
  ...
  - [ ] **Residual (2026-08-09):** strengthen disposition-classifier and post-publish
        `publish_authorized` coverage beyond source-text pins (see T8).

- [ ] T6 — Validate without publishing, then obtain real-release evidence. **(local validation
      complete under historical author-signing chain; re-validate unsigned path as needed;
      real-release evidence is REL-5-owned and REL-AI-1 stays open)**
  - [x] Run the complete preparation/classification path in a non-publishing context.
        *2026-07-18 historical evidence under the then-current signing chain. Live contract is
        unsigned-author as of 2026-08-04; residual T8 re-pins governance to that contract.*
...
- [ ] T8 — Close 2026-08-09 residual review gaps under the unsigned FR24 model.
  - [ ] `.github/workflows/release.yml` -- align `verify-publication` with `release-evidence.yml`:
        require `immutable==true`, non-empty assets, and `release_contract.py publication
        --require-immutable`.
  - [ ] `.github/workflows/release.yml` -- fail-close `emit-verification-handoff` when
        `published=true`: do not soft-succeed on missing candidate/manifest; populate
        `assets.json`; treat missing sealed-manifest coords as hard failure (exit 1), not deferred
        exit 2.
  - [ ] `eng/release_prepublish.py` -- rewrite module docstring to match workflow `prepare`/`bundle`
        + `.releaserc.json` `restore`/`verify-prepared`/`publish` under the unsigned-author model.
  - [ ] `eng/release_prepublish.py` + governance pins -- decide and document whether
        `verify-prepared`/`publish` re-run `classify-release` or keep sealed-readiness-only; pin
        the chosen contract.
  - [ ] Governance tests -- add runtime or fixture-driven proof for disposition
        `governed-publication-attempt` and the evidence-workflow `publish_authorized` gate beyond
        source-text `ShouldContain` pins.
...
- Do not reintroduce author signing. NuGet.org repository signatures are verified post-publication;
  they are not author signatures and do not replace sealed GitHub candidate checksums.
- Preserve unrelated worktree changes.

## Verification

- Governance tests for workflow/config shape and all negative authorization paths, including
  disposition topology and post-publish unauthorized-readiness fail-closed behavior.
- Evidence-helper unit/fixture tests for exact paths, checksums, manifest verification, publishable
  classification, downloaded-artifact comparison (repository signature + content equality), and
  incident records.
- Package-only consumer restore/build tests for Contracts-only and Shell/UI boundaries.
- `dotnet nuget verify --all` over downloaded published packages (repository signature path).
- Non-publishing end-to-end orchestration run using the actual inventory and semantic-release version
  contract under unsigned candidates.
- One CI-authoritative real release with durable assets and post-publication verification (REL-5).

## Definition of Done

- [ ] All acceptance criteria and repository-local tasks (including T8 residuals) are complete.
- [ ] The shared-workflow dependency or approved contingency is recorded with durable evidence
      (GOV-1 / REL-5 for accepted Builds revision).
- [ ] No G1 record-and-proceed path remains in the publish-capable lifecycle.
- [ ] No active path requires author signing or `nupkgs-signed`.
- [ ] v3.2.1 and v3.2.2 are reconciled in the ledger.
- [ ] REL-AI-1 closure is supported by durable real-release evidence via REL-5, or remains open
      with the exact blocker.

## Spec Change Log

- 2026-08-09 (intent_gap / Option 1): Human chose amend-REL-3-to-unsigned over restoring author
  signing. Amended ACs 5/7/10–13/16–18, Required Artifact Invariant, Implementation Boundary,
  T1–T5 narrative, guardrails, verification, and status correction to match done
  `spec-remove-author-signing-requirement` and REL-5. Added frozen residual Intent + T8 for
  review findings (immutable verify-publication, AD-15 fail-closed, docstring, classify contract,
  runtime disposition/auth pins). Known-bad state avoided: reviewing/closing REL-3 while ACs
  still demanded author signing against unsigned code. KEEP: pack-once sealed unsigned
  `nupkgs/*`; sealed manifest + readiness before push; release-evidence immutable + repository-
  signature/content equality; REL-4/Builds freeze contract; no author-sign reintroduction;
  prior 2026-07-18 hardening (symbol_checksum, publisher path confinement, dry-run-clean-exit
  runtime negatives) remains unless T8 explicitly replaces it.

## Implementation Record (2026-07-18; historical — pre Option 1 unsigned amend)
...
**Note:** Passages below that describe `nupkgs-signed`, author signing, or local-chain signing
reflect the 2026-07-18 implementation state and are superseded by the 2026-08-04/09 unsigned
model for current contract truth.
...
- `verify-publication` must require immutable + non-empty assets like `release-evidence.yml`.
  [`release.yml` verify-publication jq]
- AD-15 must hard-fail when `published=true` without sealed-manifest coords / assets.
  [`release.yml` emit-verification-handoff]
- Orchestrator docstring must match prepare/bundle vs restore/verify-prepared/publish.
  [`release_prepublish.py:1`](../../eng/release_prepublish.py#L1)
- Classify-or-sealed-readiness contract must be explicit and pinned.
  [`release_prepublish.py` cmd_verify_prepared / cmd_publish]
- Disposition + retroactive-auth gates need more than source-text pins.
  [`release-evidence.yml` disposition / publish_authorized]

**KEEP — unsigned exact-artifact invariant**

- Unsigned pack-once candidates under `nupkgs/`.
  [`release_prepublish.py` `_validate_unsigned_candidates`]
- Independent verifier: immutable release + repository signature + content equality.
  [`release-evidence.yml`]
- Publish only sealed manifest-authorized bytes.
  [`release_prepublish.py` `cmd_publish`]

**Historical (2026-07-18 review — already applied)**

See Review Findings below; do not re-open fixed symbol_checksum / dry-run-clean-exit / publisher
audit items unless T8 regression tests fail.
