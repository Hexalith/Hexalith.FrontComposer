...
- **.NET 10** — `global.json` pins SDK `10.0.401` with `rollForward: latestPatch`; root props enable
...
- **Source-resource SDK compatibility:** full Debug/Aspire source topology also requires SDK
  ...
  PolymorphicSerializations roots still pin that SDK. The repository root/default remains `10.0.401`;
...
- **Shared-catalog Governance (GOV-1, 2026-07-19):** `Gate 2b`'s catalog-compatibility checks
  ...
  FrontComposer-local exact graph diff, root-subsumed affected-module proof/materialization, and
  release-manifest verification are implemented. Push CI emits a truthful exact-source proof binding
  ...
  Builds commit `3f0e3595be693fce56a37648c0bd0f89390f5fd3` with the identical execution input.
