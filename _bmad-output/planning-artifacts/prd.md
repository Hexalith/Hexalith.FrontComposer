...
status: final
product_approval: pending-reapproval (D-9 amendment required after the 2026-09-22 reconciliation)
...
This table is the only authority on what still separates the product from the readiness milestone. States without a newer dated update retain the 2026-09-22 inspection point. D-9 may not read "Blocks: None" while any row is open. Evidence gates are closed by an artifact; approval gates are closed by a named approver citing the artifact they read. Anything not listed here is one of the following: a regression baseline or closed record (§5.0), an explicitly non-gating residual (§12.2), or an assumption (§13). These non-gating items cannot change milestone truth; each retains its owner and closure condition.

**Evidence gates**

| Gate | Pass condition | Owner | Evidence artifact | State |
| --- | --- | --- | --- | --- |
| G-1 Release ledger closure | The Release Owner maintains an append-only, attempt-keyed ledger for every publication-capable attempt, including failed, cancelled, deferred, no-releasable, and partial-publication paths. v4.1.1 retains its published-byte verification and pending owner hash/sign-off; v4.2.0, v4.3.0, v4.4.0, and v4.5.0 receive downloaded-byte evidence and permanent dispositions without relabelling historical incidents green. Verification reruns append observations rather than replacing an attempt state, and stale "unauthorized" sentences are corrected to current release truth. | Release Owner | `_bmad-output/implementation-artifacts/rel-ai-1-release-evidence-ledger.md`; REL-LEDGER-1 schema/classifier acceptance; REL-LEDGER-2 backfill evidence | Open. v4.1.1 awaits owner sign-off; four later tags have no rows; the ledger state model is not yet accepted. |
| G-2 GOV-1 provenance and external seam | One table names the authoritative Hexalith.Builds SHA per purpose — execution pin `4eb33928…`, current root gitlink `2fba3497…`, owner-accepted BUILD-REL-1 lineage anchor `a8a50859…`, and the future owner-accepted split-reusable revision. The implementation nonconformance register is closed or carries owner-accepted dispositions; authenticated Release and Release-Evidence run IDs prove the AD-13/AD-15 path; Architecture and the Release Owner accept the selected Builds contract; and the GOV-1 sprint action closes. Missing proof cannot satisfy this evidence gate. | Architecture + Release Owner | `.github/workflows/release.yml`; `eng/dependency-graph-policy.json`; `_bmad-output/planning-artifacts/architecture/architecture-gov-1-2026-07-19/nonconformance-register-2026-09-08.md`; authenticated run coordinates; passing GOV-1 revalidation | Open on EXT-BUILDS-1, GOV-B/GOV-E/GOV-SRC-1 implementation, proof, and GOV-ACCEPT-2. The graph/catalog core remains implemented. |
| G-3 EventStore runtime identity | The release candidate must carry one immutable exact-tuple record binding FrontComposer HEAD, EventStore source gitlink, catalog-owned package version, Builds gitlink, provider evidence, AppHost evidence, and artifact hashes. Story 11.24/v1, Story 11.25/v2, and the in-review 3.106/v3 record remain immutable historical or candidate evidence; none is projected onto a later gitlink pair. EVT-ID-1 captures the tuple current at execution and EVT-APP-1 may set `migrationApprovalClaimed: true` only after the EventStore maintainer, FrontComposer maintainer, and Release Owner sign. If no distinct EventStore maintainer owns approval, Product and Architecture first record EVT-XFER-1/OI-18; role equivalence is never inferred. | EventStore maintainer + FrontComposer maintainer + Release Owner; Product + Architecture for any ownership transfer | Current successor active-identity contract and exact-tuple provider/AppHost packet; EVT-APP-1 receipts; optional EVT-XFER-1 record | Open. At the 2026-09-22 inspection point, HEAD selected EventStore `db1e9d73…`, package `3.106.0`, and Builds `2fba3497…`; the v3 candidate binds older gitlinks, has empty receipts, and has `migrationApprovalClaimed: false`. |
| G-6 Named adopter bootstrap proof | Hexalith.Tenants boots through the documented three-call path and renders at least one generated projection and one generated command; the evidence file has a date and candidate SHA. If Product selects Parties as the substitute adopter under D-7, Parties must produce the same proof. Choosing to hold the milestone records why this gate remains open; it does not close it. | Product Owner + selected adopter maintainer | ADOPT-KIT-1; external EXT-ADOPTER-1 evidence at `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.md` and `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.json`; Product decision `_bmad-output/implementation-artifacts/tests/tenants-g6-product-decision-2026-09-26.md` | Closed 2026-09-26 by explicit Product Owner review of the 2026-09-25 independent Tenants run against candidate `05d122005d328ec8f15ecd276058be29730288c2`. The four candidate package identities and archive hashes, runtime identity, Tenants-generated projection and command renders, negative bootstrap checks, and redaction were reviewed. D-7 was not invoked. |
| G-7 MCP leak/oracle audit and acceptance | The SM-4 suite exists as a named Governance-lane test class and covers: byte-identical hidden-vs-absent shapes per caller for tools; list-vs-call and skill-vs-projection oracles; an unregistered resource URI through SDK dispatch; the non-Development ban on `AllowAll*` gates; and endpoint authentication on `MapFrontComposerMcp`. A security reviewer distinct from the PRD author records OI-2's disposition of every remaining disclosure/oracle residual after any gated-list change; implementation evidence cannot substitute for that independent acceptance. Timing side channels are out of scope (FR-19). | Security reviewer distinct from the author + Framework maintainer | MCP-SEC-1 production enforcement; MCP-SEC-2 negative/audit evidence; independent MCP-APP-1 sign-off | Closed 2026-09-26 for v2 candidate manifest `942847d17eee54061bb9b7f7bde254b361abf802dbdb70ce21c460136c5aa00b`; independent signoff: `_bmad-output/implementation-artifacts/tests/mcp-g7-security-review-2026-09-26.md`. Host deployment remains separately owned. |
| G-8 GOV-1 publication-safety conformance | Product/Architecture/Release accept the spine's adopted AD-19 boundary: a secretless, read-only builder produces one authenticated run-bound publication-candidate artifact and a distinct protected publisher treats packages as data, executes only pinned owner-controlled code, and never checks out or executes candidate source. Candidate-controlled code receives no publication secret, write scope, OIDC/attestation authority, or signing material. Canonical `architecture.md`, FC-DEP-1, the GOV-1 story, and the G2 request remain aligned with the spine. The exact spine-defined GOV-1 split implementation gate accepts `frontcomposer.gov1-split-conformance.v1` only after all closure rows, authenticated runs/checks, five review lenses, the unchanged evidence PR, and the distinct approval-projection PR pass. | Architecture + Release Owner + Product Owner | EXT-BUILDS-1; GOV-B through GOV-I; GOV-J unchanged-candidate review; GOV-ACCEPT-1/2; adopted AD-19 spine and canonical conformance packet | Open. External acceptance, all implementation/evidence slices, incident readiness, and owner approvals remain open. No current release is FR-24-compliant on protected approval alone. |

**Approval gates**

| Gate | Pass condition | Approver | Must cite | State on 2026-09-22 |
| --- | --- | --- | --- | --- |
| G-4 Product re-approval | Product records a dated D-9 amendment approving the exact PRD and addendum SHA-256 pair after OI-4, OI-10, OI-16, and OI-19 close and a refreshed synthesized postfix reviewer gate has no unresolved Critical/High finding. Any later digest change requires a fresh gate report. | Product Owner | FLUENT-APP-1; UX-A through UX-F; DOC-A through DOC-C; refreshed digest-bound review packet; final PRD-APP-1 | Open. The 2026-09-09 review is historical after this reconciliation; no implementation evidence or Product approval is inferred. |
| G-5 Epic 9 acceptance | Product records E9-APP-1 acceptance of the Story 9.8 proof packet as composed/live FC-NIP evidence. Epic 9 delivery and E9-AI-1…E9-AI-6 are closed independently of this approval. | Product Owner | `_bmad-output/implementation-artifacts/tests/9-8-live-acceptance.md` | Open on the Product record; Stories 9.1–9.8, the retrospective, the live proof, and delivery bookkeeping are done. |
...
The v1.0 readiness milestone commits to two outcomes: a **developer framework that is safe to package, consume, test, customize, and evolve** and an **operations shell an operator can trust**. In that shell, fresh rows are marked only from real command outcomes, projection freshness recovers after transport failure, and command state never overstates what the backend confirmed. Epics 9–11 delivered the runtime remediation baseline, but the 2026-09-09 validation reopened release-safety conformance and UX handoff completeness. The remaining work is listed in §0.1: release architecture, implementation, evidence, and ownership under G-1/G-2/G-8; EventStore provenance reconciliation under G-3; and Product approvals under G-4/G-5. G-7 closed on the exact 2026-09-26 MCP candidate.
...
- **UJ-4. Ravi exposes the domain surface to an AI agent.** Ravi hosts the MCP server behind host authentication, registers tenant and resource visibility gates, and allows an agent to discover visible generated tools and projection resources. The agent can call commands only after admission, schema negotiation, argument validation, and server-side injection of controlled fields, and can follow lifecycle across requests through `frontcomposer.lifecycle.subscribe`. **Edge case:** host authentication challenges unauthenticated HTTP requests; in-handler auth, tenant, and hidden-tool failures collapse to the opaque public shapes in FR-19, so a caller cannot tell "hidden" from "absent" for tools. **Non-goal:** the resource catalog (`resources/list`) is static and not tenant-filtered; FR-19 states exactly what it discloses.
...
| --- | --- | --- |
| `tools/list` handler with no agent context, missing tenant, or catalog failure | Successful `ListToolsResult` with an empty `Tools` collection; the mapped HTTP endpoint challenges an unauthenticated request before the handler | An authenticated caller always sees at least the lifecycle tool, so an empty handler list and the HTTP challenge disclose credential validity. Accepted 2026-09-26 in the exact-candidate MCP-APP-1 review. |
...
| `resources/list` | The static generated descriptor catalog (URI, name, title, description of every projection and skill resource), identical for every caller | Not tenant-filtered and not gated. Discloses bounded-context and projection names. Accepted 2026-09-26 in the exact-candidate MCP-APP-1 review. |
...
- `MapFrontComposerMcp` attaches `RequireAuthorization()` to the mapped SDK endpoints. Hosts must configure authentication and authorization middleware and supply the policy and scheme; G-7 tests endpoint metadata and an unauthenticated HTTP request.
- `AllowAllMcpTenantToolGate` and `AllowAllResourceVisibilityGate` are sample/dev escape hatches only. `MapFrontComposerMcp` rejects either resolved gate outside Development before mapping an endpoint; custom gate behavior remains a host composition responsibility accepted in the exact-candidate MCP-APP-1 review.
...
**Consequences:**
- Breaking public-surface changes update baselines, docs, and migration/deprecation plans. The ApiCompat baseline constant is `PUBLISHED_BASELINE_VERSION = "4.4.0"` (`eng/release_compatibility.py`) while v4.5.0 is already tagged; advancing the baseline after each publication, or approving a bounded lag, is OI-11.
...
| FR-29.7 EventStore runtime identity | The identity FrontComposer builds and tests against equals the owner-approved tuple, with live Pact provider evidence at that identity. | — | 11.24 historical record; 11.25 active remediation; Pact reconciliation specs | Story 11.24 done 2026-08-29 as historical authorization; exact-current-tuple recapture and approval are open in Story 11.25 (G-3, D-12). |
...
### 8.2 Readiness Program Status (sprint-status truth, 2026-09-22)

- **Epic 9:** done. Stories 9.1–9.8, E9-AI-1…E9-AI-6, the retrospective, and the 9.8 live proof are completed delivery history. G-5/E9-APP-1 remains open solely for dated Product acceptance and does not reopen the epic.
- **Epic 10:** done; tooling-governance and Testing-redaction outcomes remain reusable evidence.
- **Epic 11:** done for its bounded delivery. Stories 11.0–11.9 and 11.11–11.32 are completed history (there is no 11.10; it was split into 11.11–11.14). Story 11.25 delivered a technical identity capture only; it did not grant migration approval or close G-3. E11R-AI-1 remains open but is re-homed to EVT-ID-1/EVT-APP-1 so later tuple drift does not rewrite Story 11.25. The current Story 11.32 validator red state is new PLAN-INT-2 work, not a retroactive status change.
...
- **SM-1: Adopter bootstrap success** — Hexalith.Tenants, or Parties after a dated D-7 fallback decision, boots through the documented three-call path and renders at least one generated projection and one generated command without bespoke framework plumbing. **Evidence:** `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.md`, `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.json`, and Product decision `_bmad-output/implementation-artifacts/tests/tenants-g6-product-decision-2026-09-26.md`. **State: met 2026-09-26 for the exact 2026-09-25 Tenants run on candidate `05d122005d328ec8f15ecd276058be29730288c2`.** This is render and registration proof; no command outcome or EventStore exchange is claimed. Validates FR-1, FR-2, FR-7, FR-8.
- **SM-2: Release ledger integrity** — every publication-capable attempt has one immutable attempt identity and append-only disposition covering success, block, failure, cancellation, deferral, no releasable artifact, and partial publication. `deferred-no-ci-handoff` is valid only as the sole deferred sentinel and remains a terminal, incident-bearing, permanent disposition; a malformed, duplicate, missing, or unauthenticated handoff is `missing-artifact`. Compliant publications additionally retain publisher-sealed `publish_authorized=true`, exact candidate/GitHub asset hashes, valid NuGet repository signatures with byte-equivalent non-signature normalized ZIP members, inventory/consumer/SBOM evidence, and Release Owner sign-off; incidents are never relabelled green. **Evidence:** `_bmad-output/implementation-artifacts/rel-ai-1-release-evidence-ledger.md` plus the accepted G-8 classifier/schema. **State: unmet** — v4.1.1 awaits sign-off; v4.2.0–v4.5.0 lack rows; the state model is open (G-1/G-8). Validates FR-24, FR-25, NFR-12.
...
- **SM-4: MCP leak/oracle audit** — an independent suite proves, for every request class in FR-19, exactly the guarantee stated there: byte-identical hidden-vs-absent tool shapes per caller; no list-vs-call or skill-vs-projection oracle beyond the disclosed catalog; an unregistered URI through SDK dispatch behaves as stated; failure responses and logs do not echo hidden requested tool names, fingerprints, tenant identifiers, or internal exception text (visible tool names intentionally appear in `tools/list` and caller-visible suggestions); `AllowAll*` gates absent from non-Development hosts; endpoint authentication present. **Evidence:** `McpSecurityGovernanceTests` together with `ToolAdmissionTests`, `McpCommandToolAdapterTests`, `ProjectionReaderTaxonomyTests`, `SchemaNegotiationPrecedenceMatrixTests`, and the candidate-bound G-7 packet. **State: met 2026-09-26 for the exact v2 candidate and independent MCP-APP-1 signoff.** Validates FR-17, FR-18, FR-19, FR-19a.
...
- **Risk: EventStore identity approval is mistaken for the only remaining act.** The current Builds gitlink advanced after the 2026-09-08 evidence. Mitigation: D-12/G-3 preserve the old provenance and require the selected exact-tuple recapture plus named approval before closure.
...
| D-9 | PRD status approval | Product Owner | Resolved 2026-07-05 for D-1…D-8. **Re-approval pending** for the 2026-09-22 reconciled register after OI-4, OI-10, OI-16, OI-19, and a refreshed digest-bound postfix reviewer gate close. Document approval never authorizes publication or the milestone. | G-4. |
...
| D-11 | Shared-catalog compatibility and dependency provenance | Architecture + Product + Release Owner | Ratified 2026-07-19; reconciled 2026-09-22. The owner-accepted BUILD-REL-1 lineage anchor/predecessor is `a8a50859…`; the future owner-accepted split-reusable revision remains EXT-BUILDS-1. The execution pin is `4eb33928…`; the current root gitlink at the reconciliation point is `2fba3497…`; prior evidence retains its exact provenance. The graph/catalog core remains implemented, but GOV-1 implementation conformance is open. `eng/dependency-graph-policy.json` is executable authority; GOV-SRC-1 owns future source drift and GOV-ACCEPT-2 remains the separate Product/Release acceptance. | G-2, G-8. |
| D-12 | EventStore runtime identity | EventStore maintainer + FrontComposer maintainer + Release Owner | Reconciled 2026-09-22. Story 11.24/v1 and Story 11.25/v2 are completed immutable history; the in-review v3 record is an evidence candidate, not migration approval. The repository then selected EventStore `db1e9d73…`, package `3.106.0`, and Builds `2fba3497…`, so no prior tuple is silently current. EVT-ID-1 must capture the exact tuple at execution and EVT-APP-1 may set `migrationApprovalClaimed: true` only after all required owners sign. If approval becomes FrontComposer-owned, Product and Architecture first record EVT-XFER-1/OI-18; role equivalence is never inferred. | G-3. |
...
| D-14 | MCP schema side-effect classes, permissive gates, resource disclosure | Architecture + Security reviewer | New 2026-09-08: `Exact`, `CompatibleAdditive`, `CompatibleWarning` are approved side-effect-allowed classes; all other kinds block. `AllowAll*` gates are forbidden in non-Development hosts; the two shipped gate types are rejected at mapping. Custom gate semantics remain host-owned. The `resources/list` catalog disclosure, unregistered-URI SDK error, and `tools/list` credential signal were accepted for the exact v2 candidate by an independent reviewer on 2026-09-26 (OI-2). | G-7 (OI-3); OI-2 sign-off. |
...
**Routed with named owners:** the Hexalith.Builds `BUILD-REL-1` lineage anchor/predecessor was accepted 2026-08-08, while EXT-BUILDS-1 and FrontComposer acceptance remain routed through G-2/G-8. Current EventStore exact-tuple evidence and migration approval are routed through EVT-ID-1/EVT-APP-1 under G-3. Publication privilege and incident posture are routed through D-16/G-8.
...
**Accepted residuals:** DW-679 and the 9.8 AppHost-fallback deferred items; MCP timing side channels out of scope. The FR-19 credential signal, static catalog, SDK absent-resource distinction, and host composition limits were accepted for the exact v2 MCP candidate on 2026-09-26 (MCP-APP-1).

### 12.2 Open Items And Approved Work Mapping

The 2026-09-22 approved course correction classifies each item as implementable (I),
approval/evidence (A), or a non-sprint external dependency (X). Aliases are stable
handoff identifiers for the create-epics-and-stories workflow, not final story numbers.
No open item is closed merely because it has a target.

| ID | Class and target | Owner | Unblock condition | Gate | State |
| --- | --- | --- | --- | --- | --- |
| OI-1 | A — E9-APP-1 | Product Owner | Dated acceptance cites the Story 9.8 proof. Epic 9 delivery bookkeeping is already closed independently. | G-5 | Open |
| OI-2 | A — MCP-APP-1 | Independent security reviewer | Dated sign-off after MCP-SEC-1/2, covering every remaining FR-19 disclosure/oracle and host-authentication requirement. | G-7 | Closed 2026-09-26 for v2 candidate `942847d17eee54061bb9b7f7bde254b361abf802dbdb70ce21c460136c5aa00b`; signed disposition in the G-7 security-review packet. |
| OI-3 | I — MCP-SEC-1 and MCP-SEC-2 | Framework + Security QA | Non-Development allow-all use fails closed; endpoint authentication, SDK unregistered-URI behavior, isolation, redaction, leak/oracle checks, and a named SM-4 audit class pass. | G-7 | Closed 2026-09-26 for the v2 candidate: named audit 6/6, MCP suite 429/429, Governance lane 15/15. |
| OI-4 | A — FLUENT-APP-1 | Product Owner + Architect | D-13 accepts an exact catalog identity or records a dated alternative with revalidation triggers. | G-4 | Open |
| OI-5 | I — ADOPT-KIT-1; X — EXT-ADOPTER-1; optional A — ADOPT-APP-1 | Product Owner + selected adopter maintainer | FrontComposer publishes the deterministic kit and the selected adopter publishes dated, candidate-bound three-call/projection/command proof. A Parties substitute requires D-7 approval. | G-6 | Closed 2026-09-26: Story 12.1 kit delivered; Tenants EXT-ADOPTER-1 evidence at `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.md` and `references/Hexalith.Tenants/_bmad-output/implementation-artifacts/tests/tenants-bootstrap-acceptance.json` accepted in `_bmad-output/implementation-artifacts/tests/tenants-g6-product-decision-2026-09-26.md`. ADOPT-APP-1 was not invoked. |
| OI-6 | I — PLAN-INT-1 | Product Owner + Architect | The five canonical artifacts agree on Epic 9/11 history, residual work, FR-30, and G/OI mappings. | — | Closed 2026-09-22 by this approved reconciliation |
| OI-7 | I — GOV-SRC-1 | Architect | PRD, architecture, FC-DEP-1, G2 request, GOV-1 source, current gitlinks, and provenance identities agree; drift reopens the item. | G-2, G-8 | Open on current cross-source drift |
| OI-8 | A — REL-LEDGER-1; I — REL-LEDGER-2 | Release Owner + Architect | Accept the state model and pending v4.1.1 owner hash/sign-off; append evidence-backed rows for v4.2.0 through v4.5.0 and any later release without relabelling incidents. | G-1, G-8 | Open |
| OI-9 | A — REL-A3-APP-1 | Release Owner | Dated accept/revise/reject decision confirms the sample host is local/e2e only or updates §4/§11. | — | Open |
| OI-10 | I — DOC-C, or A — dated Product exception | Framework maintainer + Product Owner | Contingency documentation names the exact catalog-owned Fluent identity, or the exception records owner, expiry, and revisit trigger. | G-4 | Open; documentation still cites rc.2 while the catalog selects rc.5 |
| OI-11 | I — REL-BASE-1; A only for intentional lag | Release Owner | Advance `PUBLISHED_BASELINE_VERSION` from 4.4.0 to the latest evidence-backed published release, currently v4.5.0, or approve a bounded lag with rationale and expiry. | — | Open |
| OI-12 | A — GOV-ACCEPT-1 | Product Owner + Release Owner + Architect | Dated acceptance cites AD-19, the caller, production halt, deny-only emergency stop, and no-exception posture. | G-8 | Open |
| OI-13 | A — GOV-ACCEPT-2 | Product Owner + Release Owner | Dated acceptance cites exact reconciled source digests and preserves all publication-safety invariants. | G-8 | Open |
| OI-14 | X — EXT-BUILDS-1; I — GOV-B through GOV-H | Framework + Builds maintainers | The owner-accepted split revision exists; all eight closure bundles have authenticated evidence; no unresolved Critical/High item remains; delayed activation selects the accepted mode. | G-2, G-8 | Open |
| OI-15 | I/evidence — GOV-I | Release Owner + Security | `docs/release-incident-response.md` is approved and a dated tabletop proves acknowledgement, containment, immutable evidence, credential handling, external-effect inventory, recovery, and re-enable rules. | G-8 | Open |
| OI-16 | I — UX-A through UX-F | Product + UX + Framework + QA | Canonical state, focus, validation, lifecycle, fresh-row, responsive/accessibility, and testing-helper matrices have implementation and deterministic/manual evidence with no unresolved Critical/High finding. | G-4 | Open |
| OI-17 | A/evidence — GOV-J | Architect + Release Owner + Security | Deterministic checks and five review lenses pass on one unchanged authenticated candidate; every finding is dispositioned. | G-2, G-8 | Open |
| OI-18 | A — EVT-XFER-1, conditional | Product Owner + Architect + Release Owner | If invoked, a dated transfer names the receiving role, required evidence, trigger, and milestone effect before migration approval. The transfer itself does not close G-3. | G-3 | Open if ownership transfer is required |
| OI-19 | I — DOC-A and DOC-B | Framework maintainer + Documentation owner | Component/index/status/toolbar/tab and migration/catalog/classification parity tests pass with no unclassified public surface or guide. | G-4 | Open |

The detailed independently completable story boundaries and dependency ordering are
approved in `sprint-change-proposal-2026-09-22.md` §7. Final numeric story identifiers
are allocated only by the create-epics-and-stories workflow.
