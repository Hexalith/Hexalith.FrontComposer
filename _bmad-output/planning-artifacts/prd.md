...
| G-3 EventStore runtime identity | The approved course-correction target is EventStore `3.103.0` / source `059f6a89…` / current Builds gitlink `a32cb422…`, proven by a fresh exact-tuple provider-plus-AppHost capture rather than a semantic-compatibility exception. Story 11.24 and `frontcomposer-eventstore-approved-runtime-identity-v1.json` remain immutable historical authorization; Story 11.25 creates the successor active-identity record. The successor has `migrationApprovalClaimed: true`, is signed by the EventStore maintainer, FrontComposer maintainer, and Release Owner, and binds the exact evidence coordinates and hashes. If no distinct EventStore maintainer owns this approval, Product and Architecture must first record OI-18 as a separate dated ownership transfer, remove EventStore from the required approver set, and identify the accountable FrontComposer role; the PRD cannot infer equivalence from the signer. | EventStore maintainer + FrontComposer maintainer + Release Owner; Product + Architecture for any ownership transfer | Successor active-identity contract; `_bmad-output/implementation-artifacts/evidence/pact-provider-reconciliation/`; Story 11.25 evidence; D-12/OI-18 reconciliation record | Open on exact-tuple recapture and named approval. The 2026-09-12 course correction selected recapture at Builds `a32cb422…`; the 19/19 Pact interactions and AppHost smoke passed previously at `35c3d1e5…` Builds provenance on 2026-09-08 and remain supporting, not current-tuple, evidence. |
...
- **Epic 11:** Stories 11.0–11.9 and 11.11–11.24 are completed delivery history (there is no 11.10; it was split into 11.11–11.14). Story 11.24 is the immutable historical EventStore authorization record, not the active release target. The approved 2026-09-12 retrospective-remediation extension adds Stories 11.25–11.32: current identity/evidence first, three immediate gate repairs, an acceptance checkpoint, four runtime/evidence-hardening stories, and final acceptance. `epic-11` remains `in-progress` until that extension and final acceptance close.
...
| D-12 | EventStore runtime identity | EventStore maintainer + FrontComposer maintainer + Release Owner | Amended 2026-09-12 by the approved Epic 11 course correction. The active target is EventStore `3.103.0` / source `059f6a89…` / current Builds `a32cb422…`. Exact-tuple provider-plus-AppHost recapture is selected; no semantic-compatibility exception or rollback is approved. Story 11.24 and identity v1 remain immutable historical authorization, and the 2026-09-08 capture at Builds `35c3d1e5…` remains prior compatibility evidence. Story 11.25 creates the successor active record and may set `migrationApprovalClaimed: true` only after the EventStore maintainer, FrontComposer maintainer, and Release Owner sign. If approval is to become FrontComposer-owned, Product and Architecture first record the separate OI-18 ownership transfer; role equivalence is never inferred. | G-3. |
