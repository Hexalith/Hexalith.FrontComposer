...
- Story 9.1 originated the base payload record `_bmad-output/contracts/fc-nip-row-identity-producer-contract-2026-07-04.md` on 2026-07-04; its decision was approved and the record updated on 2026-07-05. Story 9.3 approved its successor in `_bmad-output/contracts/fc-nip-command-target-identity-contract-2026-08-12.md`. The 2026-08-11 retrospective rejected Story 9.2 evidence as proof of composed behavior; Stories 9.4-9.8 must implement one terminal producer boundary, observable scope-safe state, atomic per-row first-wins behavior, and composed/live acceptance.
...
- **Dependency policy:** one closed, versioned FrontComposer-owned policy defines trusted repository identities/paths, semantic profiles, affected-module argv/evidence-only dispositions, CI/Release/post-release evaluator authorizations, and limits. PR evaluation uses the exact base-commit policy and push evaluation the exact non-zero before-commit policy for both graphs; candidate policy/workflow changes cannot authorize themselves and activate only from a later base, apart from the one-time frozen, digest-approved bootstrap. Missing/unknown mappings, profiles, commands, objects, selected catalogs, or evaluator closures fail closed. Exact commits and raw fingerprints are provenance, not compatibility allowlists.
...
- **SM-2a: Dependency provenance** — every publish-capable release seals and live-verifies the complete defined depth-1/2 `hexalith.dependency-graph.v1`, immutable active policy, policy-authorized static CI/release evaluator provenance, authenticated CI handoff, and original-candidate Release verification handoff. Compatible pointer advances pass semantic-profile and affected-module gates without product-test commit/fingerprint allowlist edits; malformed, incomplete, drifted, unapproved, over-limit, legacy, or second-hop-substituted evidence cannot authorize publication. Validates FR-24, NFR-12, and NFR-13.
...
- **Risk: Workflow success is confused with release readiness.** A green evidence workflow can contain `classification=blocked` and `publish_authorized=false`, as observed for v3.2.2; mutable or merely self-recorded reusable/action refs can also execute code outside the reviewed trust root. Mitigation: authenticate the exact successful main-push CI run/head and canonical dependency handoff; require each static caller/reusable/composite-action closure to match an immutable base/before policy authorization; seal its canonical definition digest; include it in fallback invalidation; emit an always-present Release verification handoff preserving the original candidate; and fail on any metadata, provenance, manifest, second-hop, or classification mismatch.
...
| D-11 | Shared-catalog compatibility and dependency provenance | Architecture + Product + Release Owner | Resolved and ratified 2026-07-19 by Administrator as Architect + Release Owner: compatibility is semantic-profile plus affected-module Release/NuGet proof; provenance is the exact complete defined depth-1/2 `hexalith.dependency-graph.v1`. The sealed v2 manifest additionally binds the immutable active policy, authenticated CI handoff, and active-policy-authorized static CI/release evaluator/action definition digest. An authenticated Release verification handoff preserves the original CI candidate across the second workflow hop. Collection, Builds-tree extraction, and evaluator closure use ratified fail-closed ceilings; exact commits/catalog fingerprints never become compatibility allowlists. Corrected 2026-08-02: issue 17 closed without the GOV-1 amendment or a qualifying revision, so local graph diff, affected-module proof, evaluator closure, handoff schemas/consumers, manifest v2, and hostile fixtures are unblocked. A reopened issue 17 or successor request plus its accepted immutable revision gates only reusable-workflow integration, end-to-end proof, completion, release eligibility, and unfreeze. `eng/dependency-graph-policy.json` is the sole executable authority for profile values, dispositions, limits, and evaluator authorizations. Contract: `_bmad-output/contracts/shared-catalog-dependency-governance-2026-07-19.md`; spine: `_bmad-output/planning-artifacts/architecture/architecture-gov-1-2026-07-19/ARCHITECTURE-SPINE.md`; correction: `_bmad-output/planning-artifacts/sprint-change-proposal-2026-08-02.md`. | GOV-1 implementation and the next accepted governed release manifest. |
